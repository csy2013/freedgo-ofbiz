...
function ConstructLookup(requestUrl, inputFieldId, dialogTarget, dialogOptionalTarget, formName, width, height, position, modal, ajaxUrl, showDescription, presentation, defaultMinLength, defaultDelay, args) {
  ...
  var lookupId = window.GLOBAL_LOOKUP_REF.createNextKey();
  ...
  var dialogOpts = {
    ...
    open: function (event, ui) {
      ...
      jQuery.ajax({
        ...
        success: function (data) {
          ...
          var prevLookup = null;
          if (ACTIVATED_LOOKUP) {
            prevLookup = window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).lookupId;
          }
          identifyLookup(lookupId);

          if (prevLookup) {
            window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).prevLookup = prevLookup;
          }
        },
    ...
    close: function () {
      ...
      if (ACTIVATED_LOOKUP) {
        var prevLookup = window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).prevLookup;
      }
      if (prevLookup) {
  ...
  window.GLOBAL_LOOKUP_REF.setReference(lookupId, this);
  ...
  jQuery("#" + lookupId + "_button").click(
    function () {
      ...
      window.GLOBAL_LOOKUP_REF.getReference(lookupId).target = jQuery(dialogTarget);
      if (dialogOptionalTarget != null) {
        //the target2 have to be set here, because the input field is not created before
        window.GLOBAL_LOOKUP_REF.getReference(lookupId).target2 = jQuery(dialogOptionalTarget);
      }
      return false;
    }
  );
 ...
 * Fieldlookup Class & Methods
 * exsample :
 * LookupPartyName", "findCommEvents_partyIdFrom", input#findCommEvents_partyIdFrom.form-control.input-sm, null, "findCommEvents", "", "", "topcenter", "true", "findCommEvents_partyIdFrom,/partymgr/control/Looku…W_NAME_=main&amp;searchValueFieldName=partyIdFrom", true, "layer", "2", "300"]
 *************************************/
function ConstructBootsLookup(requestUrl, inputFieldId, dialogTarget, dialogOptionalTarget, formName, width, height, position, modal, ajaxUrl, showDescription, presentation, defaultMinLength, defaultDelay, args) {
  // add the presentation attribute to the request url to let the request know which decorator should be loaded
  if (!presentation) {
    var presentation = "layer"
  }
  ...
  // createAjax autocomplete
  //ajaxUrl :0_lookupId_findCommEvents_partyIdFrom,/partymgr/control/LookupPartyName,ajaxLookup=Y&amp;_LAST_VIEW_NAME_=main&amp;searchValueFieldName=partyIdFrom
  if (ajaxUrl != "" && showDescription != "") {
    SHOW_DESCRIPTION = showDescription;
    //write the new input box id in the ajaxUrl Array
    ajaxUrl = ajaxUrl.replace(ajaxUrl.substring(0, ajaxUrl.indexOf(",")), newInputBoxId);
  ...
  var dialogOpts = {
    ...
    bgiframe: true,
    autoOpen: false,
    height: (height != "") ? parseInt(height) : 700,
 ...
 * LookupPartyName", "findCommEvents_partyIdFrom", input#findCommEvents_partyIdFrom.form-control.input-sm, null, "findCommEvents", "", "", "topcenter", "true", "findCommEvents_partyIdFrom,/partymgr/control/Looku…W_NAME_=main&amp;searchValueFieldName=partyIdFrom", true, "layer", "2", "300"]
 *************************************/
function ConstructBootstrapModalLookup(requestUrl, inputFieldId, dialogTarget, dialogOptionalTarget, formName, width, height, position, modal, ajaxUrl, showDescription, presentation, defaultMinLength, defaultDelay, args) {
  // add the presentation attribute to the request url to let the request know which decorator should be loaded
  if (!presentation) {
    var presentation = "layer"
  }
  ...
  if (ajaxUrl != "" && showDescription != "") {
    SHOW_DESCRIPTION = showDescription;
    //write the new input box id in the ajaxUrl Array
    ajaxUrl = ajaxUrl.substring(ajaxUrl.indexOf(",") + 1);
    //console.log(ajaxUrl);
    ajaxUrl = ajaxUrl.replace(ajaxUrl.substring(0, ajaxUrl.indexOf(",")), newInputBoxId);
    new ajaxAutoCompleter(ajaxUrl, showDescription, defaultMinLength, defaultDelay, formName);
  }
  ...
  var dialogOpts = {
    show: function () {
      ...
      jQuery.ajax({
        ...
        success: function (data) {
          $('#' + lookupId).find('.modal-body').empty().append(data);
          lookupFormAction = jQuery("#" + lookupId + " form:first").attr("action");
          modifyBootsSubmitButton(lookupId);
          jQuery("#" + lookupId).bind("keypress", lookup_onKeyEnter);
          // set up the window chaining
          // if the ACTIVATED_LOOKUP var is set there have to be more than one lookup,
          // before registrating the new lookup we store the id of the old lookup in the
  ...
  window.GLOBAL_LOOKUP_REF.setReference(lookupId, this);
  // bind click Event to Dialog button
  console.log(jQuery("#"+lookupId).html());
  lookupModalLayer({
    id: lookupId,
  ...
  $(function () {
    jQuery("#" + lookupId + "_button").click(
      function () {
        jQuery('#' + lookupId).on('show.bs.modal', function () {
          dialogOpts.show();
        });
        jQuery('#' + lookupId).on('hide.bs.modal', function () {
          dialogOpts.hide();
          //jQuery('#'+lookupId).remove();
        });
        jQuery("#" + lookupId).modal('show');
        window.GLOBAL_LOOKUP_REF.getReference(lookupId).target = jQuery(dialogTarget);
        if (dialogOptionalTarget != null) {
          //the target2 have to be set here, because the input field is not created before
          window.GLOBAL_LOOKUP_REF.getReference(lookupId).target2 = jQuery(dialogOptionalTarget);
...
window.GLOBAL_LOOKUP_REF = new FieldLookupCounter;
...
function modifySubmitButton(lookupDiv) {
  ...
  if (lookupDiv) {
    ...
    jQuery.each(navPagers, function (navPager) {
      if (jQuery(navPagers[navPager]).attr("href").substring(0, 11) === 'javascript:') {
...
function lookupAjaxRequest(request) {
  ...
  lookupId = window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).lookupId;
...
function lookupFormAjaxRequest(formAction, form) {
  ...
  data = data + "&presentation=" + window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).presentation;
...
function lookupPaginationAjaxRequest(navAction, type) {
  lookupDiv = (window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).divRef);
  lookupContent = (window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).contentRef);
...
function modifyBootsSubmitButton(lookupDiv) {
  /* changes form/submit behavior for Lookup Layer */
  if (lookupDiv) {
    modifyCollapseable(lookupDiv);
    //find the lookup form and input button
    ...
    (input.parent()).append(jQuery("<button/>", {
      ...
      href: "javascript:void(0);",
      click: function () {
        lookupBootsFormAjaxRequest(formAction, lookupForm.attr("id"));
        return false;
      },
      text: txt
    }));
    ...
    jQuery.each(navPagers, function (navPager) {
      if (jQuery(navPagers[navPager]).attr("href").substring(0, 11) === 'javascript:') {
        jQuery(navPagers[navPager]).attr("href", "javascript:lookupBootsPaginationAjaxRequest('" + encodeURI(jQuery(navPagers[navPager]).attr("href")) + "','link')");
      } else {
        jQuery(navPagers[navPager]).attr("href", "javascript:lookupBootsPaginationAjaxRequest('" + encodeURI(jQuery(navPagers[navPager]).attr("href")) + "','link')");
      }
    });

    var navPagersSelect = jQuery("#" + lookupDiv + " .nav-pagesize");
    jQuery.each(navPagersSelect, function (navPager) {
      // that's quite weird maybe someone have a better idea ... that's where the magic happens
      try {
        var oc = jQuery(navPagersSelect[navPager]).attr("onchange");
        if ((typeof oc) == "function") { // IE6/7 Fix
          ...
          if (searchPattern.test(ocSub)) {
            ...
            var spl = ocSub.split(searchPattern);
            navPagersSelect[navPager].onchange = function () {
              lookupBootsPaginationAjaxRequest(spl[0] + this.value + spl[1], 'select');
            };
          } else if (searchPattern_IE.test(ocSub)) {
            var viewSize = navPagersSelect[navPager].value;
            var spl = ocSub.split(searchPattern_IE);
            ...
            navPagersSelect[navPager].onchange = function () {
              lookupBootsPaginationAjaxRequest(ocSub + this.value, 'select');
            };
          } else if (searchPattern2_IE.test(ocSub)) {
            ocSub = ocSub.replace(searchPattern2_IE, "");
            if (searchPattern_IE.test(ocSub)) {
          ...
          var ocSub = oc.substring((oc.indexOf('=') + 1), (oc.length));
          navPagersSelect[navPager].setAttribute("onchange", "lookupBootsPaginationAjaxRequest(" + ocSub + ",'')");
        }
            ...
            if (cellChild.size() > 0) {
              for (l in cellChild) {
                ...
                if (cellElement.tagName == 'A') {
                  ...
                  var liSub = link.substring(link.lastIndexOf('/') + 1, (link.length));
                  if (liSub.indexOf("javascript:set_") != -1) {
                    cellChild[child].href = link.replace("set_value", "set_bValue");
                    //cellElement.href = link;
                  } else {
                    cellElement.href = "javascript:lookupBootsAjaxRequest('" + liSub + "&presentation=layer')";
                  }
                }
              }
            }
        ...
        jQuery.each(cellChild, function (child) {
          if (cellChild[child].tagName == "A") {
            ...
            if (liSub.indexOf("javascript:set_") != -1) {
              ...
              cellChild[child].href = "javascript:lookupBootsAjaxRequest('" + liSub + "&presentation=layer')";
            }
          }
        });
 ...
 * Create an ajax Request
 */
function lookupBootsAjaxRequest(request) {
  // get request arguments
  var arg = request.substring(request.indexOf('?') + 1, (request.length));
  request = request.substring(0, request.indexOf('?'));
  lookupId = window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).lookupId;
  jQuery("#" + lookupId).find('.modal-body').load(request, arg, function (data) {
    modifyBootsSubmitButton(lookupId);
  });
}
 ...
 * @return
 */
function lookupBootsFormAjaxRequest(formAction, form) {
  lookupId = window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).lookupId;
  var data = jQuery("#" + form).serialize();
  data = data + "&presentation=" + window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).presentation;
  jQuery("#" + lookupId).find('.modal-body').load(formAction, data, function (data) {
    modifyBootsSubmitButton(lookupId);
  });
  var screenletTitleBar = jQuery("#" + lookupId + " .modal-content :visible:first");
  jQuery.ajax({
    url: formAction,
    type: "POST",
    data: data,
    ...
    success: function (result) {
      ...
      jQuery("#" + lookupId).find('.modal-body').html(result);
      modifyBootsSubmitButton(lookupId);
    }
  });
}

function lookupBootsPaginationAjaxRequest(navAction, type) {
  lookupDiv = (window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).divRef);
  lookupContent = (window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).contentRef);

  lookupId = window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).lookupId;
  jQuery("#" + lookupId).find('.modal-body').load(navAction, function (data) {
    modifyBootsSubmitButton(lookupId);
  });
  var screenletTitleBar = jQuery("#" + lookupId + " .modal-content :visible:first");
  jQuery.ajax({
...
var num_id = (re_id.exec(String(window.location)) ? Number(RegExp.$1) : 0);
//var obj_caller = (window.opener && window.opener.lookups? window.opener.lookups[num_id]: null);
var obj_caller = ((parent.window.opener != null && !parent.window.opener) ? parent.window.opener.lookups[num_id] : null);
...
function set_duration_value(value) {
  if (window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP)) {
    obj_caller.target = window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).target;
  }
  else {
...
function set_value(value) {
  if (obj_caller) {
    ...
    win.window.saveChooseValue(value, api.config.id);
    art.dialog.close();
  }
}

function set_choosevalues(that,value) {
  ...
  if (obj_caller) {
    ...
    if (!api) return;
    var win = art.dialog.open.origin; //来源页面
    win.window.saveMutilChooseValue(value, api.config.id);
    // art.dialog.close();
  }
}
// function passing selected value to calling window
function set_values(value, value2) {
  if (window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP)) {
    obj_caller.target = window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).target;
    obj_caller.target2 = window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).target2;
  }
  else {
    obj_caller.target = jQuery(obj_caller.targetW);
  }
...
// function passing selected value to calling window
function set_bValues(value, value2) {
  if (window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP)) {
    obj_caller.target = window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).target;
    obj_caller.target2 = window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).target2;
  }
...
function closeLookup() {
  if (window.opener != null && window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP) == null) {
    window.close();
  } else {
    obj = window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).lookupId;
    jQuery("#" + obj).dialog("close");
  }
}

//close the window after passing the value
function closeBootsLookup() {
  if (window.opener != null && window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP) == null) {
    window.close();
  } else {
    obj = window.GLOBAL_LOOKUP_REF.getReference(ACTIVATED_LOOKUP).lookupId;
    jQuery("#" + obj).modal("hide");
    //jQuery("#" + obj).remove();
  }
}
