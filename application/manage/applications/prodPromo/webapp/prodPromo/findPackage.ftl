...
<#assign commonUrl = "findPackage?lookupFlag=Y&"+ paramList +"&">
<#assign lookupFlag = "Y"/>
...
<div class="box box-info">
    <div class="box-body">
        ...
        <form class="form-inline clearfix" role="form" method="post" id="QueryForm"
              action="<@ofbizUrl>findPackage</@ofbizUrl>">
        ...
        <ul class="nav nav-tabs">
            <li role="presentation" <#if promoStatus?default("") == "">class="active"</#if>><a
                    href="<@ofbizUrl>findPackage?lookupFlag=Y</@ofbizUrl>">全部</a></li>
                                    ...
                                    <ul class="dropdown-menu" role="menu">
                                        <#if (security.hasEntityPermission("PRODPROMO_DISCOUNT", "_UPDATE", session) && (promo.promoStatus=='ACTY_AUDIT_NOPASS'))>
                                            <li><a href="javascript:void(0)"
                                                   onclick="editPackage(${promo.productPromoId})">编辑</a></li>
                                        </#if>
        ...
        <div id="modal_detailReduce" class="modal fade  bs-example-modal-lg" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog modal-lg" role="document">
               ...
               <span id="fromDate"><span>
                        ...
                        <div class="row title">
                            <div class="form-group col-sm-3">
                                <label for="subTitle" class="col-sm-6 control-label">包邮类型:</label>
                                <div class="col-sm-6">
            ...
            <script>
                $(function () {
                    ...
                    $("#btn_add").click(function () {
                        window.location.href = "<@ofbizUrl>addPackage</@ofbizUrl>";
                    });
                ...
                function deletePromo(productPromoId) {
                    ...
                    $('#modal_confirm #modal_confirm_body').html("确定删除此包邮吗？");
                ...
                function editPackage(productPromoId) {
                    window.location.href = "<@ofbizUrl>editPackage?productPromoId=" + productPromoId+"</@ofbizUrl>";
                }
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.productPromo) {
                                ...
                                for(var i=0;i<productPromoConds.length;i++){
                                    var parmoType =  productPromoConds[i].inputParamEnumId;
                                    // var condValue = productPromoConds[i].condValue;
                                }

                                // CKEDITOR.instances.Centent.setData(productPromo.promoText);
                                // $('#modal_detailReduce #condValue').text(productPromo.condValue);

                                // var parmoType =  productPromoConds.inputParamEnumId;
                                // var condValue =  productPromoConds.condValue;

                                console.log('parmoTyp1--->'+parmoType);
                                var temp = "";
                                if(parmoType == 'PPIP_ORDER_TOTAL'){
                                    console.log('parmoTyp1-2--->'+parmoType);
                                    temp = "按订单金额包邮";
                                ...
                                // var packageType="满 "+condValue+" "+temp+"包邮";
                                // $("#modal_detailReduce #packageType").text(packageType);
                                ...
                                for (var i = 0; i < product_PromoActions.length; i++) {
                                    for (var j = 0; j < product_PromoConds.length; j++) {
                                        if (product_PromoActions[i].productPromoCondSeqId == product_PromoConds[j].productPromoCondSeqId) {
                                            rule += "满" + product_PromoConds[j].condValue + ParamEnumId + "包邮,";
                                        }
                                    }
                                }
