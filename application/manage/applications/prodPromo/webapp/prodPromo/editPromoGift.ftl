...
<form class="form-horizontal" role="form" method="post" name="addPromoReduce" id="addPromoReduce"
    ...
    <div class="box box-info">
        ...
        <div class="box-body">
            ...
            <div class="row">
                <div class="form-group col-sm-6" id="giftCond" data-type="format" data-number="8"
                     data-reg="/^(\d{1,8})(\.?)(\d{1,2})$/" data-msg="价格请填写数字，最多999999.99">
                    ...
                    <div class="col-md-3"><#--促销条件枚举值-->
                        <select class="form-control  dp-vd" id="paramEnumId" name="paramEnumId">
                            <option value="PPIP_ORDER_TOTAL"
                                    <#if 'PPIP_ORDER_TOTAL' == productPromoCond.inputParamEnumId>selected=selected</#if>>
                                元
                            </option>
                            <option value="PPIP_PRODUCT_QUANT"
                                    <#if 'PPIP_PRODUCT_QUANT' == productPromoCond.inputParamEnumId>selected=selected</#if>>
                                件
                            </option>
                        </select>
                    </div>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        var productIds = '${productIds}';//选择促销的商品
        ...
        var productIds1 = '${productIds1}';//赠送的商品
        if (productIds1 != null && productIds1 != "") {
            $("#linkId1").val(productIds1)
            //商品回显
            getProductGoodsInfoListByIds1(productIds1)
        }
    });
    ...
    function getProductGoodsInfoListByIds1(ids) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#selectName1").val(curProductNames1)
            },
            error: function (data) {
                $.tipLayer("操作失败！");
            }
    ...
    function addPromoReduce() {
        ...
        var promo_CondAction = $("#paramEnumId").val() + "," + $("#condValue").val();
        $.ajax({
            url: "updatePromoGiftService",//addPromoGiftService
            type: "POST",
            async: false,
            data: {
