...
<#assign commonUrl = "findPromoReduce?lookupFlag=Y&"+ paramList +"&">
<#assign lookupFlag = "Y"/>
...
<div class="box box-info">
    <div class="box-body">
        ...
        <form class="form-inline clearfix" role="form" method="post" id="QueryForm"
              action="<@ofbizUrl>findPromoReduce</@ofbizUrl>">
        ...
        <div class="row m-b-12" style="margin-bottom:15px;">
            <div class="col-sm-6">
                ...
                <#if security.hasEntityPermission("PRODPROMO_REDUCE", "_DELETE",session)>
                    <button id="btn_add" class="btn btn-primary">
                        <i class="fa fa-plus"></i>添加
                    </button>
                </#if>
                <#--<#if (security.hasEntityPermission("PRODPROMO_REDUCE", "_UPDATE", session))&&(promoStatus=='ACTY_AUDIT_INIT')>
        ...
        <ul class="nav nav-tabs">
            <li role="presentation" <#if promoStatus?default("") == "">class="active"</#if>><a
                    href="<@ofbizUrl>findPromoReduce?lookupFlag=Y</@ofbizUrl>">全部</a></li>
                ...
                <table class="table table-bordered table-hover js-checkparent" id="prom_table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><#if promo.promoType=='PROMO_PRE_REDUCE'>每满减<#else>满减</#if></td>
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu" role="menu">
                                        <#if (security.hasEntityPermission("PRODPROMO_REDUCE", "_UPDATE", session) && (promo.promoStatus=='ACTY_AUDIT_NOPASS'))>
                                            <li><a href="javascript:void(0)" onclick="editPromoReduce('${promo.productPromoId}','N')">编辑</a></li>
                                        </#if>
                                        <#if (security.hasEntityPermission("PRODPROMO_REDUCE", "_UPDATE", session) && (promo.promoStatus=='ACTY_AUDIT_PASS'))>
                                            <li><a href="javascript:void(0)" onclick="editPromoReduce('${promo.productPromoId}','Y')">编辑</a></li>
                                        </#if>
                                        <#if (security.hasEntityPermission("PRODPROMO_REDUCE", "_UPDATE", session) && (promo.promoStatus=='ACTY_AUDIT_INIT'))>
                                            <li><a href="javascript:void(0)"  onclick="auditPromoReduce(${promo.productPromoId})">审核</a></li>
                                        </#if>
                                        <#if (security.hasEntityPermission("PRODPROMO_REDUCE", "_UPDATE", session) && (promo.promoStatus=='ACTY_AUDIT_PASS'))>
                                            <li><a href="javascript:void(0)"  onclick="editEndDate(${promo.productPromoId},'${promo.thruDate?string("yyyy-MM-dd HH:mm:ss")}','${promo.fromDate?string("yyyy-MM-dd HH:mm:ss")}')">编辑结束时间</a>
                                            </li>
                                        </#if>
                                        <#if (security.hasEntityPermission("PRODPROMO_REDUCE", "_DELETE", session))>
                                            <li><a href="javascript:void(0)" onclick="deletePromo(${promo.productPromoId})">删除</a></li>
                                        </#if>
                                        <#if (security.hasEntityPermission("PRODPROMO_REDUCE", "_LIST", session))>
                                            <li><a href="javascript:void(0)" onclick="auditLog(${promo.productPromoId})">操作日记</a></li>
                                        </#if>
                                        <#if (security.hasEntityPermission("PRODPROMO_REDUCE", "_LIST", session) && (promo.promoStatus=='ACTY_AUDIT_NOPASS'))>
                                            <li><a href="javascript:void(0)" onclick="findAuditMessage('${promo.productPromoId}')">查看驳回原因</a></li>
                                        </#if>
                                    </ul>
                                </div>
                            </td>
                        ...
                        <form id="ApprovalForm" method="post" class="form-horizontal"
                            ...
                            <div class="row">
                                <div class="form-group " data-type="required,max" data-mark="审核意见" data-number="50">
                                    ...
                                    <div class="col-sm-8">
                                        <textarea id="auditMessage"  name="auditMessage"  class="form-control dp-vd" rows="3" style="resize: none;"></textarea>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row title">
                            ...
                            <div class="form-group col-sm-4">
                                ...
                                <div class="col-sm-6">
                                    <span id="promoName"  style="word-wrap: break-word"><span>
                                </div>
                            </div>
                        ...
                        <div class="row title">
                            <div class="form-group col-sm-3">
                                <label for="subTitle" class="col-sm-6 control-label">满减类型:</label>
                                <div class="col-sm-6">
            ...
            <script>
                $(function () {
                    ...
                    $("#btn_add").click(function () {
                        window.location.href = "<@ofbizUrl>addPromoReduce</@ofbizUrl>";
                    });
                ...
                function editEndDate(productPromoId, endDate, startDate) {
                    //先校验是否在有效期内修改时间，如果不是则判断商品是否已经存在其他活动中
                ...
                function editPromoReduce(productPromoId,isPass) {
                    window.location.href = "<@ofbizUrl>editPromoReduce?productPromoId=" + productPromoId +"&isPass="+isPass+"</@ofbizUrl>";
                }
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.productPromo) {
                                ...
                                if (productPromo.promoType == 'PROMO_PRE_REDUCE') {
                                    $('#modal_detailReduce #promoType').text('每满减');
                                } else {
                                    $('#modal_detailReduce #promoType').text('满减');
                                }
                                ...
                                if (data.productStorePromoAppls) {
                                    ...
                                    $('#modal_detailReduce #fromDate').text(timeStamp2String(productStorePromoAppl.fromDate.time));
                                    $('#modal_detailReduce #thruDate').text(timeStamp2String(productStorePromoAppl.thruDate.time));
                                }
                                var condActList = data.condActList;
                                var rule = "";
                                for (var i = 0; i < condActList.length; i++) {
                                    rule += "满" + condActList[i].condValue + "减" + condActList[i].amount + ",";
                                }
