...
public class ContentUrlFilter extends ContextFilter {
            ...
            if (alternativeUrl.endsWith("-content")) {
                try {
                    ...
                    if (contentDataResourceViews.size() > 0) {
                        ...
                        List<GenericValue> contents = EntityUtil.filterByDate(
                                EntityUtil.orderBy(delegator.findByAnd("ContentAssoc", UtilMisc.toMap("contentAssocTypeId", "ALTERNATIVE_URL", "contentIdTo", contentDataResourceView.getString("contentId"))),UtilMisc.toList("createDate DESC")));
                        if (contents.size() > 0) {
    ...
    public static String makeContentAltUrl(HttpServletRequest request, HttpServletResponse response, String contentId, String viewContent) throws UnsupportedEncodingException {
        request.setCharacterEncoding("utf-8");
