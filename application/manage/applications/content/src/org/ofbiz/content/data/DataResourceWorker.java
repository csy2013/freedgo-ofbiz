...
import org.ofbiz.widget.screen.*;
...
public class DataResourceWorker implements org.ofbiz.widget.DataResourceWorkerInterface {
    ...
    public static File getContentFile(String dataResourceTypeId, String objectInfo, String contextRoot) throws GeneralException, FileNotFoundException {
        ...
        if ("OFBIZ_URL_FILE".equals(dataResourceTypeId) || "OFBIZ_URL_FILE_BIN".equals(dataResourceTypeId)) {
            String prefix = System.getProperty("ofbiz.home");
            String sep = "";
            if (objectInfo.indexOf("/") != 0 && prefix.lastIndexOf("/") != (prefix.length() - 1)) {
                sep = "/";
            }
            String imagePath = UtilProperties.getPropertyValue("content.properties", "content.webapp.path");
            file = FileUtil.getFile(prefix + "/" + imagePath + objectInfo);
            if (!file.exists()) {
    ...
    /**
     * 获取对应的images datasource 路径 images/datasource/232332232...
     *
     * @return
     */
    public static String getDataResourceContentUploadPathUrl() {
        String frameworkOfbizUrl = getDataResourceContentUploadPath(false);
        String ofbizUrlFilePath = UtilProperties.getPropertyValue("content.properties", "content.ofbiz.url.file.path");
        if (frameworkOfbizUrl.indexOf(ofbizUrlFilePath) != -1) {
            frameworkOfbizUrl = frameworkOfbizUrl.substring(frameworkOfbizUrl.indexOf(ofbizUrlFilePath) + ofbizUrlFilePath.length());
        }
        return frameworkOfbizUrl;
    }
    
    /**
     * 获取tenant 对应的images datasource 路径 images/datasource/232332232...
     *
     * @return
     */
    public static String getDataResourceContentUploadPathUrl(Map<String, Object> context) {
        String frameworkOfbizUrl = getDataResourceContentUploadPath(false, context);
        String ofbizUrlFilePath = UtilProperties.getPropertyValue("content.properties", "content.ofbiz.url.file.path");
        if (frameworkOfbizUrl.indexOf(ofbizUrlFilePath) != -1) {
            frameworkOfbizUrl = frameworkOfbizUrl.substring(frameworkOfbizUrl.indexOf(ofbizUrlFilePath) + ofbizUrlFilePath.length());
        }
...
//        FlexibleStringExpander.expandString(UtilProperties.getPropertyValue("catalog", "image.server.path"), context);
     ...
     * @return
     */
    public static String getDataResourceContentUploadPath(boolean absolute, Map<String, Object> context) {
        String initialPath = FlexibleStringExpander.expandString(UtilProperties.getPropertyValue("content.properties", "content.upload.path.prefix"), context);
//        String initialPath = UtilProperties.getPropertyValue("content.properties", "content.upload.path.prefix");
        double maxFiles = UtilProperties.getPropertyNumber("content.properties", "content.upload.max.files");
        if (maxFiles < 1) {
            maxFiles = 250;
    ...
    public static String getDataResourceContentUploadPath(String initialPath, double maxFiles, boolean absolute) {
        ...
        // first item in map is the most current directory 排除svn的目录 .svn
        File latestDir = null;
        if (UtilValidate.isNotEmpty(dirMap)) {
            // 最后一个目录可能会出现svn的目录.svn需要排除
            Iterator it = dirMap.entrySet().iterator();
            boolean isCreate = false;
            while (it.hasNext()) {
                //   // entry的输出结果如key0=value0等
                Map.Entry entry = (Map.Entry) it.next();
                latestDir = (File) entry.getValue();
                if (latestDir != null && (!".svn".equals(latestDir.getName())) && (!"output".equals(latestDir.getName()))) {
    ...
    public static void renderDataResourceAsText(Delegator delegator, String dataResourceId, Appendable out,
        ...
        if (cache) {
            ...
            if (disableCache == null || !"true".equalsIgnoreCase(disableCache)) {
                try {
    ...
    public static void writeDataResourceText(GenericValue dataResource, String mimeTypeId, Locale locale, Map<String, Object> templateContext,
        ...
        //qiniu 存储的可能是SHORT_TEXT,ELECTRONIC_TEXT,IMAGE_OBJECT,LINK,URL_RESOURCE,CONTEXT_FILE,LOCAL_FILE,OFBIZ_FILE
        //文件上传到服务器之后(备份使用)，选择是否上传到云存储
        String uploadType = UtilProperties.getPropertyValue("content", "content.image.upload.type");
        String qiniuObjectInfo = "";
        String ftpObjectInfo = "";
        if ("qiniu".equals(uploadType)) {
            qiniuObjectInfo = (String) dataResource.get("qiniuObjectInfo");
        } else if ("FTP".equals(uploadType)) {
            ftpObjectInfo = (String) dataResource.get("ftpObjectInfo");
            if(UtilValidate.isEmpty(ftpObjectInfo)){
                ftpObjectInfo = dataResource.getString("objectInfo");
            }
        }
        
        if (UtilValidate.isNotEmpty(qiniuObjectInfo)) {
            writeText(dataResource, qiniuObjectInfo, templateContext, mimeTypeId, locale, out);
        } else if (UtilValidate.isNotEmpty(ftpObjectInfo)) {
            writeText(dataResource, ftpObjectInfo, templateContext, mimeTypeId, locale, out);
        } else {
            // default type
            ...
            if ("SHORT_TEXT".equals(dataResourceTypeId) || "LINK".equals(dataResourceTypeId)) {
                ...
                if (url != null && url.getHost() != null) { // is absolute
                ...
                if (dataResourceMimeTypeId == null || dataResourceMimeTypeId.startsWith("text")) {
                    ...
                    writeText(dataResource, objectInfo, templateContext, mimeTypeId, locale, out);
                }
            } else {
...
//      add by changsy 20160606 所有的mimeTypeId 对应都是取textData
            out.append(textData);
        } else {
    ...
    public static Map<String, Object> getDataResourceStream(GenericValue dataResource, String https, String webSiteId, Locale locale, String contextRoot, boolean cache) throws IOException, GeneralException {
        ...
        if (dataResourceTypeId.endsWith("_TEXT") || "LINK".equals(dataResourceTypeId)) {
            ...
            if(text.startsWith("/images/products/")){
                URL url = null;
                String imagePath = UtilProperties.getPropertyValue("content.properties", "kaide.images.baseUrl");
                text = imagePath + text;
                String newUrl = "";
                String uploadType = UtilProperties.getPropertyValue("content", "content.image.upload.type");
                if ("qiniu".equals(uploadType)) {
                    newUrl = UtilProperties.getPropertyValue("content", "img.qiniu.domain");
                    newUrl = newUrl + dataResource.getString("qiniuObjectInfo");
                    url = new URL(newUrl);
                } else if ("FTP".equals(uploadType)) {
                    newUrl = UtilProperties.getPropertyValue("content", "content.image.upload.ftp.domain");
                    newUrl = newUrl + (dataResource.getString("ftpObjectInfo")==null?dataResource.getString("objectInfo"):dataResource.getString("ftpObjectInfo"));
                    url = new URL(newUrl);
                } else {
                    newUrl =  text;
                    url = new URL(newUrl);
            ...
            // file data
        } else if (dataResourceTypeId.endsWith("OFBIZ_URL_FILE")) {
            String objectInfo = dataResource.getString("objectInfo");
            if (UtilValidate.isNotEmpty(objectInfo)) {
                String imagePath = UtilProperties.getPropertyValue("content.properties", "kaide.images.baseUrl");
                objectInfo = imagePath + objectInfo;
                URL url = null;
                String newUrl = "";
                String uploadType = UtilProperties.getPropertyValue("content", "content.image.upload.type");
                if ("qiniu".equals(uploadType)) {
                    ...
                    newUrl = UtilProperties.getPropertyValue("content", "content.image.upload.ftp.domain");
                    String ftpObjectInfo = dataResource.getString("ftpObjectInfo");
                    if(UtilValidate.isEmpty(ftpObjectInfo)){
                        ftpObjectInfo = dataResource.getString("objectInfo");
                    }
                    newUrl = newUrl + ftpObjectInfo;
                    url = new URL(newUrl);
                } else {
                    newUrl =  objectInfo;
                    url = new URL(newUrl);
            ...
            // URL resource data
        } else if ("URL_RESOURCE".equals(dataResourceTypeId) || "OFBIZ_URL_FILE".equals(dataResourceTypeId)) {
            String objectInfo = dataResource.getString("objectInfo");
            URL url = null;
            if (UtilValidate.isNotEmpty(objectInfo)) {
                //http, https, ftp, file, and jar
                if (objectInfo.startsWith("https") || objectInfo.startsWith("http") || objectInfo.startsWith("ftp") || objectInfo.startsWith("file") || objectInfo.startsWith("ftp")) {
                    url = new URL(objectInfo);
                } else {
                    String newUrl = "";
                    String uploadType = UtilProperties.getPropertyValue("content", "content.image.upload.type");
                    if ("qiniu".equals(uploadType)) {
                        ...
                        newUrl = DataResourceWorker.buildRequestPrefix(delegator, locale, webSiteId, https);
                        if (!newUrl.endsWith("/")) {
                            newUrl = newUrl + "/";
                        }
                        newUrl = newUrl + objectInfo;
                        url = new URL(newUrl);
                    }
    ...
    public static void streamDataResource(OutputStream os, Delegator delegator, String dataResourceId, String https, String webSiteId, Locale locale, String rootDir) throws IOException, GeneralException {
        try {
            ...
            } else if ("IMAGE_OBJECT".equals(dataResourceTypeId)) {
                byte[] imageBytes = acquireImage(delegator, dataResource);
                if (imageBytes != null) {
                    os.write(imageBytes);
                }
            } else if ("LINK".equals(dataResourceTypeId)) {
                String text = dataResource.getString("objectInfo");
                os.write(text.getBytes());
            } else if ("URL_RESOURCE".equals(dataResourceTypeId)) {
