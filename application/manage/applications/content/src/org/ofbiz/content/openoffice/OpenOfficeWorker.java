...
public class OpenOfficeWorker{
     ...
     */
    public static XMultiComponentFactory getRemoteServer(String host, String port) throws Exception {

        if (UtilValidate.isEmpty(host)) {
            host = UtilProperties.getPropertyValue("openoffice-uno", "oo.host", "localhost");
        }
        if (UtilValidate.isEmpty(port)) {
            port = UtilProperties.getPropertyValue("openoffice-uno", "oo.port", "8100");
        }
    ...
    public static void convertOODocToFile(XMultiComponentFactory xmulticomponentfactory, String fileInPath, String fileOutPath, String outputMimeType) throws Exception {
        // Converting the document to the favoured type
