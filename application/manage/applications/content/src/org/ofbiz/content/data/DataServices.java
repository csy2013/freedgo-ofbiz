...
public class DataServices {
    ...
    public static Map<String, Object> createFileMethod(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        if (UtilValidate.isEmpty(dataResourceTypeId) || "LOCAL_FILE".equals(dataResourceTypeId) || "LOCAL_FILE_BIN".equals(dataResourceTypeId)) {
            ...
            absFilePath = objectInfo;
            saveImageKey = objectInfo;
            ...
            if (objectInfo.indexOf(File.separator) != 0 && prefix.lastIndexOf(File.separator) != (prefix.length() - 1)) {
                sep = File.separator;
            }
            file = new File(prefix + sep + objectInfo);
            absFilePath = prefix + sep + objectInfo;
            saveImageKey = objectInfo;
        } else if ("OFBIZ_URL_FILE".equals(dataResourceTypeId)) {
            prefix = System.getProperty("ofbiz.home");
            if (objectInfo.indexOf(File.separator) != 0 && prefix.lastIndexOf(File.separator) != (prefix.length() - 1)) {
                sep = File.separator;
            }
            String ofbizUrlFilePath = UtilProperties.getPropertyValue("content.properties", "content.ofbiz.url.file.path");
            file = new File(prefix + ofbizUrlFilePath + sep + objectInfo);
            absFilePath = prefix + ofbizUrlFilePath + sep + objectInfo;
            saveImageKey = objectInfo;
        ...
        if (filePath.lastIndexOf(File.separator) != -1) {
            filePath = filePath.substring(0, filePath.lastIndexOf(File.separator) + 1); // adding 1 to include the trailing slash
        }
        File targetDir = new File(filePath);
        if (!FileUtil.exists(targetDir)) {
            boolean created = targetDir.mkdirs();
            if (!created) {
                String errMsg = UtilProperties.getMessage(resource, "createFileMethod.unable_to_create_target_directory", locale) + " - " + prefix + sep;
        ...
        if (UtilValidate.isNotEmpty(textData)) {
            try {
                ...
                if ("qiniu".equals(uploadType)) {
                    dctx.getDispatcher().runAsync("coverUpload", UtilMisc.toMap("filePath", absFilePath, "fileKey", saveImageKey));
                }else if ("FTP".equals(uploadType)) {
                    String saveImagePath = file.getAbsolutePath();
                    
                    saveImagePath = saveImagePath.replace(File.separator,"/");
                    dctx.getDispatcher().runSync("ftpUpload", UtilMisc.toMap("filePath", saveImagePath));
                }
            } catch (IOException e) {
            ...
            try {
                ...
                if ("qiniu".equals(uploadType)) {
                    ...
                    dctx.getDispatcher().runSync("ftpUpload", UtilMisc.toMap("filePath", absFilePath));
                }
