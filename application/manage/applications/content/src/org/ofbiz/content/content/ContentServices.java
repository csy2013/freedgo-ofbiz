...
public class ContentServices {
    ...
    public static Map<String, Object> createContentMethod(DispatchContext dctx, Map<String, ? extends Object> rcontext) {
        ...
        if (Debug.infoOn()) {
            Debug.logInfo("in createContentMethod, targetOperationList: " + targetOperationList, null);
        }
        ...
        if (Debug.infoOn()) {
            Debug.logInfo("in createContentMethod, context: " + context, null);
        }
    ...
    public static String lookupSpecialPage(HttpServletRequest request, HttpServletResponse response) {
        String pageName = request.getParameter("name");
        String pageNo = request.getParameter("pageNo");
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        Map serviceIn = FastMap.newInstance();
        serviceIn.put("entityName", "SpecialPage");
        if (UtilValidate.isNotEmpty(pageName)) {
            serviceIn.put("inputFields", UtilMisc.toMap("pageName", pageName, "pageName_op", "contains", "pageName_ic", "Y"));
            
        } else {
            serviceIn.put("inputFields", UtilMisc.toMap("pageName", ""));
        }
    ...
    public static String createWebSiteTemplate(HttpServletRequest request, HttpServletResponse response) {
        String type = request.getParameter("type");
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        Map reqMap = UtilHttp.getParameterMap(request);
        
        //判断当前是否存在temlateId
        String indexTempalteId = request.getParameter("templateId");
        try {
            if (UtilValidate.isNotEmpty(indexTempalteId)) {
                GenericValue gv = delegator.makeValue("WebSiteIndexTemplate");
                gv.setNextSeqId();
                gv.put("webSiteId", "daojia");
                gv = gv.create();
                indexTempalteId = gv.getString("siteIndexTemplateId");
            }
            if ("RollAdv".equals(type)) {
                //需要轮播图的contentIds，定义
                //[10001,store],10003,goods|32323,activity|334,subject
                Object advHref = reqMap.get("rollImgAdvHref");
                if (advHref instanceof String[]) {
                    //创建advert
                    GenericValue advert = delegator.makeValue("Advert");
                    advert.put("advertTypeId", "slidePic");
                    advert.put("webSiteId", "daojia");
                    advert.put("dateCreated", UtilDateTime.nowTimestamp());
                    advert.setNextSeqId();
                    advert = advert.create();
                    String advertId = advert.getString("advertId");
                    String[] advs = (String[]) advHref;
                    if (UtilValidate.isNotEmpty(advs)) {
                        //http://changsy.cn//images/www_changsy_cn/datasource/1471857489323/10086.jpeg
                        Object imgPath = reqMap.get("rollImgAdvSrc");
                        String[] imgArr = null;
                        if (imgPath instanceof String[]) {
                            imgArr = (String[]) imgPath;
                        }
                        
                        for (int i = 0; i < advs.length; i++) {
                            String adv = advs[i];
                            String[] advStrs = adv.split(",");
                            String id = advStrs[0];
                            String idType = advStrs[1];
                            String imagePath = imgArr[i];
                            //创建advertContent
                            GenericValue advertContent = delegator.makeValue("AdvertContent");
                            advertContent.put("advertId", advertId);
                            advertContent.put("contentId", "");
                            advertContent.put("fromDate", UtilDateTime.nowTimestamp());
                            advertContent.put("sequenceNum", i + 1);
                            advertContent.put("defineType", "A");
                            advertContent.setNextSeqId();
    ...
    public static Map<String, Object> addImageResourceToContent(DispatchContext dcx, Map<String, ? extends Object> context) {
        String imageUrl = (String) context.get("imageUrl");
        String qiniuImageUrl = (String) context.get("qiniuImageUrl");
        ...
        if (UtilValidate.isNotEmpty(imageUrl) && imageUrl.length() > 0) {
            ...
            if (UtilValidate.isNotEmpty(contentId)) {
                ...
                try {
                    dataResourceResult = dispatcher.runSync("createDataResource", dataResourceCtx);
                    result.put("dataResourceId",dataResourceResult.get("dataResourceId"));
                } catch (GenericServiceException e) {
                    Debug.logError(e, module);
                    return ServiceUtil.returnError(e.getMessage());
                }
                ...
                try {
                    ...
                    result.put("contentId",contentResult.get("contentId"));
                } catch (GenericServiceException e) {
                    Debug.logError(e, module);
                    return ServiceUtil.returnError(e.getMessage());
                }
    ...
    /**
     * 更新分类导航 add by dongxiao 2016.4.14
     * @param dctx
     * @param context
     * @return
     */
    public static Map<String, Object> updateNavigation(DispatchContext dctx,Map<String, ? extends Object> context) throws GenericEntityException {
        Delegator delegator = dctx.getDelegator();
        String id = (String) context.get("id");
        String parentId =   UtilValidate.isNotEmpty(context.get("parentId")) ? (String) context.get("parentId") : null;
        String navigationGroupId =   UtilValidate.isNotEmpty(context.get("navigationGroupId")) ? (String) context.get("navigationGroupId") : null;
        String navigationName = (String) context.get("navigationName");
        String productCategory = (String) context.get("productCategory");
        String contentId = (String) context.get("contentId");
        String isEnable = (String) context.get("isEnabled");
        String sequence = "";
        int zIndex = (Integer) context.get("zIndex");
        String flag = "S";
        List<GenericValue> toGetMaxSequences = null;
        try {
            toGetMaxSequences = delegator.findByAnd("Navigation",UtilMisc.toMap("parentId",parentId,"navigationGroupId",navigationGroupId),UtilMisc.toList("-sequence"));
        } catch (GenericEntityException e) {
            e.printStackTrace();
            flag = "F";
        }
        if (id == null){                            //新增，或者添加子分类
            id = delegator.getNextSeqId("Navigation");
            if (zIndex == 0){
                if (UtilValidate.isEmpty(toGetMaxSequences)){
                    sequence = "1";
                }else{
                    sequence = (Integer.parseInt(EntityUtil.getFirst(toGetMaxSequences).getString("sequence")) + 1) + "";
                }
            }else if (zIndex == 1){      //1表示第一级添加子分类
                if (UtilValidate.isEmpty(toGetMaxSequences)){
                    String s = delegator.findByPrimaryKey("Navigation",UtilMisc.toMap("id",parentId)).getString("sequence");
                    sequence =  s + ".1";
                }else{
                    String[] arrayStr = EntityUtil.getFirst(toGetMaxSequences).getString("sequence").split("\\.");
                    sequence = arrayStr[0] + "." + (Integer.parseInt(arrayStr[1]) + 1);
                }
            }else if (zIndex == 2){
                if (UtilValidate.isEmpty(toGetMaxSequences)){
                    String s = delegator.findByPrimaryKey("Navigation",UtilMisc.toMap("id",parentId)).getString("sequence");
                    ...
                    String[] arrayStr = EntityUtil.getFirst(toGetMaxSequences).getString("sequence").split("\\.");
                    sequence = arrayStr[0] + "." + arrayStr[1] + "." + (Integer.parseInt(arrayStr[2]) + 1);
                }
            }
        }else{
            sequence =  delegator.findByPrimaryKey("Navigation",UtilMisc.toMap("id",id)).getString("sequence");
        }
        
        GenericValue genericValue = delegator.makeValue("Navigation",UtilMisc.toMap("id",id,"parentId",parentId,"navigationGroupId",navigationGroupId,"navigationName",navigationName,"productCategoryId",productCategory,
                "navigationImg",contentId,"isEnable",isEnable,"sequence",sequence));
        delegator.createOrStore(genericValue);
        Map<String,Object> result = ServiceUtil.returnSuccess();
        result.put("flag", flag);
        result.put("id", id);
        result.put("navigationName", navigationName);
        result.put("productCategory", productCategory);
        result.put("contentId", contentId);
        result.put("isEnabled", isEnable);
        return result;
    }
    
    /**
     * 上下移动分类导航 add by dongxiao 2016.4.14
     * @param dctx
     * @param context
     * @return
     */
    public static Map<String, Object> navigationMove(DispatchContext dctx,Map<String, ? extends Object> context) throws GenericEntityException {
        String flag = "S";
        Delegator delegator = dctx.getDelegator();
        String id = (String) context.get("id");
        String moveId =   UtilValidate.isNotEmpty(context.get("moveId")) ? (String) context.get("moveId") : null;
        int zIndex = (Integer) context.get("zIndex");
        GenericValue current = null;
        GenericValue move = null;
        current = delegator.findByPrimaryKey("Navigation",UtilMisc.toMap("id",id));
        move = delegator.findByPrimaryKey("Navigation", UtilMisc.toMap("id", moveId));
        List<GenericValue> toStoreList = new LinkedList<GenericValue>();
        if (zIndex == 1){
            String currentSeqRoot =  move.getString("sequence");
            String moveSeqRoot =  current.getString("sequence");
            current.setString("sequence", currentSeqRoot);
            move.setString("sequence", moveSeqRoot);
            toStoreList.add(current);
            toStoreList.add(move);
            List<GenericValue> currentSecondList = delegator.findByAnd("Navigation",UtilMisc.toMap("parentId",current.getString("id")));
            for (GenericValue s : currentSecondList){
                String secondSeqence = s.getString("sequence");
                String lastSIndex = secondSeqence.substring(secondSeqence.lastIndexOf("."), secondSeqence.length());
                secondSeqence = current.getString("sequence") + lastSIndex;
                s.setString("sequence", secondSeqence);
                toStoreList.add(s);
                List<GenericValue> currentThirdList = delegator.findByAnd("Navigation",UtilMisc.toMap("parentId",s.getString("id")));
                for (GenericValue t : currentThirdList){
                    String tSeqence = t.getString("sequence");
                    String lastTIndex = tSeqence.substring(tSeqence.lastIndexOf("."), tSeqence.length());
                    tSeqence =  s.getString("sequence") + lastTIndex;
                    t.setString("sequence", tSeqence);
                    toStoreList.add(t);
                }
            }
            List<GenericValue> moveSecondList = delegator.findByAnd("Navigation",UtilMisc.toMap("parentId",move.getString("id")));
            for (GenericValue s : moveSecondList){
                String secondSeqence = s.getString("sequence");
                String lastSIndex = secondSeqence.substring(secondSeqence.lastIndexOf("."), secondSeqence.length());
                secondSeqence = move.getString("sequence") + lastSIndex;
                s.setString("sequence", secondSeqence);
                toStoreList.add(s);
                List<GenericValue> moveThirdList = delegator.findByAnd("Navigation",UtilMisc.toMap("parentId",s.getString("id")));
                for (GenericValue t : moveThirdList){
                    String tSeqence = t.getString("sequence");
                    String lastTIndex = tSeqence.substring(tSeqence.lastIndexOf("."), tSeqence.length());
                    tSeqence = s.getString("sequence") + lastTIndex;
                    t.setString("sequence", tSeqence);
...
//        result.put("id", id);
    ...
    /**
     * 删除分类导航 add by dongxiao 2016.4.18
     * @param dctx
     * @param context
     * @return
     */
    public static Map<String, Object> delNavigation(DispatchContext dctx,Map<String, ? extends Object> context) throws GenericEntityException {
        String flag = "S";
        Delegator delegator = dctx.getDelegator();
        String id = (String) context.get("id");
        int zIndex = (Integer) context.get("zIndex");
        GenericValue navigation = delegator.findByPrimaryKey("Navigation", UtilMisc.toMap("id",id));
        List<EntityCondition> conditions = new LinkedList<EntityCondition>();
        conditions.add(EntityCondition.makeCondition("sequence",EntityOperator.GREATER_THAN,navigation.get("sequence")));
        conditions.add(EntityCondition.makeCondition("parentId",EntityOperator.EQUALS   ,navigation.get("parentId")));
        List<GenericValue> siblings = delegator.findList("Navigation",EntityCondition.makeCondition(conditions,EntityOperator.AND) ,null,null,null,false);
        if (UtilValidate.isNotEmpty(siblings)){
            if (zIndex == 1){
                delegator.removeByAnd("NavigationBrand",UtilMisc.toMap("navigationId",id));
                delegator.removeByAnd("NavigationBanner",UtilMisc.toMap("navigationId",id));
//                for (GenericValue s : siblings){
//                    String newSeq = (Integer.parseInt(s.getString("sequence")) - 1) + "";
//                    updateNavigationAndChildrenSequence(s.getString("id"), newSeq, delegator);
//                }
            }else if (zIndex == 2){
//                for (GenericValue s : siblings){
//                    String[] sequenceArray = s.getString("sequence").split("\\.");
//                    String newSeq = sequenceArray[0] + "." +  (Integer.parseInt(sequenceArray[1]) - 1);
//                    updateNavigationAndChildrenSequence(s.getString("id"), newSeq, delegator);
//                }
            }else if (zIndex == 3){
                delegator.removeByAnd("NavigationProduct", UtilMisc.toMap("navigationId", id));
//                for (GenericValue s : siblings){
//                    String[] sequenceArray = s.getString("sequence").split("\\.");
//                    String newSeq = sequenceArray[0] + "."  + sequenceArray[1] + "."  + (Integer.parseInt(sequenceArray[2]) - 1);
//                    s.set("sequence",newSeq);
//                    s.store();
//                }
            }
        }
        delegator.removeValue(navigation);
        Map<String,Object> result = ServiceUtil.returnSuccess();
        result.put("flag", flag);
        return result;
    }
    ...
    /**
     * add by dongxiao 2016.4.18 递归更新分类导航及下级的sequence
     * @param navigationId
     * @param delegator
     * @throws GenericEntityException
     */
    private static void updateNavigationAndChildrenSequence(String navigationId,String newSequence, Delegator delegator) throws GenericEntityException {
        GenericValue navigation = delegator.findByPrimaryKey("Navigation", UtilMisc.toMap("id", navigationId));
        String sequence = newSequence;
        navigation.set("sequence",sequence);
        delegator.store(navigation);
        List<GenericValue> navigationChildren = delegator.findByAnd("Navigation",UtilMisc.toMap("parentId",navigationId));
        if (UtilValidate.isNotEmpty(navigationChildren)){
            for (GenericValue nc : navigationChildren){
                String childrenSequence = nc.getString("sequence");
                String lastIndex = childrenSequence.substring(childrenSequence.lastIndexOf("."), childrenSequence.length());
                childrenSequence = sequence + lastIndex;
                updateNavigationAndChildrenSequence(nc.getString("id"),childrenSequence, delegator);
            }
        }else{
            return;
        }
