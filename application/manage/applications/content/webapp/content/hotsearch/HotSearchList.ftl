    ...
    <form id="QueryForm" class="form-inline clearfix">
      <div class="form-group">
        ...
        <div class="input-group m-b-10">
          ...
          <input type="text" id="hotSearchKeyName" class="form-control" value="">
        </div>
        <#--<div class="input-group m-b-10">
...
<script>
	...
	$(function(){
		data_tbl = $('#data_tbl').dataTable({
			ajaxUrl: ajaxUrl,
			columns:[
				{"title":"热门搜索ID","code":"hotSearchId","checked":true},
				{"title":"关键词","code":"hotSearchKeyName"},
					...
					{"title":"操作","code":"option",
					 "handle":function(td,record){
					 	var btns = "<div class='btn-group'>"+
	  							   "<button type='button' class='btn btn-danger btn-sm' onclick='javascript:editInit("+record.hotSearchId+");'>编辑</button>"+
							  	   "</div>";
						td.append(btns);
					 }
					}
...
//			var geoId = $('#QueryForm #geoId').val();
			var hotSearchKeyName = $('#QueryForm #hotSearchKeyName').val();
			ajaxUrl = changeURLArg(ajaxUrl,"webSiteName",webSiteName);
			ajaxUrl = changeURLArg(ajaxUrl,"hotSearchKeyName",hotSearchKeyName);
//			ajaxUrl = changeURLArg(ajaxUrl,"geoId",geoId);
	    ...
	    $('#btn_add').click(function(){
	    	window.location = '<@ofbizUrl>HotSearchAddPage</@ofbizUrl>';
	    });
    ...
    function editInit(id){
    	window.location = '<@ofbizUrl>HotSearchEditPage</@ofbizUrl>?hotSearchId='+id;
    }
