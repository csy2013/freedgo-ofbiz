        ...
        <div class="row m-b-10">
            ...
            <div class="col-sm-6">
                ...
                <#if security.hasEntityPermission("BANNERMGR_LIST", "_CREATE", session)>
                    ...
                    <!-- 是否有删除权限-->
                <#if security.hasEntityPermission("BANNERMGR_LIST", "_DELETE", session)>
...
<div id="modal_edit"  class="modal fade " tabindex="-1" role="dialog" aria-labelledby="modal_edit_title">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="modal_add_title">编辑热门品牌</h4>
            </div>
            <div class="modal-body">
...
<script>
    var data_tbl;
    var ajaxUrl = "brandRecommendList";
    var del_ids;
    var curProductIds="";
    ...
    $(function(){
                ...
                {"title":"操作","code":"option",
                    "handle":function(td,record){
                        var btns = "<div class='btn-group'>"+
                                <!-- 是否都有权限-->
                                <#if security.hasEntityPermission("BRANDRECOMEND_LIST", "_UPDATE", session)>
                                "<button type='button' class='btn btn-danger btn-sm'  onclick='javascript:editBrandRecommend("+record.recommendId+")'>编辑</button>"+
                                </#if>
                                ...
                                "<li class='edit_li'>" +
                                <#if security.hasEntityPermission("BRANDRECOMEND_LIST", "_DELETE", session)>
        ...
        $('#QueryForm #searchBtn').on('click',function(){
            var brandName = $('#QueryForm #brandName').val();
            ajaxUrl = changeURLArg(ajaxUrl,"brandName",brandName);
        ...
        $('#btn_add').click(function(){
            window.location = '<@ofbizUrl>brandRecommendAddPage</@ofbizUrl>';
        });
        ...
        $("#editForm").dpValidate({
            ...
            callback: function(){
                ...
                console.log(isAllWebSite);
                console.log(webSite);

                updateBrandRecommendInfo(recommendId,sequenceId,isAllWebSite,webSite);
            }
        });
    });
    ...
    function setDelete(id){
        ...
        $('#modal_confirm #modal_confirm_body').html("该热门品牌信息删除后将无法使用，是否继续？");
        $('#modal_confirm').modal('show');
    }

    // 编辑热门品牌
    function editBrandRecommend(id){
        //清空序号
        $('#modal_edit #sequenceId').val('');
        $('#modal_edit #recommendId').val('');
        $('#modal_edit #isAllWebSite').attr("checked",false);
        $.ajax({
            url: "brandRecommendEditInit",
            type: "POST",
            data : {recommendId:id
            },
            dataType : "json",
     ...
     * @param webSite
     */
    function updateBrandRecommendInfo(recommendId,sequenceId,isAllWebSite,webSite){
        //异步调用编辑序号方法
        $.ajax({
            url: "brandRecommendEdit",
            type: "POST",
