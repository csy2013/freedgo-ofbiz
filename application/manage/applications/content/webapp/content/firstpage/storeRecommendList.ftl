        ...
        <div class="row m-b-10">
            ...
            <div class="col-sm-6">
                <div class="dp-tables_btn">
                    <!-- 是否有新增权限-->
                <#if security.hasEntityPermission("STORERECOMEND_LIST", "_CREATE", session)>
                    <button id="btn_add" class="btn btn-primary">
                        <i class="fa fa-plus"></i>${uiLabelMap.BrandCreate}
                    </button>
                </#if>
                    <!-- 是否有删除权限-->
                <#if security.hasEntityPermission("STORERECOMEND_LIST", "_DELETE", session)>
...
<div id="modal_edit"  class="modal fade " tabindex="-1" role="dialog" aria-labelledby="modal_edit_title">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="modal_add_title">编辑热门店铺</h4>
            </div>
            <div class="modal-body">
...
<script>
    var data_tbl;
    var ajaxUrl = "storeRecommendList";
    var del_ids;
    var curProductIds="";
    ...
    $(function(){
        ...
        data_tbl = $('#data_tbl').dataTable({
            ...
            columns:[
                ...
                {"title":"商家编号","code":"partyId"},
                {"title":"店铺LOGO","code":"logoImg",
                    "handle":function(td,record){
                        if(record.logoImg){
                            td.html("<img  src='/content/control/getImage?contentId="+record.logoImg+"' style='width: 50px;height: 40px;'></img>");
                        }
                    }},
                ...
                {"title":"操作","code":"option",
                    "handle":function(td,record){
                        var btns = "<div class='btn-group'>"+
                                <!-- 是否都有权限-->
                                <#if security.hasEntityPermission("STORERECOMEND_LIST", "_UPDATE", session)>
                                "<button type='button' class='btn btn-danger btn-sm'  onclick='javascript:editStoreRecommend("+record.recommendId+")'>编辑</button>"+
                                </#if>
        ...
        $('#QueryForm #searchBtn').on('click',function(){
            var storeName = $('#QueryForm #storeName').val();
            ajaxUrl = changeURLArg(ajaxUrl,"storeName",storeName);
        ...
        $('#btn_add').click(function(){
            window.location = '<@ofbizUrl>storeRecommendAddPage</@ofbizUrl>';
        });
        ...
        $("#editForm").dpValidate({
            ...
            callback: function(){
                ...
                updateStoreRecommendInfo(recommendId,sequenceId,isAllWebSite,webSite);
            }
        });
    });
    ...
    function setDelete(id){
        ...
        $('#modal_confirm #modal_confirm_body').html("该热门店铺信息删除后将无法使用，是否继续？");
        $('#modal_confirm').modal('show');
    }

    // 编辑热门店铺
    function editStoreRecommend(id){
        //清空序号
        $('#modal_edit #sequenceId').val('');
        $('#modal_edit #recommendId').val('');
        $('#modal_edit #isAllWebSite').attr("checked",false);
        $.ajax({
            url: "storeRecommendEditInit",
            type: "POST",
            data : {recommendId:id
            },
            dataType : "json",
     ...
     * @param webSite
     */
    function updateStoreRecommendInfo(recommendId,sequenceId,isAllWebSite,webSite){
        //异步调用编辑序号方法
        $.ajax({
            url: "storeRecommendEdit",
            type: "POST",
