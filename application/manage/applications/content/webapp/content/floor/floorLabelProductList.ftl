<#assign commonUrl = "floorLabelProductManager?floorId="+parameters.floorId+"&floorLabelId="+parameters.floorLabelId+"&lookupFlag=Y&">
...
<div class="box box-info">
    <div class="box-body">
        ...
        <#if floorLabelProductList?has_content>
            <div class="col-sm-6">
    ...
    <#if floorLabelProductList?has_content>
        <div class="row">
            <div class="col-sm-12">
                <table class="table table-bordered table-hover js-checkparent">
                    ...
                    <tbody>
                        <#list floorLabelProductList as floorLabelProduct>
                        <tr>
                            <td><input value="${floorLabelProduct.floorLabelProductId?if_exists}" class="js-checkchild"
                                       type="checkbox"></td>
                            <td>
                                <span class="col-sm-6" style="display:inline-block;">
                                    <img height="100"
                                         src="${floorLabelProduct.imgUrl?if_exists}"
                                         class="cssImgSmall" alt=""/>
                                </span>
                                <span class="col-sm-6" style="display:inline-block;">
                                ${floorLabelProduct.productName?default("N/A")}
                                </span>
                            </td>
                            <td>${floorLabelProduct.productId?default("N/A")}</td>
                            <td>${floorLabelProduct.sequenceNum?default("N/A")}</td>
                            <td>
                                <div class="btn-group">
                                    <#if security.hasEntityPermission("FLOOR_LABEL_PRODUCT", "_UPDATE", session)>
                                        <button type="button" class="btn btn-danger btn-sm"
                                                onclick="editFloor('${floorLabelProduct.floorLabelProductId?default("N/A")}')">
                                            编辑
                                        </button>
                                    </#if>
                                    ...
                                    <ul class="dropdown-menu" role="menu">
                                        <#if security.hasEntityPermission("FLOOR_LABEL_PRODUCT", "_DELETE", session)>
                                            <li><a href="#"
                                                   onclick="delFloor('${floorLabelProduct.floorLabelProductId?default("N/A")}')">删除</a>
                                            </li>
                                        </#if>
                                    </ul>
                                </div>
        ...
        <#assign viewIndexLast = Static["org.ofbiz.base.util.UtilMisc"].getViewLastIndex(floorLabelProductListSize, viewSize) />

        <#assign messageMap = Static["org.ofbiz.base.util.UtilMisc"].toMap("lowCount", lowIndex, "highCount", highIndex, "total", floorLabelProductListSize)/>
        ...
        listSize=floorLabelProductListSize viewSize=viewSize ajaxFirstUrl="" firstUrl="" paginateFirstLabel="" paginatePreviousStyle="nav-previous" ajaxPreviousUrl="" previousUrl="" paginatePreviousLabel=""
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                        aria-hidden="true">&times;</span></button>
                <h4 id="header" class="modal-title">新增标签商品</h4>
            </div>
            <div class="modal-body">
                <form role="form" class="form-horizontal" id="addForm" action="">
                    <input id="floorLabelProductId" name="floorLabelProductId" type="hidden">
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('#btn_add').click(function () {
            $('#addForm').attr("action", "<@ofbizUrl>addFloorLabelProduct</@ofbizUrl>");
            $("#header").html('新增标签商品');
            $("#floorLabelProductId").val('');
            $("#selectId").val('');
            $("#productId").val('');
            $("#selectName").val('');
        ...
        $('#addForm').dpValidate({
            validate: true,
            callback: function () {
                var floorLabelProductId = $('#floorLabelProductId').val();
                ...
                $.ajax({
                    ...
                    type: "POST",
                    data: {
                        floorLabelProductId: floorLabelProductId,
                        floorId: floorId,
        ...
        $('#modal_confirm #ok').click(function (e) {
            //异步调用删除方法
            $.ajax({
                url: "<@ofbizUrl>removeFloorLabelProduct</@ofbizUrl>",
                type: "POST",
                data: {floorLabelProductIds: floorLabelProductIds},
                dataType: "json",
                success: function (data) {
                    delIds = "";
                    //设置提示弹出框内容
    ...
    function delFloor(floorLabelProductId) {
        floorLabelProductIds = floorLabelProductId;
        //设置删除弹出框内容
        $('#modal_confirm #modal_confirm_body').html("${uiLabelMap.IsDel}");
        $('#modal_confirm').modal('show');
    }
    function editFloor(floorLabelProductId) {
        $.ajax({
            url: "<@ofbizUrl>floorLabelProductDetail</@ofbizUrl>",
            type: "POST",
            data: {
                floorLabelProductId: floorLabelProductId
            },
            dataType: "json",
            success: function (data) {
                $('#addForm').attr("action", "<@ofbizUrl>editFloorLabelProduct</@ofbizUrl>");
                $("#header").html('编辑标签商品');
                $("#floorLabelProductId").val(data.floorLabelProductId);
