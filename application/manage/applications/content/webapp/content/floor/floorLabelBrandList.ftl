<#assign commonUrl = "floorLabelBrandManager?floorId="+parameters.floorId+"&floorLabelId="+parameters.floorLabelId+"&lookupFlag=Y&">
...
<div class="box box-info">
    <div class="box-body">
        ...
        <#if floorLabelBrandList?has_content>
            <div class="col-sm-6">
    ...
    <#if floorLabelBrandList?has_content>
        <div class="row">
            <div class="col-sm-12">
                <table class="table table-bordered table-hover js-checkparent">
                    ...
                    <tbody>
                        <#list floorLabelBrandList as floorLabelBrand>
                        <tr>
                            <td><input value="${floorLabelBrand.floorLabelBrandId?if_exists}" class="js-checkchild"
                                       type="checkbox"></td>
                            <td>${floorLabelBrand.brandName?default("N/A")}</td>
                            <td><img height="100" src="${floorLabelBrand.imgUrl?if_exists}" class="cssImgSmall" alt=""/>
                            </td>
                            <td>
                                <#if (floorLabelBrand.isEnabled)?default("N") == "N">
                                    <button type='button' class='btn btn-sm' value="N"
                                            id="${floorLabelBrand.floorLabelBrandId}" onclick="changeEnabled(this)">否
                                    </button>
                                <#else>
                                    <button type='button' class='btn btn-primary btn-sm' value="Y"
                                            id="${floorLabelBrand.floorLabelBrandId}" onclick="changeEnabled(this)">是
                                    </button>
                                </#if>
                            </td>
                            <td>${floorLabelBrand.sequenceNum?default("N/A")}</td>
                            <td>
                                <div class="btn-group">
                                    <#if security.hasEntityPermission("FLOOR_LABEL_BRAND", "_UPDATE", session)>
                                        <button type="button" class="btn btn-danger btn-sm"
                                                onclick="editFloor('${floorLabelBrand.floorLabelBrandId?default("N/A")}')">
                                            编辑
                                        </button>
                                    </#if>
                                    ...
                                    <ul class="dropdown-menu" role="menu">
                                        <#if security.hasEntityPermission("FLOOR_LABEL_BRAND", "_DELETE", session)>
                                            <li><a href="#"
                                                   onclick="delFloor('${floorLabelBrand.floorLabelBrandId?default("N/A")}')">删除</a>
                                            </li>
                                        </#if>
                                    </ul>
                                </div>
        ...
        <#assign viewIndexLast = Static["org.ofbiz.base.util.UtilMisc"].getViewLastIndex(floorLabelBrandListSize, viewSize) />

        <#assign messageMap = Static["org.ofbiz.base.util.UtilMisc"].toMap("lowCount", lowIndex, "highCount", highIndex, "total", floorLabelBrandListSize)/>
        ...
        listSize=floorLabelBrandListSize viewSize=viewSize ajaxFirstUrl="" firstUrl="" paginateFirstLabel="" paginatePreviousStyle="nav-previous" ajaxPreviousUrl="" previousUrl="" paginatePreviousLabel=""
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                        aria-hidden="true">&times;</span></button>
                <h4 id="header" class="modal-title">新增标签品牌</h4>
            </div>
            <div class="modal-body">
                <form role="form" class="form-horizontal" id="addForm" action="">
                    <input id="floorLabelBrandId" name="floorLabelBrandId" type="hidden">
...
<script type="text/javascript">
    var floorLabelBrandIds;
    $(function () {

        $('#btn_add').click(function () {
            $('#addForm').attr("action", "<@ofbizUrl>addFloorLabelBrand</@ofbizUrl>");
            $("#header").html('新增标签品牌');
            $("#floorLabelBrandId").val('');
            $("#productBrandId").val('');
            $("#linkUrl").val('');
            $('#enabled1').click();
            $('#sequenceNum').val('');
        ...
        $('#addForm').dpValidate({
            validate: true,
            callback: function () {
                var floorLabelBrandId = $('#floorLabelBrandId').val();
                ...
                $.ajax({
                    ...
                    type: "POST",
                    data: {
                        floorLabelBrandId: floorLabelBrandId,
                        floorId: floorId,
        ...
        $('#modal_confirm #ok').click(function (e) {
            //异步调用删除方法
            $.ajax({
                url: "<@ofbizUrl>removeFloorLabelBrand</@ofbizUrl>",
                type: "POST",
                data: {floorLabelBrandIds: floorLabelBrandIds},
                dataType: "json",
                success: function (data) {
                    delIds = "";
                    //设置提示弹出框内容
    ...
    function delFloor(floorLabelBrandId) {
        floorLabelBrandIds = floorLabelBrandId;
        //设置删除弹出框内容
        $('#modal_confirm #modal_confirm_body').html("${uiLabelMap.IsDel}");
        $('#modal_confirm').modal('show');
    }
    function editFloor(floorLabelBrandId) {
        $.ajax({
            url: "<@ofbizUrl>floorLabelBrandDetail</@ofbizUrl>",
            type: "POST",
            data: {
                floorLabelBrandId: floorLabelBrandId
            },
            dataType: "json",
            success: function (data) {
                $('#addForm').attr("action", "<@ofbizUrl>editFloorLabelBrand</@ofbizUrl>");
                $("#header").html('编辑标签品牌');
                $("#floorLabelBrandId").val(data.floorLabelBrandId);
    ...
    function changeEnabled(obj) {
        var obj = $(obj);
        var isEnabled;
        var floorLabelBrandId = obj.attr("id");
        if (obj.attr("value") == "N") {
            obj.html("是");
            isEnabled = "Y";
            obj.attr("value", "Y");
        ...
        $.ajax({
            url: "<@ofbizUrl>editFloorLabelBrand</@ofbizUrl>",
            type: "POST",
            data: {
                floorLabelBrandId: floorLabelBrandId,
                isEnabled: isEnabled
