<#assign commonUrl = "floorBannerManager?floorId="+parameters.floorId+"&lookupFlag=Y&">
...
<div class="box box-info">
    <div class="box-body">
        ...
        <#if floorBannerList?has_content>
            <div class="col-sm-6">
    ...
    <#if floorBannerList?has_content>
        <div class="row">
            <div class="col-sm-12">
                <table class="table table-bordered table-hover js-checkparent">
                    ...
                    <tbody>
                        <#list floorBannerList as floorBanner>
                        <tr>
                            <td><input value="${floorBanner.floorBannerId?if_exists}" class="js-checkchild"
                                       type="checkbox"></td>
                            <td><img height="100" src="${floorBanner.imgUrl?if_exists}" class="cssImgSmall" alt=""/>
                            </td>
                            <td>${floorBanner.bannerName?default("N/A")}</td>
                            <td>${floorBanner.linkUrl?default("N/A")}</td>
                            <td>
                                <#if (floorBanner.isEnabled)?default("N") == "N">
                                    <button type='button' class='btn btn-sm' value="N"
                                            id="${floorBanner.floorBannerId}" onclick="changeEnabled(this)">否
                                    </button>
                                <#else>
                                    <button type='button' class='btn btn-primary btn-sm' value="Y"
                                            id="${floorBanner.floorBannerId}" onclick="changeEnabled(this)">是
                                    </button>
                                </#if>
                            </td>
                            <td>${floorBanner.sequenceNum?default("N/A")}</td>
                            <td>
                                <div class="btn-group">
                                    <#if security.hasEntityPermission("FLOOR_LABEL_BANNER", "_UPDATE", session)>
                                        <button type="button" class="btn btn-danger btn-sm"
                                                onclick="editFloor('${floorBanner.floorBannerId?default("N/A")}')">
                                            编辑
                                        </button>
                                    </#if>
                                    ...
                                    <ul class="dropdown-menu" role="menu">
                                        <#if security.hasEntityPermission("FLOOR_LABEL_BANNER", "_DELETE", session)>
                                            <li><a href="#"
                                                   onclick="delFloor('${floorBanner.floorBannerId?default("N/A")}')">删除</a>
                                            </li>
                                        </#if>
                                    </ul>
                                </div>
        ...
        <#assign viewIndexLast = Static["org.ofbiz.base.util.UtilMisc"].getViewLastIndex(floorBannerListSize, viewSize) />

        <#assign messageMap = Static["org.ofbiz.base.util.UtilMisc"].toMap("lowCount", lowIndex, "highCount", highIndex, "total", floorBannerListSize)/>
        ...
        listSize=floorBannerListSize viewSize=viewSize ajaxFirstUrl="" firstUrl="" paginateFirstLabel="" paginatePreviousStyle="nav-previous" ajaxPreviousUrl="" previousUrl="" paginatePreviousLabel=""
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                        aria-hidden="true">&times;</span></button>
                <h4 id="header" class="modal-title">新增楼层广告</h4>
            </div>
            <div class="modal-body">
                <form role="form" class="form-horizontal" id="addForm" action="">
                    <input id="floorBannerId" name="floorBannerId" type="hidden">
...
// 初始化图片选择
        ...
        $('#btn_add').click(function () {
            $('#addForm').attr("action", "<@ofbizUrl>addFloorBanner</@ofbizUrl>");
            $("#header").html('新增楼层广告');
            $("#floorBannerId").val('');
            $("#bannerName").val('');
            $("#imgUrl").val('');
            ...
            // 清除已经选择的链接类型
            $('#addForm #firstLinkType').val('');
            $('#addForm #linkId').val('');
            $('#addForm #selectName').html('');
        });
        $('#save').click(function () {
            $("#addForm").dpValidate({
                clear: true
        ...
        $('#addForm').dpValidate({
            validate: true,
            callback: function () {
                var floorBannerId = $('#floorBannerId').val();
                var floorId = $('#floorId').val();
                var bannerName = $('#bannerName').val();
                var imgUrl = $('#imgUrl').val();

                var firstLinkType = $('#addForm #firstLinkType').val();
                var linkUrl = $('#addForm #linkUrl').val();
                var linkId = $('#addForm #linkId').val();
                var linkName = $('#addForm #selectName').html();
        ...
        $('#modal_confirm #ok').click(function (e) {
            //异步调用删除方法
            $.ajax({
                url: "<@ofbizUrl>removeFloorBanner</@ofbizUrl>",
                type: "POST",
                data: {floorBannerIds: floorBannerIds},
                dataType: "json",
                success: function (data) {
                    delIds = "";
                    //设置提示弹出框内容
    ...
    function delFloor(floorBannerId) {
        floorBannerIds = floorBannerId;
        //设置删除弹出框内容
        $('#modal_confirm #modal_confirm_body').html("${uiLabelMap.IsDel}");
        $('#modal_confirm').modal('show');
    }
    function editFloor(floorBannerId) {
        $.ajax({
            url: "<@ofbizUrl>floorBannerDetail</@ofbizUrl>",
            type: "POST",
            data: {
                floorBannerId: floorBannerId
            },
            ...
            success: function (data) {
                ...
                $('#addForm').attr("action", "<@ofbizUrl>editFloorBanner</@ofbizUrl>");
                $("#header").html('编辑楼层广告');
                $("#floorBannerId").val(data.floorBannerId);
    ...
    function changeEnabled(obj) {
        ...
        $.ajax({
            url: "<@ofbizUrl>editFloorBanner</@ofbizUrl>",
            type: "POST",
            data: {
                floorBannerId: floorBannerId,
                isEnabled: isEnabled
