...
//Get Promo Text Data  Add By Alex Yao
            ...
            if (featureSet) {
                ...
                if (variantTree && imageMap) {
                    ...
                    if (variants) {
                        ...
                        // Create the javascript to return the listPrice for each variant  Add By AlexYao
                        listPriceJS = new StringBuffer();
                        listPriceJS.append("function getListPrice(sku) { ");
                        // Create the javascript to return the promotion for each variant  Add By AlexYao
                        listPromoJS = new StringBuffer();
                        listPromoJS.append("function getListPromo(sku) { ");
                        ...
                        variants.each { variantAssoc ->
                            ...
                            if (variantPriceMap && variantPriceMap.basePrice) {
                                ...
                                if(variantPriceMap.listPrice && variantPriceMap.basePrice < variantPriceMap.listPrice){
                                    listPriceJS.append("  if (sku == \"" + variant.productId + "\") return \"" + numberFormat.format(variantPriceMap.listPrice) + "\"; ");
                                }else{
                                    listPriceJS.append("  if (sku == \"" + variant.productId + "\") return \"" + "\"; ");
                                }
                            }
                            //promotion  Add By AlexYao
                            html="";
                            temp=0;
                            productPromosAll.each { productPromo ->
                                promo = delegator.findByAnd("ProductPromoProduct", [productPromoId : productPromo.productPromoId, productId : variant.productId]);
                                if(promo.size()>0){
                                    if(temp==0){
                                        html+="<span class='attr'>促销信息&nbsp;</span><span class=\'promo\'><label class=\'low\'>"+productPromo.promoName+"</label>&nbsp;"+productPromo.promoText+"</span>";temp=1;
                                    }else{
                                        html+="<br><span class='attr'>&nbsp;</span><span class=\'promo\'><label class=\'low\'>"+productPromo.promoName+"</label>&nbsp;"+productPromo.promoText+"</span>";
                                    }
                                }
                            }
                            listPromoJS.append("  if (sku == \"" + variant.productId + "\") return \"" + html + "\"; ");
                    ...
                    jsBuf.append(listPriceJS.toString());
                    jsBuf.append(listPromoJS.toString());
                    jsBuf.append("</script>");
    ...
    alsoBoughtProducts = dispatcher.runSync("getAssociatedProducts", [productIdTo : productId, type : "ALSO_BOUGHT", checkViewAllow : true, prodCatalogId : currentCatalogId, sortDescending : true]);
    ...
    crossSellProducts = dispatcher.runSync("getAssociatedProducts", [productIdTo : productId, type : "PRODUCT_COMPLEMENT", checkViewAllow : true, prodCatalogId : currentCatalogId]);
...
wechatId = session.getAttribute("wechatId");
