...
if (product) {
    ...
    // get the product distinguishing features 获取产品的使用的特性
    ...
    // an example of getting features of a certain type to show 获取产品的特性 应用值
    ...
    // get product variant for Box/Case/Each 在产品关联中 使用了Alternative_Packing (如果该产品是某个产品的package，找到对应的主产品)
    productVariants = [];
    ...
    // 如果是虚拟产品， Special Variant Code
    if ("Y".equals(product.isVirtual)) {
//        虚拟的多样化方法 是 VV_FEATURETREE
        ...
        if(isAlternativePacking){
            ...
            // make a list of variant sku with requireAmount
//            WG-9943,有一种产品是既是虚拟又是变形产品，通过在关联关系中加入ALTERNATIVE_PACKAGE
