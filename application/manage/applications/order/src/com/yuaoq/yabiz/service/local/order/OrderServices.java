        ...
        if (orderHeaderList != null) {
            for (int i = 0; i < orderHeaderList.size(); i++) {
                ...
                if(paymentStatusId!=null) {
                    //未付费并且订单状态不为取消状态
                    if ("PAYMENT_NOT_RECEIVED".equals(paymentStatusId) && (!"ORDER_CANCELLED".equals(statusId))) {
                        payOrders.add(orderMap);
                    } else if ("ORDER_CANCELLED".equals(statusId)) {
                        cancelOrders.add(orderMap);
                    } else if ("ORDER_CREATED".equals(statusId) || "ORDER_APPROVED".equals(statusId) || "ORDER_COMPLETED".equals(statusId)) {
                        ...
                        try {
                            ...
                            if (UtilValidate.isNotEmpty(sorderHeaderList)) {
                                ...
                                for (int k = 0; k < sorderHeaderList.size(); k++) {
                                    ...
                                    if ("SHIPMENT_DELIVERED".equals(shipment.get("statusId"))) {
                                    ...
                                    }else if(!"SHIPMENT_CANCELLED".equals(shipment.get("statusId"))){
                                        //正在shipping
