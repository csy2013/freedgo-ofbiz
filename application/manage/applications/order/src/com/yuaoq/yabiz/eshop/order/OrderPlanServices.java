...
//        List<String> fieldsToSelect = FastList.newInstance();
        ...
        for (int i = 0; i < reservationList.size(); i++) {
            ...
            try {
                ...
                eli = delegator.findListIteratorByCondition(dveTemp, makeExpr("id", idtemp), null, null, null, findOpts);
        ...
        if (UtilValidate.isNotEmpty(status) && reservationList2 != null && reservationList2.size() > 0) {
            ...
            if ("0".equals(status) || "2".equals(status)) {     //已取消或已完成
                ...
                for (GenericValue g : reservationList2) {
                    ...
                    if ("1".equals(g.get("status")) && (dd.before(new java.sql.Timestamp(System.currentTimeMillis()))) || dd.equals(new java.sql.Timestamp(System.currentTimeMillis()))) {
                        reservationList3.add(g);
                    }
                }
            } else if ("1".equals(status)) {                   //已预约
                DateFormat sdf = new SimpleDateFormat("yyyyMMdd");
                DateFormat sdf2 = new SimpleDateFormat("yyyyMMddHHmmss");
                for (GenericValue g : reservationList2) {
                    Date dateTemp = sdf.parse(g.getTimestamp("planDate").toString());
                    ...
                    Date d = sdf2.parse(dateStr);
                    java.sql.Timestamp dd = new java.sql.Timestamp(d.getTime());
                    if (g.get("status").equals(status) && dd.after(new java.sql.Timestamp(System.currentTimeMillis()))) {
                        reservationList3.add(g);
                    }
                }
            }
