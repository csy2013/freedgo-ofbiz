...
public class ShoppingCartItem implements java.io.Serializable {
    ...
    private String activityId = null;//活动Id  Add By AlexYao 2016/01/29
    ...
    /* begin 活动id Add By ALexYao 2016/01/29 */
    ...
    /* end 活动id Add By AlexYao 2016/01/29 */
    ...
    public static ShoppingCartItem makeItem(Integer cartLocation, String productId, BigDecimal selectedAmount, BigDecimal quantity, BigDecimal unitPrice,
                                            ...
                                            String itemType, ShoppingCart.ShoppingCartItemGroup itemGroup, LocalDispatcher dispatcher, ShoppingCart cart, Boolean triggerExternalOpsBool, Boolean triggerPriceRulesBool, String parentProductId, Boolean skipInventoryChecks, Boolean skipProductChecks, String isChoose)
            throws CartItemModifyException, ItemNotFoundException {
        ...
        return makeItem(cartLocation, productId, selectedAmount, quantity, unitPrice,
                ...
                parentProductId, skipInventoryChecks, skipProductChecks, isChoose);
    ...
    public static ShoppingCartItem makeItem(Integer cartLocation, String productId, BigDecimal selectedAmount, BigDecimal quantity, BigDecimal unitPrice,
        ...
        return makeItem(cartLocation, product, selectedAmount, quantity, unitPrice,
                ...
                additionalProductFeatureAndAppls, attributes, prodCatalogId, configWrapper,
                itemType, itemGroup, dispatcher, cart, triggerExternalOpsBool, triggerPriceRulesBool, parentProduct, skipInventoryChecks, skipProductChecks, isChoose);
    }
    ...
    public static ShoppingCartItem makeItem(Integer cartLocation, GenericValue product, BigDecimal selectedAmount,
                                            ...
                                            ShoppingCart cart, Boolean triggerExternalOpsBool, Boolean triggerPriceRulesBool, GenericValue parentProduct, Boolean skipInventoryChecks, Boolean skipProductChecks, String isChoose) throws CartItemModifyException {
        ...
        return makeItem(cartLocation, product, selectedAmount,
                ...
                triggerExternalOpsBool, triggerPriceRulesBool, parentProduct, skipInventoryChecks, skipProductChecks, isChoose);
    }
    ...
    public static ShoppingCartItem makeItem(Integer cartLocation, GenericValue product, BigDecimal selectedAmount,
        ...
        //add by changsy 20180增加产品店铺到行项目
        Delegator delegator = cart.getDelegator();
        String productStoreId = "";
        try {
            List<GenericValue> productStores = delegator.findByAndCache("ProductStoreProductView", UtilMisc.toMap("productId", product.get("productId")));
            if (UtilValidate.isNotEmpty(productStores)) {
                GenericValue productStore = productStores.get(0);
                productStoreId = (String) productStore.get("productStoreId");
                newItem.setProductStoreId(productStoreId);
                newItem.setProductStoreName((String) productStore.get("storeName"));
            }
        } catch (GenericEntityException e) {
            throw new CartItemModifyException(e);
        }
        newItem.setIsChoose(isChoose);
...
//        String productStoreId = cart.getProductStoreId();
      ...
      /*  if (!skipInventoryChecks && !"PURCHASE_ORDER".equals(cart.getOrderType())) {
        ...
        if(!"buyCart".equals(cart.getCartType())) {
...
//            ProductPromoWorker.doPromotions(cart, dispatcher);  //去掉促销，项目  Edit By AlexYao  2016-3-12 19:27:41
        }
        ...
        if (!"PURCHASE_ORDER".equals(cart.getOrderType())) {
            ...
            if (triggerExternalOps && ProductStoreWorker.autoSaveCart(delegator, productStoreId)) {
                try {
                    if ("shoppingCart".equals(cart.getCartType())) {
                        ShoppingListEvents.fillAutoSaveList(cart, dispatcher);
                    } else if ("wishCart".equals(cart.getCartType())) {
                        ShoppingListEvents.fillWishList(cart, dispatcher);
                    } else if ("buyCart".equals(cart.getCartType())) {
                        ShoppingListEvents.fillBuyList(cart, dispatcher);
                    }
    ...
    public void updatePrice(LocalDispatcher dispatcher, ShoppingCart cart) throws CartItemModifyException {
        ...
        String isGift = this.getAttribute("isGift") == null ? "N":(String)this.getAttribute("isGift");
        if (_product != null && isModifiedPrice == false && "N".equals(isGift)) {
            try {
                ...
                priceContext.put("productStoreId", this.getProductStoreId());
                ...
                if (configWrapper != null) {
                    ...
                    if (UtilValidate.isNotEmpty(productStore) && "Y".equals(productStore.get("showPricesWithVatTax"))) {
                        ...
                        // Get Taxes
                        Map<String, Object> totalPriceWithTaxMap = dispatcher.runSync("calcTaxForDisplay", UtilMisc.toMap("basePrice", totalPrice, "productId", this.productId, "productStoreId", null));
...
//        return BigDecimal.ONE;
     ...
     */
    public Set<Map> getFeatureSet() {
        Set<Map> featureSet = new LinkedHashSet<Map>();
        GenericValue product = this.getProduct();
        if (product != null) {
            List<GenericValue> featureAppls = null;
            try {
                featureAppls = product.getRelated("ProductFeatureAssoc");
               /* List<EntityExpr> filterExprs = UtilMisc.toList(EntityCondition.makeCondition("productFeatureApplTypeId", EntityOperator.EQUALS, "STANDARD_FEATURE"));
                ...
                if (featureAppls != null) {
                    for (GenericValue appl : featureAppls) {
                        ...
                        featureMap.put("featureId", appl.getString("productFeatureId"));
                        GenericValue feature = appl.getRelatedOne("ProductFeature");
                        featureMap.put("productFeatureName", feature.getString("productFeatureName"));
                        featureMap.put("productFeatureType", feature.getRelatedOne("ProductFeatureType").getString("productFeatureTypeName"));
        ...
        if (this.additionalProductFeatureAndAppls != null) {
            for (GenericValue appl : this.additionalProductFeatureAndAppls.values()) {
                ...
                try {
                    feature = appl.getRelatedOne("ProductFeature");
                    featureMap.put("productFeatureName", feature.getString("productFeatureName"));
                    featureMap.put("productFeatureType", feature.getRelatedOne("ProductFeatureType").getString("productFeatureTypeName"));
                    featureSet.add(featureMap);
                } catch (GenericEntityException e) {
    ...
    public List<GenericValue> getStandardFeatureList() {
        ...
        if (product != null) {
            try {
                features = product.getRelated("ProductFeatureAndAssoc");
            } catch (GenericEntityException e) {
                Debug.logError(e, "Unable to get features from product : " + product.get("productId"), module);
            }
        }
...
//        Debug.logInfo("Price" + getBasePrice() + " quantity" +  quantity + " Rental adj:" + getRentalAdjustment() + " other adj:" + getOtherAdjustments(), module);
    ...
    public BigDecimal getItemSubTotalForPromo(BigDecimal quantity) {
    ...
    public BigDecimal getItemSubTotalForPromo() {
        return this.getItemSubTotalForPromo(this.getQuantity());
    }
    
    
    public BigDecimal getDisplayItemSubTotal() {
        return this.getDisplayPrice().multiply(this.getQuantity()).add(this.getOtherAdjustments());
    }
    ...
    public void setOrderItemAttribute(String name, String value) {
        if (orderItemAttributes == null) {
            orderItemAttributes = FastMap.newInstance();
        }
    ...
    /* add by dongxiao 2016.1.11 判断购物车行项目是否包含该商品id */
    public boolean equals(String productId) {
        if (this.productId == null || productId == null) {
            // all non-product items are unique
            return false;
    ...
    public GenericValue getProduct() {
        ...
        if (this.productId != null) {
            try {
                this._product = this.getDelegator().findByPrimaryKey("Product", UtilMisc.toMap("productId", productId));
            } catch (GenericEntityException e) {
