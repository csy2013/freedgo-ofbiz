...
public class ShoppingCartEvents {
    ...
    /** Event to add an item to the shopping cart. 注释掉by dong xiao 2016.1.04*/
    ...
    /**
     * Event to add an item to the shopping cart. add by dong xiao 2016.1.4
     */
    public static String addToCart(HttpServletRequest request, HttpServletResponse response) {
//        SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
//        System.out.println("+++++++++++++++++++++++++ CREATE ORDER - appAddToCart, START TIME :" + sdf.format(new java.util.Date()));
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        ShoppingCart cart = getCartObject(request);
        ShoppingCartHelper cartHelper = new ShoppingCartHelper(delegator, dispatcher, cart);
        //给购物车设置店铺 Add By AlexYao
        ...
        String itemGroupNumber = (String) paramMap.get("itemGroupNumber");
        // Add by zhajh at 2014/12/30 Begin
        HttpSession session = request.getSession();
        String itemDesiredDeliveryDate = (String) paramMap.get("itemDesiredDeliveryDate");
        // 缺省期望日期的檢查
        if (UtilValidate.isNotEmpty(itemDesiredDeliveryDate)) {
            itemDesiredDeliveryDate = itemDesiredDeliveryDate.trim();

            String nowDate = getCurrentTime();
            int checkFlg = compare_date(itemDesiredDeliveryDate, nowDate);
            if (checkFlg < 0) {
                request.setAttribute("_ERROR_MESSAGE_", " 【期望递送日期】 在 【当前日期】 之后");
                return "error";
            } else {
                // 保存期望日期
                //request.setAttribute("currentItemDesiredDeliveryDate", itemDesiredDeliveryDate);
                session.setAttribute("currentItemDesiredDeliveryDate", itemDesiredDeliveryDate);
            }
        }
        // Add by zhajh at 2014/12/30 End
...
//            if (!ProductWorker.isDecimalQuantityOrderAllowed(delegator, productId, cart.getProductStoreId())) {
        ...
        // Add by zhajh at 2015/1/1 对于配置产品的预期到达时间 Begin
        //String itemDesiredDeliveryDateCheck =(String)request.getAttribute("itemDesiredDeliveryDate");
        String itemDesiredDeliveryDateCheck = (String) session.getAttribute("currentItemDesiredDeliveryDate");
        if (!UtilValidate.isEmpty(itemDesiredDeliveryDateCheck)) {
            if (UtilValidate.isEmpty((String) paramMap.get("itemDesiredDeliveryDate"))) {
                paramMap.put("itemDesiredDeliveryDate", itemDesiredDeliveryDateCheck);
                session.removeAttribute("currentItemDesiredDeliveryDate");
            }
        }
        // Add by zhajh at 2015/1/1 对于配置产品的预期到达时间 End

//        System.out.println("+++++++++++++++++++++++++ CREATE ORDER - appAddToCart11111111, START TIME :" + sdf.format(new java.util.Date()));
        // Translate the parameters and add to the cart
        result = cartHelper.addToCart(catalogId, shoppingListId, shoppingListItemSeqId, productId, productCategoryId,
                itemType, itemDescription, price, amount, quantity, reservStart, reservLength, reservPersons,
                accommodationMapId, accommodationSpotId,
                shipBeforeDate, shipAfterDate, configWrapper, itemGroupNumber, paramMap, parentProductId);
//        System.out.println("+++++++++++++++++++++++++ CREATE ORDER - appAddToCart222222222, START TIME :" + sdf.format(new java.util.Date()));
        controlDirective = processResult(result, request);
...
//        System.out.println("+++++++++++++++++++++++++ CREATE ORDER - appAddToCart11111111111111111111111111, START TIME :" + sdf.format(new java.util.Date()));
        // Determine where to send the browser
        return "success";
    }
...
//                System.out.println("dt1在dt2后");
    ...
    public static String addSomeOrderProducts(HttpServletRequest request, HttpServletResponse response) {
        HttpSession session = request.getSession();
        String currenPartyId = request.getParameter("currenPartyId");
        if (UtilValidate.isNotEmpty(currenPartyId)){
            session.setAttribute("currenPartyId",currenPartyId);
            request.setAttribute("currenPartyId",currenPartyId);
        }
        String productIds = request.getParameter("productIds");
        String[] productIdArray = productIds.split(",");
        for (String pId : productIdArray){
            request.setAttribute("add_product_id",pId);
            addToCart(request,response);
        }
        String activityIds = request.getParameter("activityIds");
        String productPrices = request.getParameter("productPrices");
        if (UtilValidate.isNotEmpty(activityIds) && UtilValidate.isNotEmpty(productPrices)){            //说明商品参加了活动
            String[] activityIdArray = activityIds.split(",");
            String[] productPriceArray = productPrices.split(",");
            ShoppingCart cart = getCartObject(request);
            for (int i = 0; i < cart.items().size(); i ++){
                cart.items().get(i).setActivityId(activityIdArray[i]);
                cart.items().get(i).setBasePrice(new BigDecimal(productPriceArray[i]));
            }
        }

        request.setAttribute("clear", "N");
        return "success";
    }
...
//        SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
//        System.out.println("+++++++++++++++++++++++++ CREATE ORDER - appClearCart, START TIME :" + sdf.format(new java.util.Date()));
        ShoppingCart cart = getCartObject(request);
        cart.clear();
...
//        SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
//        System.out.println("+++++++++++++++++++++++++ CREATE ORDER - clearcartfororder1, START TIME :" + sdf.format(new java.util.Date()));
        HttpSession session = request.getSession();
...
//        System.out.println("+++++++++++++++++++++++++ CREATE ORDER , END TIME :" + sdf.format(new java.util.Date()));
        return "success";
    }

    /** Gets or creates the shopping cart object 原有注释掉 by dongxiao at 2016.1.4 */
...
//            shoppingcart 根据应用的不同需要加载不同的webShoppingCart
            ...
            String classstr = UtilProperties.getPropertyValue("order.properties",context.substring(1)+".shopping.cart.class");
            if(classstr==null || classstr.equals("")){
                context = "default";
                classstr = UtilProperties.getPropertyValue("order.properties",context+".shopping.cart.class");
            }
            try {
    ...
    /** Gets or creates the shopping cart object 新增getCartObject by dongxiao at 2016.1.4  */
    public static ShoppingCart getCartObject(HttpServletRequest request, Locale locale, String currencyUom) {
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        ShoppingCart cart = (ShoppingCart) request.getAttribute("shoppingCart");
        HttpSession session = request.getSession(true);
    ...
    public static String loadCartFromOrder(HttpServletRequest request, HttpServletResponse response) {
        ...
        session.setAttribute("shoppingCart", cart);
        session.setAttribute("productStoreId", null);
    ...
    public static String initSaleOrderEntry(HttpServletRequest request, HttpServletResponse response){
    ...
    public static String dealSaleOrderEntry(HttpServletRequest request, HttpServletResponse response){
        initializeOrderEntry(request, response);
        setOrderCurrencyAgreementShipDates(request,response);
        String viewname = addToCart(request,response);
        return "cart";
    }
     ...
     * @return
     */
    public static String salesRouteOrderEntryInit(HttpServletRequest request, HttpServletResponse response) {
        HttpSession session = request.getSession();
        String clear = request.getParameter("clear");
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        if (UtilValidate.isEmpty(clear)){
            salesRemoveAttribute(request, response);
            ShoppingCart currentCart = (ShoppingCart) session.getAttribute("shoppingCart");
            if (currentCart != null) {
                session.removeAttribute("shoppingCart");
                destroyCart(request, response);
            ...
            try {
                productStores = delegator.findByAnd("ProductStore", new HashMap<String, Object>());
            } catch (GenericEntityException e) {
                e.printStackTrace();
            }
...
//            cart.setProductStoreId(productStoreId);
    ...
    /**
     * 订单下一步参数传递 add by dongxiao 2016.1.7
     * @param request
     * @param response
     * @return
     * @throws GenericEntityException
     */
    public static String orderNext(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException, ItemNotFoundException, CartItemModifyException {
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        HttpSession session = request.getSession();
        String currenPartyId = request.getParameter("currenPartyId");
        session.setAttribute("currenPartyId",currenPartyId);
        ShoppingCart cart = getCartObject(request);
        for (ShoppingCartItem sci : cart.items()){
            String productId = sci.getProductId();
            BigDecimal quantity = new BigDecimal(request.getParameter(productId)) ;
            LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
            cart.updateProductItemPrice(productId, quantity, dispatcher);
        }
//        Long integralPerMoney = delegator.findByPrimaryKey("PartyIntegralSet",UtilMisc.toMap("partyIntegralSetId","PARTY_INTEGRAL_SET")).getLong("integralValue");
        cart.setOrderPartyId(currenPartyId);
        cart.setShipToCustomerPartyId(currenPartyId);
        session.setAttribute("shoppingCart", cart);
        request.setAttribute("success",0);
//        request.setAttribute("integralPerMoney",integralPerMoney);
        return "success";
    }
    ...
    /**
     * add by dongxiao 2016.1.4
     * 销售订单清除Session
     * @param request
     * @param response
     * @return
     */
    public static void salesRemoveAttribute(HttpServletRequest request, HttpServletResponse response) {
        HttpSession session = request.getSession();

        // ////////////////////////////////////////////
        // 新建客户地址的客户id
        String addressPartyId = (String) session.getAttribute("addressPartyId");
        if (addressPartyId != null) {
            session.removeAttribute("addressPartyId");
        }
        // 产品店铺
        String currenProductStoreId = (String) session.getAttribute("currenProductStoreId");
        if (currenProductStoreId != null) {
            session.removeAttribute("currenProductStoreId");
        }
        // 销售渠道
        String currenSalesChannelEnumId = (String) session.getAttribute("currenSalesChannelEnumId");
        if (currenSalesChannelEnumId != null) {
            session.removeAttribute("currenSalesChannelEnumId");
        }
        // 支付方式
        String currenPaymentMethodTypeAndId = (String) session.getAttribute("currenPaymentMethodTypeAndId");
        if (currenPaymentMethodTypeAndId != null) {
            session.removeAttribute("currenPaymentMethodTypeAndId");
        }
        // 送货方式
        String currenSalesShippingMethod = (String) session.getAttribute("currenSalesShippingMethod");
        if (currenSalesShippingMethod != null) {
            session.removeAttribute("currenSalesShippingMethod");
        }
        // 送货地址
        String currenShippingContactMechId = (String) session.getAttribute("currenShippingContactMechId");
        if (currenShippingContactMechId != null) {
            session.removeAttribute("currenShippingContactMechId");
        }
        // 选择目录

        String currenCatalogId = (String) session.getAttribute("currenCatalogId");
        if (currenCatalogId != null) {
            session.removeAttribute("currenCatalogId");
        }
        // 缺省最早送货日期

        String currenShipAfterDate = (String) session.getAttribute("currenShipAfterDate");
        if (currenShipAfterDate != null) {
            session.removeAttribute("currenShipAfterDate");
        }
        // 缺省最迟送货日期
        String currenShipBeforeDate = (String) session.getAttribute("currenShipBeforeDate");
        if (currenShipBeforeDate != null) {
            session.removeAttribute("currenShipBeforeDate");
        }
        // 经销商标识
        String currenPartyIdDistribute = (String) session.getAttribute("currenPartyIdDistribute");
        if (currenPartyIdDistribute != null) {
            session.removeAttribute("currenPartyIdDistribute");
        }
        // 仓库
        String currenShipGroupFacilityId = (String) session.getAttribute("currenShipGroupFacilityId");
        if (currenShipGroupFacilityId != null) {
            session.removeAttribute("currenShipGroupFacilityId");
        }
        // 订单名称
        String currenSalesOrderName = (String) session.getAttribute("currenSalesOrderName");
        if (currenSalesOrderName != null) {
            session.removeAttribute("currenSalesOrderName");
        }
        // 客户名称
        String currenPartyId = (String) session.getAttribute("currenPartyId");
        if (currenPartyId != null) {
            session.removeAttribute("currenPartyId");
        }
        // 预约单号
        String currenReserveOrderId = (String) session.getAttribute("currenReserveOrderId");
        if (currenReserveOrderId != null) {
            session.removeAttribute("currenReserveOrderId");
        }
        // 拆分偏好
        String currenMaySplit = (String) session.getAttribute("currenMaySplit");
        if (currenMaySplit != null) {
            session.removeAttribute("currenMaySplit");
        }
        // 货币
        String currencyUomId = (String) session.getAttribute("currencyUomId");
        if (currencyUomId != null) {
            session.removeAttribute("currencyUomId");
        }
        // 是否开运输发票
        String currenInvoicePerShipment = (String) session.getAttribute("currenInvoicePerShipment");
        if (currenInvoicePerShipment != null) {
            session.removeAttribute("currenInvoicePerShipment");
        }
        // 发票标题
        String currenInvoiceTitle = (String) session.getAttribute("currenInvoiceTitle");
        if (currenInvoiceTitle != null) {
            session.removeAttribute("currenInvoiceTitle");
        }

        // 销售渠道(更新)
        String currenSalesChannelId = (String) session.getAttribute("currenSalesChannelId");
        if (currenSalesChannelId != null) {
            session.removeAttribute("currenSalesChannelId");
        }

        // 预约人
        String currenAppointPerson = (String) session.getAttribute("currenAppointPerson");
        if (currenAppointPerson != null) {
            session.removeAttribute("currenAppointPerson");
        }
        // 预约人性别
        String currencyAppointGender = (String) session.getAttribute("currencyAppointGender");
        if (currencyAppointGender != null) {
            session.removeAttribute("currencyAppointGender");
        }
        // 预留电话
        String currenAppointTel = (String) session.getAttribute("currenAppointTel");
        if (currenAppointTel != null) {
            session.removeAttribute("currenAppointTel");
        }
        // 备注
        String currenAppointRemarks = (String) session.getAttribute("currenAppointRemarks");
        if (currenAppointRemarks != null) {
            session.removeAttribute("currenAppointRemarks");
        }
        // 预约日期
        String currenAppointDate = request.getParameter("currenAppointDate");
        if (currenAppointDate != null) {
            session.removeAttribute("currenAppointDate");
        }
        // 宴会类型
        String currenDinnerType = (String) session.getAttribute("currenDinnerType");
        if (currenDinnerType != null) {
            session.removeAttribute("currenDinnerType");
        }
        // 是否包间
        String currenDinnerRooms = (String) session.getAttribute("currenDinnerRooms");
        if (currenDinnerRooms != null) {
            session.removeAttribute("currenDinnerRooms");
        }
        // 桌数
        String currenDinnerTables = (String) session.getAttribute("currenDinnerTables");
        if (currenDinnerTables != null) {
            session.removeAttribute("currenDinnerTables");
        }

        return;
    }

    // Add by zhajh at 2014/12/6 销售订单基本信息变化后保存当前信息内容处理 Begin
    public static String saveSalesCurrInfo(
            HttpServletRequest request, HttpServletResponse response) {
        HttpSession session = request.getSession();
        ShoppingCart cart = getCartObject(request);
        // 变化对象
        String currenSelName = request.getParameter("currenSelName");
        // 参数的取得
        // 产品店铺
        String currenProductStoreId = request.getParameter("currenProductStoreId");
        if (UtilValidate.isNotEmpty(currenProductStoreId)) {
            session.setAttribute("currenProductStoreId", currenProductStoreId);
        }
        // 选择目录
        ...
        if (UtilValidate.isNotEmpty(currenSalesChannelEnumId)) {
            session.setAttribute("currenSalesChannelEnumId", currenSalesChannelEnumId);
        }
        // 支付方法
        String currenPaymentMethodTypeAndId = request.getParameter("currenPaymentMethodTypeAndId");
        if (UtilValidate.isNotEmpty(currenPaymentMethodTypeAndId)) {
            session.setAttribute("currenPaymentMethodTypeAndId", currenPaymentMethodTypeAndId);
        }
        // 送货方式
        String currenSalesShippingMethod = request.getParameter("currenSalesShippingMethod");
        if (UtilValidate.isNotEmpty(currenSalesShippingMethod)) {
            session.setAttribute("currenSalesShippingMethod", currenSalesShippingMethod);
        }
        // 送货地址
        String currenShippingContactMechId = request.getParameter("currenShippingContactMechId");
        if (UtilValidate.isNotEmpty(currenShippingContactMechId)) {
            session.setAttribute("currenShippingContactMechId", currenShippingContactMechId);
        }
        ...
        if (UtilValidate.isNotEmpty(currenPartyIdDistribute)) {
            session.setAttribute("currenPartyIdDistribute", currenPartyIdDistribute);
        }
        // 仓库
        String currenShipGroupFacilityId = request.getParameter("currenShipGroupFacilityId");
        if (UtilValidate.isNotEmpty(currenShipGroupFacilityId)) {
            session.setAttribute("currenShipGroupFacilityId", currenShipGroupFacilityId);
        }
        // 订单名称
        String currenSalesOrderName = request.getParameter("currenSalesOrderName");
        if (UtilValidate.isNotEmpty(currenSalesOrderName)) {
            session.setAttribute("currenSalesOrderName", currenSalesOrderName);
        }
        // 选择目录
        String currenCatalogId = request.getParameter("currenCatalogId");
        if (UtilValidate.isNotEmpty(currenCatalogId)) {
            session.setAttribute("currenCatalogId", currenCatalogId);
        }
        // 缺省最早送货日期
        String currenShipAfterDate = request.getParameter("currenShipAfterDate");
        String currenShipAfterDateFormat = "";
        if (UtilValidate.isNotEmpty(currenShipAfterDate)) {
            Date d = new Date(currenShipAfterDate);
            SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            currenShipAfterDateFormat = formatter.format(d);
            session.setAttribute("currenShipAfterDate", currenShipAfterDateFormat);
        }
        if (UtilValidate.isNotEmpty(currenShipAfterDateFormat)) {
            if (currenShipAfterDateFormat.length() == 10) {
                currenShipAfterDateFormat += " 00:00:00.000";
            }
            cart.setShipAfterDate(java.sql.Timestamp.valueOf(currenShipAfterDateFormat));
        }
        // 缺省最迟送货日期
        String currenShipBeforeDate = request.getParameter("currenShipBeforeDate");
        String currenShipBeforeDateFormat = "";
        if (UtilValidate.isNotEmpty(currenShipBeforeDate)) {
            Date d = new Date(currenShipBeforeDate);
            SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            currenShipBeforeDateFormat = formatter.format(d);
            session.setAttribute("currenShipBeforeDate", currenShipBeforeDateFormat);
        }
        if (UtilValidate.isNotEmpty(currenShipBeforeDateFormat)) {
            if (currenShipBeforeDateFormat.length() == 10) {
                currenShipBeforeDateFormat += " 00:00:00.000";
            }
            cart.setShipBeforeDate(java.sql.Timestamp.valueOf(currenShipBeforeDateFormat));
        }
        // 预约单号
        String currenReserveOrderId = request.getParameter("currenReserveOrderId");
        if (UtilValidate.isNotEmpty(currenReserveOrderId)) {
            session.setAttribute("currenReserveOrderId", currenReserveOrderId);
        }
        // 拆分偏好
        String currenMaySplit = request.getParameter("currenMaySplit");
        if (UtilValidate.isNotEmpty(currenMaySplit)) {
            session.setAttribute("currenMaySplit", currenMaySplit);
        }
        // 货币
        String currencyUomId = request.getParameter("currencyUomId");
        if (UtilValidate.isNotEmpty(currencyUomId)) {
            session.setAttribute("currencyUomId", currencyUomId);
        }

        // 是否开运输发票
        String currenInvoicePerShipment = request.getParameter("currenInvoicePerShipment");
        if (UtilValidate.isNotEmpty(currenInvoicePerShipment)) {
            session.setAttribute("currenInvoicePerShipment", currenInvoicePerShipment);
        }

        // 发票标题
        String currenInvoiceTitle = request.getParameter("currenInvoiceTitle");
        if (UtilValidate.isNotEmpty(currenInvoiceTitle)) {
            session.setAttribute("currenInvoiceTitle", currenInvoiceTitle);
        }
        // 销售渠道(更新)
        String currenSalesChannelId = request.getParameter("currenSalesChannelId");
        if (UtilValidate.isNotEmpty(currenSalesChannelId)) {
            session.setAttribute("currenSalesChannelId", currenSalesChannelId);
        }
        // 预约人
        String currenAppointPerson =request.getParameter("currenAppointPerson");
        if (UtilValidate.isNotEmpty(currenAppointPerson)) {
            session.setAttribute("currenAppointPerson", currenAppointPerson);
        }
        // 到店人数
        ...
        // 预约人性别
        String currenAppointGender = request.getParameter("currenAppointGender");
        if (UtilValidate.isNotEmpty(currenAppointGender)) {
            session.setAttribute("currenAppointGender", currenAppointGender);
        }
        // 预留电话
        String currenAppointTel = request.getParameter("currenAppointTel");
        if (UtilValidate.isNotEmpty(currenAppointTel)) {
            session.setAttribute("currenAppointTel", currenAppointTel);
        }
        // 备注
        String currenAppointRemarks =request.getParameter("currenAppointRemarks");
        if (UtilValidate.isNotEmpty(currenAppointRemarks)) {
            session.setAttribute("currenAppointRemarks", currenAppointRemarks);
        }
        // 预约日期
        String currenAppointDate = request.getParameter("currenAppointDate");
        if (UtilValidate.isNotEmpty(currenAppointDate)) {
            session.setAttribute("currenAppointDate",currenAppointDate);
        }
        // 宴会类型
        String currenDinnerType =request.getParameter("currenDinnerType");
        if (UtilValidate.isNotEmpty(currenDinnerType)) {
            session.setAttribute("currenDinnerType", currenDinnerType);
        }
        // 是否包间
        String currenDinnerRooms =request.getParameter("currenDinnerRooms");
        if (UtilValidate.isNotEmpty(currenDinnerRooms)) {
            session.setAttribute("currenDinnerRooms", currenDinnerRooms);
        }
        // 桌数
        String currenDinnerTables =request.getParameter("currenDinnerTables");
        if (UtilValidate.isNotEmpty(currenDinnerTables)) {
            session.setAttribute("currenDinnerTables", currenDinnerTables);
        }
        //配送方式
        String distributionMethod = request.getParameter("distributionMethod");
        if (UtilValidate.isNotEmpty(distributionMethod)){
            session.setAttribute("distributionMethod", distributionMethod);
        }
        // 产品店铺
        request.setAttribute("currenProductStoreId", currenProductStoreId);
        // 销售渠道
        request.setAttribute("currenSalesChannelEnumId", currenSalesChannelEnumId);
        // 支付方式
        request.setAttribute("currenPaymentMethodTypeAndId", currenPaymentMethodTypeAndId);
        // 送货方式
        request.setAttribute("currenSalesShippingMethod", currenSalesShippingMethod);
        // 送货地址
        request.setAttribute("currenShippingContactMechId", currenShippingContactMechId);
        // 客户名称
        request.setAttribute("currenPartyId", currenPartyId);
        // 经销商标识
        request.setAttribute("currenPartyIdDistribute", currenPartyIdDistribute);
        // 仓库
        request.setAttribute("currenShipGroupFacilityId", currenShipGroupFacilityId);
        // 订单名称
        request.setAttribute("currenSalesOrderName", currenSalesOrderName);
        // 选择目录
        request.setAttribute("currenCatalogId", currenCatalogId);
        // 缺省最早送货日期
        request.setAttribute("currenShipAfterDate", currenShipAfterDate);
        // 缺省最迟送货日期
        request.setAttribute("currenShipBeforeDate", currenShipBeforeDate);
        // 预约单号
        request.setAttribute("currenReserveOrderId", currenReserveOrderId);
        // 拆分偏好
        request.setAttribute("currenMaySplit", currenMaySplit);
        // 货币
        request.setAttribute("currencyUomId", currencyUomId);
        // 是否开运输发票
        request.setAttribute("currenInvoicePerShipment", currenInvoicePerShipment);
        // 发票标题
        request.setAttribute("currenInvoiceTitle", currenInvoiceTitle);
        // 销售渠道（更新）
        request.setAttribute("currenSalesChannelId", currenSalesChannelId);
        // 预约人
        request.setAttribute("currenAppointPerson", currenAppointPerson);
        //到店人数
        request.setAttribute("personNumber", personNumber);
        // 预约人性别
        request.setAttribute("currenAppointGender", currenAppointGender);
        // 预留电话
        request.setAttribute("currenAppointTel", currenAppointTel);
        // 备注
        request.setAttribute("currenAppointRemarks", currenAppointRemarks);
        // 预约日期
        request.setAttribute("currenAppointDate", currenAppointDate);
        // 变化对象
        request.setAttribute("currenSelName", currenSelName);
        // 宴会类型
        request.setAttribute("dinnerType", currenDinnerType);
        // 是否包间
        request.setAttribute("dinnerRooms", currenDinnerRooms);
        // 桌数
        request.setAttribute("dinnerTables", currenDinnerTables);
        //配送方式
        request.setAttribute("distributionMethod", distributionMethod);
