...
public class ShoppingListEvents {
    ...
    public static String addBulkFromCart(Delegator delegator, LocalDispatcher dispatcher, ShoppingCart cart, GenericValue userLogin, String shoppingListId, String shoppingListTypeId, String[] items, boolean allowPromo, boolean append) throws IllegalArgumentException {
        ...
        if (UtilValidate.isEmpty(shoppingListId)) {
            ...
            try {
                newListResult = dispatcher.runSync("createShoppingList", UtilMisc.<String, Object>toMap("userLogin", userLogin, "productStoreId",null, "partyId", cart.getOrderPartyId(), "shoppingListTypeId", shoppingListTypeId, "currencyUom", cart.getCurrency()));
            ...
            if (cartIdInt != null) {
                ...
                if (allowPromo || !item.getIsPromo()) {
                    ...
                    try {
                        ...
                        ctx.put("isChoose",item.getIsChoose());
                        ctx.put("modifiedPrice",item.getBasePrice());
...
//          shoppingListItems = shoppingList.getRelated("ShoppingListItem");
            shoppingListItems = delegator.findByAnd("ShoppingListItem",UtilMisc.toMap("shoppingListId",shoppingList.getString("shoppingListId")),UtilMisc.toList("-shoppingListItemSeqId"));
            if (shoppingListItems == null) {
                shoppingListItems = FastList.newInstance();
            }
            
            //add by changsy 2018 增加购物车的item修改时间如果当选时间的24以外，自动改成未选择
            for (GenericValue item : shoppingListItems) {
                if (UtilValidate.isNotEmpty(item.get("isChoose")) && "Y".equals(item.getString("isChoose"))) {
                    Date yesterday = new java.util.Date((new java.util.Date()).getTime() - 86400000);
                    if (item.getTimestamp("lastUpdatedStamp").before(new Timestamp(yesterday.getTime()))) {
                        item.setString("isChoose", "N");
                    }
                }
            }
        ...
        if (!append) {
            ...
            if (UtilValidate.isEmpty(cartType) || "shoppingCart".equals(cartType)) {
                cart.setAutoSaveListId(shoppingListId);
            } else if (UtilValidate.isNotEmpty(cartType) && "wishCart".equals(cartType)) {
                cart.setWishListId(shoppingListId);
            } else if (UtilValidate.isNotEmpty(cartType) && "buyCart".equals(cartType)) {
                cart.setBuyListId(shoppingListId);
            }
        ...
        for (GenericValue shoppingListItem : shoppingListItems) {
            ...
            String isChoose = shoppingListItem.getString("isChoose");
            try {
                ...
                if (UtilValidate.isNotEmpty(configId)) {
                    //
                    configWrapper = ProductConfigWorker.loadProductConfigWrapper(delegator, dispatcher, configId, productId, null, prodCatalogId, cart.getWebSiteId(), cart.getCurrency(), cart.getLocale(), cart.getAutoUserLogin());
                }
                ...
                /*依次插入到第一个位置，则从shoppingCartItem按照seq倒序*/
                cart.addOrIncreaseItem(location,productId, null, quantity, null, null, null, null, null, null, attributes, prodCatalogId, configWrapper, null, null, null, dispatcher, isChoose,price);
//                location++;
    ...
    public static String getAutoSaveListId(Delegator delegator, LocalDispatcher dispatcher, String partyId, GenericValue userLogin, String productStoreId) throws GenericEntityException, GenericServiceException {
        ...
        if (partyId != null) {
            Map<String, Object> findMap = UtilMisc.<String, Object>toMap("partyId", partyId, "shoppingListTypeId", "SLT_SPEC_PURP", "listName", PERSISTANT_LIST_NAME);
     ...
     * Finds or creates a specialized (auto-save) shopping list used to record shopping bag contents between user visits.
     */
    public static String getBuyListId(Delegator delegator, LocalDispatcher dispatcher, String partyId, GenericValue userLogin, String productStoreId) throws GenericEntityException, GenericServiceException {
        if (partyId == null && userLogin != null) {
            partyId = userLogin.getString("partyId");
        }
        ...
        // TODO: add sorting, just in case there are multiple...
        if (partyId != null) {
            Map<String, Object> findMap = UtilMisc.<String, Object>toMap("partyId", partyId, "shoppingListTypeId", "SLT_DIRECT_PUR", "listName", PERSISTANT_LIST_NAME);
            List<GenericValue> existingLists = delegator.findByAnd("ShoppingList", findMap);
            Debug.logInfo("Finding existing auto-save shopping list with:  \nfindMap: " + findMap + "\nlists: " + existingLists, module);
        ...
        if (list == null && dispatcher != null) {
            Map<String, Object> listFields = UtilMisc.<String, Object>toMap("userLogin", userLogin, "productStoreId", productStoreId, "shoppingListTypeId", "SLT_DIRECT_PUR", "listName", PERSISTANT_LIST_NAME);
            Map<String, Object> newListResult = dispatcher.runSync("createShoppingList", listFields);
     ...
     * Finds or creates a specialized (auto-save) shopping list used to record shopping bag contents between user visits.
     */
    public static String getWishListId(Delegator delegator, LocalDispatcher dispatcher, String partyId, GenericValue userLogin, String productStoreId) throws GenericEntityException, GenericServiceException {
        if (partyId == null && userLogin != null) {
            partyId = userLogin.getString("partyId");
        }
        ...
        // TODO: add sorting, just in case there are multiple...
        if (partyId != null) {
            Map<String, Object> findMap = UtilMisc.<String, Object>toMap("partyId", partyId, "shoppingListTypeId", "SLT_WISH_LIST", "listName", PERSISTANT_LIST_NAME);
            List<GenericValue> existingLists = delegator.findByAnd("ShoppingList", findMap);
            Debug.logInfo("Finding existing auto-save shopping list with:  \nfindMap: " + findMap + "\nlists: " + existingLists, module);
        ...
        if (list == null && dispatcher != null) {
            Map<String, Object> listFields = UtilMisc.<String, Object>toMap("userLogin", userLogin, "productStoreId", productStoreId, "shoppingListTypeId", "SLT_WISH_LIST", "listName", PERSISTANT_LIST_NAME);
            Map<String, Object> newListResult = dispatcher.runSync("createShoppingList", listFields);
    ...
    public static void fillAutoSaveList(ShoppingCart cart, LocalDispatcher dispatcher) throws GeneralException {
        if (cart != null && dispatcher != null) {
            ...
            if (autoSaveListId == null) {
                autoSaveListId = getAutoSaveListId(delegator, dispatcher, null, userLogin, null);
...
//                List<GenericValue> shoppingListItems = shoppingList.getRelated("ShoppingListItem");
                List<GenericValue> shoppingListItems = delegator.findByAnd("ShoppingListItem",UtilMisc.toMap("shoppingListId",shoppingList.getString("shoppingListId")),UtilMisc.toList("shoppingListItemSeqId"));
     ...
     * Fills the specialized shopping list with the current shopping cart if one exists (if not leaves it alone)
     */
    public static void fillWishList(ShoppingCart cart, LocalDispatcher dispatcher) throws GeneralException {
        if (cart != null && dispatcher != null) {
            GenericValue userLogin = ShoppingListEvents.getCartUserLogin(cart);
            Delegator delegator = cart.getDelegator();
            String wishListId = cart.getWishListId();
            if (wishListId == null) {
                wishListId = getWishListId(delegator, dispatcher, null, userLogin, null);
                cart.setBuyListId(wishListId);
            }
            GenericValue shoppingList = delegator.findByPrimaryKey("ShoppingList", UtilMisc.toMap("shoppingListId", wishListId));
            Integer currentListSize = 0;
            if (UtilValidate.isNotEmpty(shoppingList)) {
//                List<GenericValue> shoppingListItems = shoppingList.getRelated("ShoppingListItem");
                List<GenericValue> shoppingListItems = delegator.findByAnd("ShoppingListItem",UtilMisc.toMap("shoppingListId",shoppingList.getString("shoppingListId")),UtilMisc.toList("shoppingListItemSeqId"));
            ...
            try {
                String[] itemsArray = makeCartItemsArray(cart);
                if (itemsArray != null && itemsArray.length != 0) {
                    addBulkFromCart(delegator, dispatcher, cart, userLogin, wishListId, null, itemsArray, false, false);
                } else if (itemsArray.length == 0 && currentListSize != 0) {
                    clearListInfo(delegator, wishListId);
                }
            } catch (IllegalArgumentException e) {
                throw new GeneralException(e.getMessage(), e);
            }
     ...
     * Fills the specialized shopping list with the current shopping cart if one exists (if not leaves it alone)
     */
    public static void fillBuyList(ShoppingCart cart, LocalDispatcher dispatcher) throws GeneralException {
        if (cart != null && dispatcher != null) {
            GenericValue userLogin = ShoppingListEvents.getCartUserLogin(cart);
            Delegator delegator = cart.getDelegator();
            String buyListId = cart.getBuyListId();
            if (buyListId == null) {
                buyListId = getBuyListId(delegator, dispatcher, null, userLogin, null);
                cart.setBuyListId(buyListId);
            }
            GenericValue shoppingList = delegator.findByPrimaryKey("ShoppingList", UtilMisc.toMap("shoppingListId", buyListId));
            Integer currentListSize = 0;
            if (UtilValidate.isNotEmpty(shoppingList)) {
//                List<GenericValue> shoppingListItems = shoppingList.getRelated("ShoppingListItem");
                List<GenericValue> shoppingListItems = delegator.findByAnd("ShoppingListItem",UtilMisc.toMap("shoppingListId",shoppingList.getString("shoppingListId")),UtilMisc.toList("shoppingListItemSeqId"));
            ...
            try {
                String[] itemsArray = makeCartItemsArray(cart);
                if (itemsArray != null && itemsArray.length != 0) {
                    addBulkFromCart(delegator, dispatcher, cart, userLogin, buyListId, null, itemsArray, false, false);
                } else if (itemsArray.length == 0 && currentListSize != 0) {
                    clearListInfo(delegator, buyListId);
                }
            } catch (IllegalArgumentException e) {
                throw new GeneralException(e.getMessage(), e);
            }
    ...
    public static String restoreAutoSaveList(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (autoSaveListId == null) {
            ...
            try {
                existingAutoSaveListId = getAutoSaveListId(delegator, dispatcher, null, userLogin, null);
            } catch (GeneralException e) {
                Debug.logError(e, module);
            }
