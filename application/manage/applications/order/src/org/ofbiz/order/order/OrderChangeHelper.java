...
//            OrderChangeHelper.createOrderInvoice(dispatcher, userLogin, orderId);
    ...
    /**
     * 支付完成实物订单变成待发货状态  Add By AlexYao 2016/01/30
     * @param dispatcher
     * @param userLogin
     * @param orderId
     * @return
     */
    public static boolean shipOrder(LocalDispatcher dispatcher, GenericValue userLogin, String orderId) {
        try {
            OrderChangeHelper.createReceivedPayments(dispatcher, userLogin, orderId);
//            OrderChangeHelper.createOrderInvoice(dispatcher, userLogin, orderId);
            OrderChangeHelper.orderStatusChanges(dispatcher, userLogin, orderId, "ORDER_WAITSHIP", "ITEM_WAITPAY", "ITEM_WAITSHIP", null);
        } catch (GenericEntityException e) {
            Debug.logError(e, module);
            return false;
        } catch (GenericServiceException e) {
    ...
    /**
     * 支付完成实物订单变成待收货状态  Add By AlexYao 2016/01/30
     * @param dispatcher
     * @param userLogin
     * @param orderId
     * @return
     */
    public static boolean receiveOrder(LocalDispatcher dispatcher, GenericValue userLogin, String orderId) {
        try {
            OrderChangeHelper.createReceivedPayments(dispatcher, userLogin, orderId);
//            OrderChangeHelper.createOrderInvoice(dispatcher, userLogin, orderId);
            OrderChangeHelper.orderStatusChanges(dispatcher, userLogin, orderId, "ORDER_WAITRECEIVE", "ITEM_WAITPAY", "ITEM_WAITRECEIVE", null);
        } catch (GenericEntityException e) {
            Debug.logError(e, module);
            return false;
        } catch (GenericServiceException e) {
    ...
    /**
     * 支付完成虚拟订单变成待评价状态  Add By AlexYao 2016/01/30
     * @param dispatcher
     * @param userLogin
     * @param orderId
     * @return
     */
    public static boolean evaluateOrder(LocalDispatcher dispatcher, GenericValue userLogin, String orderId) {
        try {
            OrderChangeHelper.createReceivedPayments(dispatcher, userLogin, orderId);
//            OrderChangeHelper.createOrderInvoice(dispatcher, userLogin, orderId);
            OrderChangeHelper.orderStatusChanges(dispatcher, userLogin, orderId, "ORDER_WAITEVALUATE", "ITEM_WAITPAY", "ITEM_WAITEVALUATE", null);
        } catch (GenericEntityException e) {
            Debug.logError(e, module);
            return false;
        } catch (GenericServiceException e) {
    ...
    public static boolean payOrder(LocalDispatcher dispatcher, GenericValue userLogin, String orderId) {
        return false;
    }
}
