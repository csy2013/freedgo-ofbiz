...
public class ShoppingCartHelper {
    ...
    /** Event to add an item to the shopping cart with accommodation. 注释掉 by dongxiao 2016.1.6 */
    /*public Map<String, Object> addToCart(String catalogId, String shoppingListId, String shoppingListItemSeqId, String productId,
            String productCategoryId, String itemType, String itemDescription,
            BigDecimal price, BigDecimal amount, BigDecimal quantity,
    ...
    public Map<String, Object> addToCart(String catalogId, String shoppingListId, String shoppingListItemSeqId, String productId,
        ...
        if (!UtilValidate.isEmpty(context.get("currenPartyId")) && cart.orderPartyId == null) {
            cart.setOrderPartyId((String) context.get("currenPartyId"));
            cart.setShipToCustomerPartyId((String) context.get("currenPartyId"));
        }
        ...
        try {
            if (productId != null) {
                ...
                itemId = cart.addOrIncreaseItem(productId, amount, quantity, reservStart, reservLength,
                        reservPersons, accommodationMapId, accommodationSpotId, shipBeforeDate, shipAfterDate, additionalFeaturesMap, attributes,
                        orderItemAttributes, catalogId, configWrapper, itemType, itemGroupNumber, pProductId, dispatcher, "Y",price);
                ...
                if (UtilValidate.isNotEmpty(productId) && orderItem.get("quantity") != null) {
                    ...
                    if (EntityTypeUtil.hasParentType(delegator, "ProductType", "productTypeId", ProductWorker.getProductTypeId(delegator, productId), "parentTypeId", "AGGREGATED")) {
                        try {
                            ...
                            configWrapper = ProductConfigWorker.loadProductConfigWrapper(delegator, dispatcher, configId, aggregatedProdId, null, catalogId, cart.getWebSiteId(), cart.getCurrency(), cart.getLocale(), cart.getAutoUserLogin());
                        } catch (GenericEntityException e) {
                    ...
                    try {
                        this.cart.addOrIncreaseItem(UtilValidate.isNotEmpty(aggregatedProdId) ? aggregatedProdId : productId, amount, orderItem.getBigDecimal("quantity"),
                                null, null, null, null, null, null, null, catalogId, configWrapper, orderItemTypeId, itemGroupNumber, null, dispatcher, "Y",null);
                        noItems = false;
                    } catch (CartItemModifyException e) {
                        errorMsgs.add(e.getMessage());
                    } catch (ItemNotFoundException e) {
            ...
            if (UtilValidate.isNotEmpty(quantStr)) {
                ...
                if (quantity.compareTo(BigDecimal.ZERO) > 0) {
                    ...
                    try {
                        ...
                        this.cart.addOrIncreaseItem(productId, null, quantity, null, null, null, null, null, null, null, catalogId, null, null, itemGroupNumberToUse, originalProductId, dispatcher, "Y",null);
                    } catch (CartItemModifyException e) {
                ...
                if (UtilValidate.isNotEmpty(quantStr)) {
                    ...
                    if (quantity.compareTo(BigDecimal.ZERO) > 0) {
                        ...
                        try {
                            ...
                            int index = this.cart.addOrIncreaseItem(productId, null, quantity, null, null, null, requirement.getTimestamp("requiredByDate"), null, null, null, catalogId, null, null, itemGroupNumber, null, dispatcher, "Y",null);
            ...
            if (quantity != null && quantity.compareTo(BigDecimal.ZERO) > 0) {
                try {
                    this.cart.addOrIncreaseItem(productCategoryMember.getString("productId"),
                            ...
                            catalogId, null, null, itemGroupNumber, null, dispatcher, "Y",null);
     ...
     * 修改商品数量
     */
    public Map<String, Object> modifyCart(String productId, String quantityStr, Locale locale) {
        Map<String, Object> result = null;
        if (locale == null) {
        ...
        ArrayList<String> errorMsgs = new ArrayList<String>();
        List<ShoppingCartItem> cartItems = this.cart.findAllCartItems(productId);
        try {
            for (ShoppingCartItem cartItem : cartItems) {
                ...
                if (cartItem != null) {
                    try {
                        ...
                        BigDecimal minQuantity = ShoppingCart.getMinimumOrderQuantity(delegator, cartItem.getBasePrice(), cartItem.getProductId());
                        if (quantity.compareTo(minQuantity) < 0) {
                            quantity = minQuantity;
                        }
                        cartItem.setQuantity(quantity, dispatcher, this.cart, true, false);
                        cart.setItemShipGroupQty(cartItem, quantity, 0);
            ...
            Debug.logWarning(e.getMessage(), UtilProperties.getMessage(resource_error, "OrderCaughtExceptionOnCartUpdate", cart.getLocale()));
            e.printStackTrace();
        }
     ...
     * Update the items in the shopping cart.
     */
    public Map<String, Object> deleteCart(String[] selectedItems) {
        Map<String, Object> result = null;
        ...
        for (int si = 0; si < selectedItems.length; si++) {
            String productId = selectedItems[si];
            List<ShoppingCartItem> items = null;
            try {
                
                items = this.cart.findAllCartItems(productId);
