...
public class OrderReadHelper {
   ...
   public List<Map<String, Object>> getShippableItemInfo(String shipGroupSeqId) {
    ...
    public BigDecimal getOrderAdjustmentsTotal1() {
        ...
        if(UtilValidate.isNotEmpty(adjustments)){
            for (int i = 0; i < adjustments.size(); i++) {
                ...
                adjustTotalAmount = adjustTotalAmount.add(adjustment.getBigDecimal("amount"));
            }
        }
    ...
    */
    ...
    public static BigDecimal getOrderGrandTotal(List<GenericValue> orderItems, List<GenericValue> adjustments) {
        ...
        total = ((total).add(adj)).setScale(scale,rounding);
        return total;
    }
    ...
    public static BigDecimal calcOrderAdjustments(List<GenericValue> orderHeaderAdjustments, BigDecimal subTotal, boolean includeOther, boolean includeTax, boolean includeShipping) {
        ...
        if (UtilValidate.isNotEmpty(orderHeaderAdjustments)) {
            List<GenericValue> filteredAdjs = filterOrderAdjustments(orderHeaderAdjustments, includeOther, false, includeShipping, false, false);
    ...
    public static BigDecimal getOrderItemsSubTotal1(List<GenericValue> orderItems, List<GenericValue> adjustments) {
        ...
        if(UtilValidate.isNotEmpty(orderItems)){
            for (int i = 0; i < orderItems.size(); i++) {
                ...
                totalSub = totalSub.add(unitPrice.multiply(quantity));
            }
        }
        return totalSub;
    }
...
//        Debug.logInfo("rental parameters....Nbr of persons:" + persons + " extra% 2nd person:" + secondPersonPerc + " extra% Nth person:" + nthPersonPerc + " Length: " + length + "  total rental adjustment:" + rentalAdjustment ,module);
   ...
   public static Map<String, Object> getOrderTaxByTaxAuthGeoAndPartyForDisplay(List<GenericValue> orderAdjustmentsOriginal) {
    ...
    //add by dongxiao 2015.12.30 得到订单商品
    public static List<GenericValue> getOrderProducts(Delegator delegator,String orderId) throws GenericEntityException {
        ...
        for (GenericValue oi : orderItems){
            GenericValue product = delegator.findByPrimaryKey("Product",UtilMisc.toMap("productId",oi.get("productId")));
            products.add(product);
        }
        return products;
    }

    //add by dongxiao 2015.12.30 得到订单商品原始价格
    public static BigDecimal getOrderProductsPrice(Delegator delegator,String orderId) throws GenericEntityException {
        List<GenericValue> products = new ArrayList<GenericValue>();
        List<GenericValue> orderItems =  new ArrayList<GenericValue>();
        orderItems = delegator.findByAnd("OrderItem",UtilMisc.toMap("orderId",orderId));
        BigDecimal originalPrice = BigDecimal.ZERO;
        for (GenericValue oi : orderItems){
            GenericValue productPrice = delegator.findByAnd("ProductAndPriceView", UtilMisc.toMap("productId", oi.get("productId"), "productPriceTypeId", "DEFAULT_PRICE")).get(0);
            originalPrice = productPrice.getBigDecimal("price");
        }
        return originalPrice;
    }

    public static BigDecimal getProductPrice(Delegator delegator,String productId) throws GenericEntityException {
        BigDecimal originalPrice = BigDecimal.ZERO;
        GenericValue productPrice = delegator.findByAnd("ProductAndPriceView", UtilMisc.toMap("productId", productId, "productPriceTypeId", "DEFAULT_PRICE")).get(0);
        originalPrice = productPrice.getBigDecimal("price");
        return originalPrice;
    }

    //add by dongxiao 2016.2.5 得到订单商品
    public static List<GenericValue> getOrderItems(Delegator delegator,String orderId) throws GenericEntityException {
        List<GenericValue> products = new ArrayList<GenericValue>();
        List<GenericValue> orderItems =  new ArrayList<GenericValue>();
        orderItems = delegator.findByAnd("OrderItem",UtilMisc.toMap("orderId",orderId));
        return orderItems;
    ...
    //add by dongxiao 2015.12.30 得到订单商品数量
    public static BigDecimal getOrderProductsNum(Delegator delegator,String orderId) throws GenericEntityException {
        List<GenericValue> orderItems =  new ArrayList<GenericValue>();
    ...
    //add by dongxiao 2016.3.8 得到退单商品数量
    public static BigDecimal getReturnProductNum(Delegator delegator,String orderId) throws GenericEntityException {
        ...
        for (GenericValue ri : returnItems){
            number = number.add(ri.getBigDecimal("returnQuantity"));
        }
        return number;
    }

    //add by dongxiao 2015.12.30 得到订单购买者信息
    public static Map<String,Object> getOrderPurchaseInfo(Delegator delegator,String orderId) {
        DynamicViewEntity dve = new DynamicViewEntity();
        Map<String,Object> info = new HashMap<String, Object>();
        dve.addMemberEntity("OT", "OrderRole");
        dve.addAliasAll("OT", "", null);

        dve.addMemberEntity("PP", "Person");
        ...
        try {
            if (UtilValidate.isNotEmpty(eli.getCompleteList())){
                try {
                    billToCustomer = eli.getCompleteList().get(0);
                } catch (GenericEntityException e) {
                    e.printStackTrace();
                }
                //下单用户信息
                info.put("name",(UtilValidate.isNotEmpty(billToCustomer.getString("nickname")))? billToCustomer.getString("nickname") : "" );
                info.put("fromMobile",(UtilValidate.isNotEmpty(billToCustomer.getString("mobile")))? billToCustomer.getString("mobile") : "" );
            }
        } catch (GenericEntityException e) {
            e.printStackTrace();
        }
        ...
        try {
            if (UtilValidate.isNotEmpty(eli.getCompleteList())){
                ...
                GenericValue city = delegator.findByPrimaryKey("Geo",UtilMisc.toMap("geoId", receivingCustomer.get("stateProvinceGeoId")));
                GenericValue country = delegator.findByPrimaryKey("Geo",UtilMisc.toMap("geoId", receivingCustomer.get("countyGeoId")));
                String address = "";
                if (province != null){
                    address += province.getString("name");
                }
                if (city != null){
                    address += city.getString("name");
                }
                if (country != null){
                    address += country.getString("name");
                }
                info.put("postalCode",receivingCustomer.get("postalCode"));
                info.put("tel",receivingCustomer.get("tel"));
                info.put("detailAddress",receivingCustomer.get("address1"));*/
            }else{
                info.put("toName","");
    ...
    //add by dongxiao 2015.12.30 得到收货人信息
    public static  Map<String,Object>   getToCustomerInfo(Delegator delegator,String orderId) {
        DynamicViewEntity dve = new DynamicViewEntity();
        Map<String,Object> info = new HashMap<String, Object>();
        List<EntityCondition> conditions = FastList.newInstance();
        conditions.add(EntityCondition.makeCondition("orderId", orderId));
        ...
        try {
            if (UtilValidate.isNotEmpty(eli.getCompleteList())){
                ...
                String address = "";
                if (province != null){
                    address += province.getString("geoName");
                }
                if (city != null){
                    address += " "+city.getString("geoName");
                }
                if (country != null){
                    address += " "+country.getString("geoName");
                }
    ...
    public static String  getCustomerName(Delegator delegator,String orderId) {
        DynamicViewEntity dve = new DynamicViewEntity();
        Map<String,Object> info = new HashMap<String, Object>();
        String name = "";
        dve.addMemberEntity("OT", "OrderRole");
        ...
        try {
            if (UtilValidate.isNotEmpty(eli.getCompleteList())){
                ...
                //下单用户信息
//                name = (UtilValidate.isNotEmpty(billToCustomer.getString("name")))? billToCustomer.getString("name") : "" ;
                name = (UtilValidate.isNotEmpty(billToCustomer.getString("nickname")))? billToCustomer.getString("nickname") : "" ;
    ...
    //add by dongxiao 2015.12.30 得到订单购买者信息
    public static Map<String,Object> getOrderUserInfo(Delegator delegator,String orderId) {
        DynamicViewEntity dve = new DynamicViewEntity();
        Map<String, Object> info = new HashMap<String, Object>();
        dve.addMemberEntity("OT", "OrderRole");
        dve.addAliasAll("OT", "", null);
        ...
        try {
            if (UtilValidate.isNotEmpty(eli.getCompleteList())) {
                ...
                //下单用户信息
                info.put("name", (UtilValidate.isNotEmpty(billToCustomer.getString("name"))) ? billToCustomer.getString("name") : "");
                info.put("fromMobile", (UtilValidate.isNotEmpty(billToCustomer.getString("mobile"))) ? billToCustomer.getString("mobile") : "");
            }
        } catch (GenericEntityException e) {
            e.printStackTrace();
    ...
    //add by dongxiao 2015.12.30 得到订单商家
    public static String getOrderBusinessName(Delegator delegator,String orderId) throws GenericEntityException {
        DynamicViewEntity dve = new DynamicViewEntity();
        Map<String,Object> info = new HashMap<String, Object>();
        List<EntityCondition> conditions = FastList.newInstance();
        String name = "";
        EntityListIterator eli = null;
        // do the lookup
        dve.addMemberEntity("ORR", "OrderRole");
        dve.addAliasAll("ORR", "", null);

        dve.addMemberEntity("PG", "PartyGroup");
        dve.addAlias("PG", "partyName");
        dve.addAlias("PG", "partyId");

        dve.addViewLink("ORR", "PG", Boolean.FALSE, ModelKeyMap.makeKeyMapList("partyId"));
        conditions.add(EntityCondition.makeCondition("orderId",orderId));
        conditions.add(EntityCondition.makeCondition("roleTypeId", "BILL_FROM_VENDOR"));
        EntityFindOptions findOpts = new EntityFindOptions(true, EntityFindOptions.TYPE_SCROLL_INSENSITIVE, EntityFindOptions.CONCUR_READ_ONLY, false);
        eli = delegator.findListIteratorByCondition(dve, EntityCondition.makeCondition(conditions, EntityOperator.AND), null, null, null, findOpts);
        List<GenericValue> list = eli.getCompleteList();
        if (UtilValidate.isNotEmpty(list)){
            GenericValue business = list.get(0);
            name = business.getString("partyName");
        }
        if (eli != null) {
            try {
                eli.close();
    ...
    //add by dongxiao 2016.1.29 判断订单是否有退货单、退款单 返回returnType 0:退货单，1：退款单,""：没有退货单或退款单
    public static String getOrderReturnType(Delegator delegator,String orderId,String productId) throws GenericEntityException {
        DynamicViewEntity dve = new DynamicViewEntity();
        Map<String,Object> info = new HashMap<String, Object>();
        List<EntityCondition> conditions = FastList.newInstance();
        String returnType = "";
        ...
        dve.addAliasAll("RI", "", null);
        dve.addViewLink("OI", "RI", Boolean.FALSE, ModelKeyMap.makeKeyMapList("orderId", "orderId", "productId", "productId"));
        conditions.add(EntityCondition.makeCondition("orderId", orderId));
        conditions.add(EntityCondition.makeCondition("productId",productId));
        eli = delegator.findListIteratorByCondition(dve, EntityCondition.makeCondition(conditions, EntityOperator.AND), null, null, null, findOpts);
        if (UtilValidate.isNotEmpty(eli.getCompleteList())){
            returnType = eli.getCompleteList().get(0).getString("returnType");
        }
        if (eli != null) {
            try {
                eli.close();
    ...
    //add by dongxiao 2016.2.5 判断订单是否有退货单、退款单 返回returnType 0:退货单，1：退款单,""：没有退货单或退款单
    public static List<GenericValue> getProductFeature(Delegator delegator,String productId) throws GenericEntityException {
        DynamicViewEntity dve = new DynamicViewEntity();
        Map<String,Object> info = new HashMap<String, Object>();
        List<EntityCondition> conditions = FastList.newInstance();
        ...
        // do the lookup
        GenericValue business = null;
        dve.addMemberEntity("pfa", "ProductFeatureAssoc");
        dve.addMemberEntity("pf", "ProductFeature");
        dve.addMemberEntity("pft", "ProductFeatureType");
        dve.addAliasAll("pfa", "", null);
        dve.addAliasAll("pf", "", null);
        //dve.addAliasAll("pft","", null);
        dve.addAlias("pft", "productFeatureTypeId");
        dve.addAlias("pft","productFeatureTypeName");
        dve.addViewLink("pfa", "pf", Boolean.FALSE, ModelKeyMap.makeKeyMapList("productFeatureId", "productFeatureId"));
        dve.addViewLink("pf", "pft", Boolean.FALSE, ModelKeyMap.makeKeyMapList("productFeatureTypeId", "productFeatureTypeId"));
        conditions.add(EntityCondition.makeCondition("productId",productId));
        eli = delegator.findListIteratorByCondition(dve, EntityCondition.makeCondition(conditions, EntityOperator.AND), null, UtilMisc.toSet("productFeatureTypeName","productFeatureName"), null, findOpts);
    ...
    public static List<String> getRturnImagesContentIds (Delegator delegator,String returnId){
        List<GenericValue> returnContents = null;
        try {
            returnContents = delegator.findByAnd("ReturnContent", UtilMisc.toMap("returnId", returnId));
        } catch (GenericEntityException e) {
            e.printStackTrace();
        }
        ...
        if (UtilValidate.isNotEmpty(returnContents)) {
            for (GenericValue returnContent : returnContents) {
                String contentId = returnContent.getString("contentId");
                list.add(contentId);
            }
        }
        return list;
    ...
    public static List<String> getRturnImages (Delegator delegator,String returnId){
        List<GenericValue> returnContents = null;
        try {
            returnContents = delegator.findByAnd("ReturnContent", UtilMisc.toMap("returnId", returnId));
        } catch (GenericEntityException e) {
    ...
    public static List<GenericValue> getDeliveryItems (Delegator delegator,String logisticsNumber) throws GenericEntityException {
        List<GenericValue> deliveryItems = delegator.findByAnd("DeliveryItem",UtilMisc.toMap("logisticsNumber",logisticsNumber),UtilMisc.toList("-dateTime"));
        return deliveryItems;
    }

    /*是否全部退单*/
    public static boolean isAllReturn (Delegator delegator,String orderId) throws GenericEntityException {
        boolean flag = true;
        BigDecimal orderProductNum = getOrderProductsNum(delegator,orderId);
        BigDecimal returnProductNum = getReturnProductNum(delegator, orderId);
        if (orderProductNum.equals(returnProductNum)){      //购买数量与退款数量一致
            List<GenericValue> returnItems = delegator.findByAnd("ReturnItem",UtilMisc.toMap("orderId",orderId));
            for (GenericValue g : returnItems){
                if (!"RETURN_COMPLETED".equals(g.getString("statusId"))){
                    flag = false;
                    break;
                }
            }
    ...
    public static String toSqlInStr (String ids){
        StringBuffer sb = new StringBuffer();
        List<String> idsArray = UtilMisc.toListArray(ids.split(","));
    ...
    //2018.5.5 得到订单组对应订单表orderGroupOrderRel获得orderGroupId
    public static String getOrderGroupId (Delegator delegator,String orderId)throws GenericEntityException{
        List<GenericValue> orderGroupOrderRel =  new ArrayList<GenericValue>();
        orderGroupOrderRel = delegator.findByAnd("OrderGroupOrderRel",UtilMisc.toMap("orderId",orderId));
        if(UtilValidate.isNotEmpty(orderGroupOrderRel)){

            return  orderGroupOrderRel.get(0).getString("orderGroupId");
        }
        return "";
    }
    //2018.5.7 得到促销信息(表orderAdjustment)
    public static List<GenericValue> getOrderAdjustments(Delegator delegator,String orderId) throws GenericEntityException {
        List<GenericValue> getOrderAdjustments =  FastList.newInstance();
        getOrderAdjustments = delegator.findByAnd("OrderAdjustment",UtilMisc.toMap("orderId",orderId));
        return getOrderAdjustments;
    }


    // Add by zhajh 20170410 得到订单商家信息 Begin
    public static GenericValue getOrderBusinessInfo(Delegator delegator,String orderId) throws GenericEntityException {
        DynamicViewEntity dve = new DynamicViewEntity();
        Map<String,Object> info = new HashMap<String, Object>();
        List<EntityCondition> conditions = FastList.newInstance();
        String name = "";
        ...
        dve.addViewLink("P", "PB", Boolean.FALSE, ModelKeyMap.makeKeyMapList("businessPartyId", "partyId"));
        conditions.add(EntityCondition.makeCondition("orderId",orderId));
        eli = delegator.findListIteratorByCondition(dve, EntityCondition.makeCondition(conditions, EntityOperator.AND), null, null, null, findOpts);
        if (UtilValidate.isNotEmpty(eli.getCompleteList())){
            business = eli.getCompleteList().get(0);
        }
        if (eli != null) {
            try {
                eli.close();
    ...
    // Add by zhajh 20170410 得到订单商家信息 End



    //add by zhajh 2018.5.24 得到订单店铺
    public static String getProductStoreNameByOrderId(Delegator delegator,String orderId) throws GenericEntityException {
        GenericValue orderHeaderInfo=delegator.findByPrimaryKey("OrderHeader",UtilMisc.toMap("orderId",orderId));
        GenericValue productStoreInfo=null;
        String name = "";
        if(UtilValidate.isNotEmpty(orderHeaderInfo)){
            String productStoreId=orderHeaderInfo.getString("productStoreId");
            productStoreInfo=delegator.findByPrimaryKey("ProductStore",UtilMisc.toMap("productStoreId",productStoreId));
            if(UtilValidate.isNotEmpty(productStoreInfo)){
                name=productStoreInfo.getString("storeName");
            }
        }
        return name;
    }
