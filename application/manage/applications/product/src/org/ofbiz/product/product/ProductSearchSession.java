...
public class ProductSearchSession {
    ...
    public static final String module = org.ofbiz.product.product.ProductSearchSession.class.getName();
    ...
    public static final String checkDoKeywordOverride(HttpServletRequest request, HttpServletResponse response) {
        ...
        org.ofbiz.product.product.ProductSearchSession.processSearchParameters(requestParams, request);
    ...
    public static void processSearchParameters(Map<String, Object> parameters, HttpServletRequest request) {
        ...
        // if there is another category, add a constraint for it 默认搜索下级
        if (UtilValidate.isNotEmpty(parameters.get("SEARCH_CATEGORY_ID"))) {
            ...
            searchAddConstraint(new ProductFieldConstraint(searchCategoryId, "platformClassId"), session);
            constraintsChanged = true;
        }
        
        // 搜索第一分类下的产品
        if (UtilValidate.isNotEmpty(parameters.get("PARENT_CATEGORY_ID"))) {
            String parentCategoryId = (String) parameters.get("PARENT_CATEGORY_ID");
            //逗号分隔的未经categoryId
            String searchCategoryId = getSubCategories(parentCategoryId, request);
            if (UtilValidate.isNotEmpty(searchCategoryId)) {
                searchAddConstraint(new ProductFieldConstraint(searchCategoryId, "parentCategoryId"), session);
                constraintsChanged = true;
            }
        }
        ...
        for (int catNum = 1; catNum < 10; catNum++) {
            if (UtilValidate.isNotEmpty(parameters.get("SEARCH_CATEGORY_ID" + catNum))) {
                ...
                searchAddConstraint(new CategoryConstraint(searchCategoryId, !"N".equals(searchSubCategories), exclude), session);
                constraintsChanged = true;
            }
        }
        ...
        if (UtilValidate.isNotEmpty(parameters.get("SEARCH_STRING"))) {
            ...
            searchAddConstraint(new KeywordConstraint(keywordString, anyPrefixSuffix, anyPrefixSuffix, null, "AND".equals(searchOperator)), session);
            constraintsChanged = true;
        }
        ...
        // 增加搜索店铺
        if (UtilValidate.isNotEmpty(parameters.get("SEARCH_STORE_ID"))) {
            String storeId = (String) parameters.get("SEARCH_STORE_ID");
            searchAddConstraint(new ProductSearch.ProductStoreFieldConstraint(storeId, "productStoreId"), session);
            constraintsChanged = true;
        }
        
        // 增加品牌
        if (UtilValidate.isNotEmpty(parameters.get("SEARCH_BRAND_NAME"))) {
            String brandName = (String) parameters.get("SEARCH_BRAND_NAME");
            searchAddConstraint(new ProductSearch.BrandFieldConstraint(brandName, "brandName"), session);
            constraintsChanged = true;
        }
        // 增加品牌
        if (UtilValidate.isNotEmpty(parameters.get("SEARCH_BRAND_IDS"))) {
            String brandName = (String) parameters.get("SEARCH_BRAND_IDS");
            searchAddConstraint(new ProductSearch.BrandFieldConstraint(brandName, "brandIds"), session);
            constraintsChanged = true;
        }
        ...
        // 增加特征的选择项IDs
        if (UtilValidate.isNotEmpty(parameters.get("SEARCH_FEAS"))) {
            String features = (String) parameters.get("SEARCH_FEAS");
            String[] featureArray = features.split("\\|");
            if (UtilValidate.isNotEmpty(featureArray)) {
                for (int i = 0; i < featureArray.length; i++) {
                    String productFeatureId = featureArray[i];
                    if (UtilValidate.isNotEmpty(productFeatureId)) {
                        searchAddConstraint(new FeatureConstraint(productFeatureId, null), session);
                        constraintsChanged = true;
                    }
                }
            }
        ...
        if (UtilValidate.isNotEmpty(parameters.get("LIST_PRICE_LOW")) || UtilValidate.isNotEmpty(parameters.get("LIST_PRICE_HIGH"))) {
            ...
            if (UtilValidate.isNotEmpty(parameters.get("LIST_PRICE_LOW"))) {
                try {
                    ...
                    Debug.logError("Error parsing LIST_PRICE_LOW parameter [" + parameters.get("LIST_PRICE_LOW") + "]: " + e.toString(), module);
                }
            }
            if (UtilValidate.isNotEmpty(parameters.get("LIST_PRICE_HIGH"))) {
                try {
                    ...
                    Debug.logError("Error parsing LIST_PRICE_HIGH parameter [" + parameters.get("LIST_PRICE_HIGH") + "]: " + e.toString(), module);
                }
            }
        ...
        if (sortOrder != null) {
            if ("SortKeywordRelevancy".equals(sortOrder) || "SKR".equals(sortOrder)) {
                searchSetSortOrder(new SortKeywordRelevancy(), session);
        ...
        //set the ExcludeProductTypes
        if (UtilValidate.isNotEmpty(parameters.get("EXCLUDE_PRODUCT_TYPE_IDS"))) {
            String productName = (String) parameters.get("EXCLUDE_PRODUCT_TYPE_IDS");
            searchAddConstraint(new ProductSearch.ExcludeProductTypesConstraint(productName), session);
            constraintsChanged = true;
        }
        //set the IncludeProductTypes
        if (UtilValidate.isNotEmpty(parameters.get("INCLUDE_PRODUCT_TYPE_IDS"))) {
            String productName = (String) parameters.get("INCLUDE_PRODUCT_TYPE_IDS");
            searchAddConstraint(new ProductSearch.IncludeProductTypesConstraint(productName), session);
            constraintsChanged = true;
        }
        ProductSearchOptions productSearchOptions = getProductSearchOptions(session);
    ...
    public static String getSubCategories(String prioritizeCategoryId, HttpServletRequest request) {
        String retCate = "";
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        DynamicViewEntity dynamicView = new DynamicViewEntity();
        dynamicView.addMemberEntity("PC1", "ProductCategory");
        List<EntityCondition> exps = FastList.newInstance();
        EntityFindOptions findOpts = new EntityFindOptions(true, EntityFindOptions.TYPE_SCROLL_INSENSITIVE, EntityFindOptions.CONCUR_READ_ONLY, true);
        dynamicView.addAlias("PC1", "firstCategoryId", "productCategoryId", null, false, false, null);
        dynamicView.addAlias("PC1", "firstLevel", "productCategoryLevel", null, false, false, null);
        exps.add(EntityCondition.makeCondition("firstLevel", 1L));
        dynamicView.addMemberEntity("PC2", "ProductCategory");
        dynamicView.addAlias("PC2", "secondCategoryId", "productCategoryId", null, false, false, null);
        dynamicView.addViewLink("PC1", "PC2", false, ModelKeyMap.makeKeyMapList("productCategoryId", "primaryParentCategoryId"));
        dynamicView.addAlias("PC2", "secondLevel", "productCategoryLevel", null, false, false, null);
        exps.add(EntityCondition.makeCondition("secondLevel", 2L));
        dynamicView.addMemberEntity("PC3", "ProductCategory");
        dynamicView.addAlias("PC3", "thirdCategoryId", "productCategoryId", null, false, false, null);
        dynamicView.addViewLink("PC2", "PC3", false, ModelKeyMap.makeKeyMapList("productCategoryId", "primaryParentCategoryId"));
        dynamicView.addAlias("PC3", "thirdLevel", "productCategoryLevel", null, false, false, null);
        exps.add(EntityCondition.makeCondition("thirdLevel", 3L));
        exps.add(EntityCondition.makeCondition("firstCategoryId", prioritizeCategoryId));
        try {
            EntityListIterator pli = delegator.findListIteratorByCondition(dynamicView, EntityCondition.makeCondition(exps, EntityOperator.AND), null, UtilMisc.toList("thirdCategoryId"), null, findOpts);
            List<GenericValue> categoryList = pli.getCompleteList();
            if (UtilValidate.isNotEmpty(categoryList)) {
                for (int i = 0; i < categoryList.size(); i++) {
                    GenericValue category = categoryList.get(i);
                    if (i == (categoryList.size() - 1)) {
                        retCate += category.getString("thirdCategoryId");
                    } else {
                        retCate += category.getString("thirdCategoryId") + ",";
                    }
                }
    ...
    public static Map<String, Object> getProductSearchResult(HttpServletRequest request, Delegator delegator, String prodCatalogId) {
        ...
        if ("Y".equals(noConditionFind) || UtilValidate.isNotEmpty(productSearchConstraintList)) {
            ...
            productSearchContext.setMaxResults(500);
            productSearchContext.setResultOffset(0);
            //获取所有的条件对应的产品不做分页处理
            allProductIds = productSearchContext.doSearch();
            if (maxResultsInt > 0) {
        ...
        result.put("allProductIds", allProductIds);
    ...
    public static String makeSearchParametersString(ProductSearchOptions productSearchOptions) {
        ...
        for (ProductSearchConstraint psc : constraintList) {
            if (psc instanceof CategoryConstraint) {
                CategoryConstraint cc = (CategoryConstraint) psc;
                categoriesCount++;
                if (isNotFirst) {
            ...
            } else if (psc instanceof FeatureConstraint) {
                FeatureConstraint fc = (FeatureConstraint) psc;
                featuresCount++;
                if (isNotFirst) {
                    searchParamString.append("&amp;");
                } else {
            ...
            } else if (psc instanceof KeywordConstraint) {
                KeywordConstraint kc = (KeywordConstraint) psc;
                keywordsCount++;
                if (isNotFirst) {
                    searchParamString.append("&amp;");
                } else {
        ...
        ResultSortOrder resultSortOrder = productSearchOptions.getResultSortOrder();
        if (resultSortOrder != null) {
            if (resultSortOrder instanceof SortKeywordRelevancy) {
    ...
    public static List<Map<String, String>> listCountByFeatureForType(String productFeatureTypeId, HttpSession session, Delegator delegator) {
        ...
        while ((searchResult = eli.next()) != null) {
            featureCountList.add(UtilMisc.<String, String>toMap("productFeatureId", searchResult.get("pfacProductFeatureId"), "productFeatureTypeId", searchResult.get("pfcProductFeatureTypeId"), "description", searchResult.get("pfcDescription"), "featureCount", Long.toString((Long) searchResult.get("featureCount"))));
        }
     ...
     * @param request
     * @param response
     * @return [{id:100001,name:商品名称,desc:商品描述,img:图片地址,price:默认价格},{...}],{listSize:10}
     */
    public static String doKeywordsearchResult(HttpServletRequest request, HttpServletResponse response) {
        Map<String, Object> result = FastMap.newInstance();
        checkDoKeywordOverride(request, response);
        ProductSearchSession.processSearchParameters(UtilHttp.getParameterMap(request), request);
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        String prodCatalogId = CatalogWorker.getCurrentCatalogId(request);
        Map<String, Object> searchResult = getProductSearchResult(request, delegator, prodCatalogId);
        List<String> productIds = (List<String>) searchResult.get("productIds");
        ...
        try {
            if (UtilValidate.isNotEmpty(productIds)) {
                for (int i = 0; i < productIds.size(); i++) {
                    ...
                    String productName = contentWrapper.get("PRODUCT_NAME").toString();
                    String productDesc = contentWrapper.get("DESCRIPTION").toString();
                    String img = contentWrapper.get("SMALL_IMAGE_URL").toString();
                    Map pMap = FastMap.newInstance();
                    pMap.put("name", productName);
                    pMap.put("id", productId);
                    pMap.put("desc", productDesc);
                    pMap.put("img", img);
                    List<GenericValue> productPrices = EntityUtil.filterByDate(delegator.findByAnd("ProductPrice", UtilMisc.toMap("productId", productId, "productPriceTypeId", "DEFAULT_PRICE")));
                    if (UtilValidate.isNotEmpty(productPrices)) {
                        Object price = EntityUtil.getFirst(productPrices).get("price");
        ...
        result.put("listSize", searchResult.get("listSize"));
