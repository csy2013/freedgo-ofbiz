...
public class ProductConfigWorker {
    ...
    public static final String module = org.ofbiz.product.config.ProductConfigWorker.class.getName();
    ...
    public static ProductConfigWrapper getProductConfigWrapper(String productId, String currencyUomId, String catalogId,String webSiteId,String productStoreId,GenericValue autoUserLogin,LocalDispatcher localDispatcher,Delegator delegator,Locale locale ) {
        ProductConfigWrapper configWrapper = null;
        try {
            /* caching: there is one cache created, "product.config"  Each product's config wrapper is cached with a key of
             * productId::catalogId::webSiteId::currencyUomId, or whatever the SEPARATOR is defined above to be.
    ...
    public static void fillProductConfigWrapper(ProductConfigWrapper configWrapper, HttpServletRequest request) {
        ...
        for (int k = 0; k < numOfQuestions; k++) {
            ...
            if (opts == null) {
                ...
                ConfigItem question = configWrapper.getQuestions().get(k);
            ...
            for (String opt: opts) {
                ...
                try {
                    ...
                    ConfigOption option = configWrapper.getItemOtion(k, cnt);
                    ...
                    if (tempResult.size() == selectedOptionSize && configsToCheck.containsAll(tempResult)) {
                        ...
                        if (UtilValidate.isNotEmpty(configOptionProductOptions)) {
                            ...
                            for (ConfigItem ci: questions) {
                                ...
                                boolean match = true;
                                for (ConfigOption anOption : selectedOptions) {
