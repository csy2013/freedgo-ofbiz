...
public class ProductEvents {
    ...
    public static final String module = org.ofbiz.product.product.ProductEvents.class.getName();
    ...
    public static String updateProductAssoc(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (!"CREATE".equals(updateMode) && !UtilValidate.isNotEmpty(fromDateStr)) {
    ...
    public static String clearAllLastViewed(HttpServletRequest request, HttpServletResponse response) {
        org.ofbiz.product.product.ProductEvents.clearLastViewedCategories(request, response);
        org.ofbiz.product.product.ProductEvents.clearLastViewedProducts(request, response);
    ...
    // Add by zhajh at 20170506 根据选择商品ID和contentId 对图片信息的处理 Begin

    /**
     * 根据选择商品ID和contentId 对图片信息的处理
     *
     * @param request
     * @param response
     * @return
     */
    public static String creatProductContentByIdForProduct(HttpServletRequest request, HttpServletResponse response) {
        GenericValue userLogin = (GenericValue) request.getSession().getAttribute("userLogin");
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        String productId = request.getParameter("productId");// 商品ID
        String contentId = request.getParameter("contentId");// contentId
        String picIndex = request.getParameter("picIndex");//  图片序号
        Timestamp nowTimestamp = UtilDateTime.nowTimestamp();
        List<GenericValue> productContentInfos = FastList.newInstance();
        String productContentTypeId = "";
        Boolean isCreateProductContent = false;

        List<GenericValue> curProductContentForTypeInfos = FastList.newInstance();// 当前商品的该类型的的图片集
        if (UtilValidate.isNotEmpty(picIndex)) {
            if ("1".equals(picIndex)) {
                productContentTypeId = "ADDITIONAL_IMAGE_1";
            } else if ("2".equals(picIndex)) {
                productContentTypeId = "ADDITIONAL_IMAGE_2";
            } else if ("3".equals(picIndex)) {
                productContentTypeId = "ADDITIONAL_IMAGE_3";
            } else if ("4".equals(picIndex)) {
                productContentTypeId = "ADDITIONAL_IMAGE_4";
            } else if ("5".equals(picIndex)) {
                productContentTypeId = "ADDITIONAL_IMAGE_5";
            }
        }
        // 商品Id，contentId，和contentTypeId 都有值的场合下
        if (UtilValidate.isNotEmpty(productId) &&
                UtilValidate.isNotEmpty(contentId) &&
                UtilValidate.isNotEmpty(productContentTypeId)) {
            try {
                // 首先删除该商品该分类图片的productcontent数据
                curProductContentForTypeInfos = delegator.findByAnd("ProductContent", UtilMisc.toMap("productId", productId, "productContentTypeId", productContentTypeId));
                if (curProductContentForTypeInfos.size() > 0) {
                    delegator.removeAll(curProductContentForTypeInfos);
                }
//                productContentInfos = delegator.findByAnd("ProductContent", UtilMisc.toMap("productId", productId, "contentId", contentId, "productContentTypeId", productContentTypeId));
                ...
                try {
                    ...
                    productContentCtx.put("fromDate", nowTimestamp);
                    productContentCtx.put("userLogin", userLogin);
                    productContentCtx.put("contentId", contentId);
                    dispatcher.runSync("createProductContent", productContentCtx);
                    isCreateProductContent = true;
                } catch (GenericServiceException e) {
                    Debug.logError(e, module);
                    return "error";
                }
        ...
        request.setAttribute("picIndex", picIndex);

        request.setAttribute("isCreate", isCreateProductContent);
        return "success";
    }
    // Add by zhajh at 20170506 根据选择商品ID和contentId 对图片信息的处理
