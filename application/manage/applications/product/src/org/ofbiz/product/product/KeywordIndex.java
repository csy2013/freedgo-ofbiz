...
public class KeywordIndex {
    ...
    public static final String module = org.ofbiz.product.product.KeywordIndex.class.getName();
    
    public static void forceIndexKeywords(GenericValue product) throws GenericEntityException {
        org.ofbiz.product.product.KeywordIndex.indexKeywords(product, true);
    }
    
    public static void indexKeywords(GenericValue product) throws GenericEntityException {
        org.ofbiz.product.product.KeywordIndex.indexKeywords(product, false);
    }
...
//        keywords.put(product.getString("productId").toLowerCase(), Long.valueOf(pidWeight));
        ...
        //品牌
        if (!"0".equals(UtilProperties.getPropertyValue("prodsearch", "index.weight.Product.brand", "0"))) {
            GenericValue brand = delegator.findByPrimaryKey("ProductBrand",UtilMisc.toMap("productBrandId",product.getString("brandId")));
            if (UtilValidate.isNotEmpty(brand)) {
                strings.add(brand.getString("brandName"));
            }
        }
        //分类
        if (!"0".equals(UtilProperties.getPropertyValue("prodsearch", "index.weight.Product.category", "0"))) {
            GenericValue category = delegator.findByPrimaryKey("ProductCategory", UtilMisc.toMap("productCategoryId", product.getString("primaryProductCategoryId")));
            if (UtilValidate.isNotEmpty(category)) {
                strings.add(category.getString("categoryName"));
            }
        }
        //分类
        if (!"0".equals(UtilProperties.getPropertyValue("prodsearch", "index.weight.Product.platCategory", "0"))) {
            GenericValue category = delegator.findByPrimaryKey("ProductCategory", UtilMisc.toMap("productCategoryId", product.getString("platformClassId")));
            if (UtilValidate.isNotEmpty(category)) {
                strings.add(category.getString("categoryName"));
            }
        }
        //标签
        //分类
        if (!"0".equals(UtilProperties.getPropertyValue("prodsearch", "index.weight.Product.label", "0"))) {
            List<GenericValue> tagAssoc = delegator.findByAnd("ProductTagAssoc", UtilMisc.toMap("productId", product.getString("productId")));
            if (UtilValidate.isNotEmpty(tagAssoc)) {
                ...
                if (UtilValidate.isNotEmpty(tags)) {
                    for (int i = 0; i < tags.size(); i++) {
                        strings.add(tags.get(i).getString("tagName"));
        ...
        if (UtilValidate.isNotEmpty(productContentTypes))
        ...
        for (
                Map.Entry<String, Long> entry : keywords.entrySet())
