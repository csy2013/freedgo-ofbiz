...
public class ImageManagementServices {
    ...
    public static final String module = org.ofbiz.product.imagemanagement.ImageManagementServices.class.getName();
    ...
    public static Map<String, Object> addMultipleuploadForProduct(DispatchContext dctx, Map<String, ? extends Object> context)
        ...
        if (UtilValidate.isNotEmpty(uploadFileName)) {
            ...
            if (fileLocation.lastIndexOf(fileSeparator) != -1) {
                filenameToUse = fileLocation.substring(fileLocation.lastIndexOf(fileSeparator) + 1);
            }
            ...
            String targetDirectory = imageServerPath + fileSeparator + productId;
            ...
            File file = new File(imageServerPath + fileSeparator + productId + fileSeparator + uploadFileName);
            String imageName = null;
            imagePath = imageServerPath + fileSeparator + productId + fileSeparator + uploadFileName;
            ...
            if (UtilValidate.isNotEmpty(imageResize)) {
                File fileOriginal = new File(imageServerPath + fileSeparator + productId + fileSeparator + imageName);
                ...
                try {
                    resultResize.putAll(org.ofbiz.product.imagemanagement.ImageManagementServices.scaleImageMangementInAllSize(context, imageName, sizeType, productId));
                } catch (IOException e) {
            ...
            String imageUrl = imageServerUrl + fileSeparator + productId + fileSeparator + imageName;
            String imageUrlThumb = imageServerUrl + fileSeparator + productId + fileSeparator + filenameToUseThumb;
    ...
    public static Map<String, Object> removeProductContentsAndImageFiles(DispatchContext dctx, Map<String, ? extends Object> context) {
        String productId = (String) context.get("productId");
        String contentTypeId = (String) context.get("productContentTypeId");
        Delegator delegator = dctx.getDelegator();
        LocalDispatcher dispatcher = dctx.getDispatcher();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        try {
            List<GenericValue> productContents = delegator.findByAnd("ProductContent",UtilMisc.toMap("productId",productId,"productContentTypeId",contentTypeId));
            if (UtilValidate.isNotEmpty(productContents)) {
                for (GenericValue productContent : productContents) {
                    String contentId = (String) productContent.get("contentId");
                    String productContentTypeId = (String)productContent.get("productContentTypeId");
                    Timestamp fromDate = (Timestamp)productContent.get("fromDate");
                    Map<String, Object> result = dispatcher.runSync("removeProductContentAndImageFileForAdditional",
                            UtilMisc.toMap("productId", productId, "contentId",contentId,"productContentTypeId",productContentTypeId,"fromDate",fromDate,"userLogin",userLogin));
            ...
            for( String sizeType : ScaleImage.sizeTypeList ) {
                String contTypeId = "XTRA_IMG_" + viewNumber + "_" + sizeType.toUpperCase();
                productContents = delegator.findByAnd("ProductContent",UtilMisc.toMap("productId",productId,"productContentTypeId",contTypeId));
                if (UtilValidate.isNotEmpty(productContents)) {
                    for (GenericValue productContent : productContents) {
                        String contentId = (String) productContent.get("contentId");
                        String productContentTypeId = (String)productContent.get("productContentTypeId");
                        Timestamp fromDate = (Timestamp)productContent.get("fromDate");
                        Map<String, Object> result = dispatcher.runSync("removeProductContentAndImageFileForAdditional", UtilMisc.toMap("productId", productId, "contentId", contentId,"productContentTypeId",productContentTypeId,"fromDate",fromDate,"userLogin",userLogin));
                        if (ServiceUtil.isError(result)) {
                            return result;
                        }
                    }
    ...
    public static Map<String, Object> removeImageFileForImageManagement(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        try {
            if (UtilValidate.isNotEmpty(contentId)) {
                String imageServerPath = FlexibleStringExpander.expandString(UtilProperties.getPropertyValue("catalog", "image.management.path"), context);
                File file = new File(imageServerPath + fileSeparator + productId + fileSeparator + dataResourceName);
                file.delete();
            }
        } catch (Exception e) {
            return ServiceUtil.returnError(e.getMessage());
    ...
    public static Map<String, Object> removeImageFileForAdditional(DispatchContext dctx, Map<String, ? extends Object> context) {
        String productId = (String) context.get("productId");
        String contentId = (String) context.get("contentId");
        String dataResourceName = (String) context.get("dataResourceName");
        ...
        try {
            if (UtilValidate.isNotEmpty(contentId)) {
                ...
                String imagePath = StringUtil.replaceString(objectInfo,"/images/","");
                String filePath = imageServerPath  + imagePath;
    ...
    public static Map<String, Object> scaleImageMangementInAllSize(Map<String, ? extends Object> context, String filenameToUse, String resizeType, String productId)
        ...
        resultBufImgMap.putAll(ImageTransform.getBufferedImage(imageServerPath + fileSeparator + productId + fileSeparator + filenameToUse, locale));
        ...
        if (resultBufImgMap.containsKey("responseMessage") && "success".equals(resultBufImgMap.get("responseMessage"))) {
            ...
            for (String sizeType : sizeTypeList) {
                ...
                resultScaleImgMap.putAll(ImageTransform.scaleImage(bufImg, imgHeight, imgWidth, imgPropertyMap, sizeType, locale, imgExtension, imageServerPath + fileSeparator + productId + fileSeparator + filenameToUse,imageServerPath + fileSeparator + productId + fileSeparator + filenameToUse));

                File deleteFile = new File(imageServerPath + fileSeparator + filenameToUse);
                deleteFile.delete();

                /* write Return Result */
                String imageUrl = imageServerUrl + fileSeparator + productId + fileSeparator + filenameToUse;
    ...
    public static Map<String, Object> createContentThumbnail(DispatchContext dctx, Map<String, ? extends Object> context, GenericValue userLogin, ByteBuffer imageData, String productId, String imageName) {
        ...
        if (fileLocationThumb.lastIndexOf(fileSeparator) != -1) {
            filenameToUseThumb = fileLocationThumb.substring(fileLocationThumb.lastIndexOf(fileSeparator) + 1);
        }
        ...
        File fileOriginalThumb = new File(imageServerPath + fileSeparator + productId + fileSeparator + filenameToUseThumb);
        try {
        ...
        try {
            resultResizeThumb.putAll(org.ofbiz.product.imagemanagement.ImageManagementServices.scaleImageMangementInAllSize(context, filenameToUseThumb, "thumbnail", productId));
        } catch (IOException e) {
            String errMsg = "Scale additional image in all different sizes is impossible : " + e.toString();
            Debug.logError(e, errMsg, module);
            return ServiceUtil.returnError(errMsg);
    ...
    public static Map<String, Object> createNewImageThumbnail(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        try {
            BufferedImage bufImg = ImageIO.read(new File(imageServerPath + fileSeparator + productId + fileSeparator + dataResourceName));
            ...
            if (dataResourceName.length() > 3) {
                ...
                Map<String, Object> resultResize = org.ofbiz.product.imagemanagement.ImageManagementServices.resizeImage(bufImg, imgHeight, imgWidth, resizeHeight, resizeWidth);
                ImageIO.write((RenderedImage) resultResize.get("bufferedImage"), mimeType, new File(imageServerPath + fileSeparator + productId + fileSeparator + filenameToUse));
                ...
                String contentIdThumb = (String) contentThumbResult.get("contentId");
                String imageUrlThumb = imageServerUrl + fileSeparator + productId + fileSeparator + filenameToUse;
                org.ofbiz.product.imagemanagement.ImageManagementServices.createContentAndDataResource(dctx, userLogin, filenameToUse, imageUrlThumb, contentIdThumb, "image/jpeg");
    ...
    public static Map<String, Object> renameImage(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        try {
            ...
            if (imageType.equals(mimeType)) {
                BufferedImage bufImg = ImageIO.read(new File(imageServerPath + fileSeparator + productId + fileSeparator + dataResourceName));
                ImageIO.write(bufImg, imgExtension, new File(imageServerPath + fileSeparator + productId + fileSeparator + filenameToUse));
                ...
                if (contentAssocList.size() > 0) {
                    for (int i = 0; i < contentAssocList.size(); i++) {
                        ...
                        String imageUrlAssoc = imageServerUrl + fileSeparator + productId + fileSeparator + filenameToUseAssoc;

                        BufferedImage bufImgAssoc = ImageIO.read(new File(imageServerPath + fileSeparator + productId + fileSeparator + drDataResourceNameAssoc));
                        ImageIO.write(bufImgAssoc, imgExtension, new File(imageServerPath + fileSeparator + productId + fileSeparator + filenameToUseAssoc));

                        File fileAssoc = new File(imageServerPath + fileSeparator + productId + fileSeparator + drDataResourceNameAssoc);
                        fileAssoc.delete();
