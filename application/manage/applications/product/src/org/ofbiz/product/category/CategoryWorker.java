...
public class CategoryWorker {
    ...
    public static List<GenericValue> getRelatedCategoriesRetRecursive(ServletRequest request, String attributeName, String parentId, boolean limitView, boolean excludeEmpty, boolean recursive) {
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        List<GenericValue> recurisiveCate = getRelatedCategoriesRet(delegator, attributeName, parentId, limitView, excludeEmpty, recursive);
//    if(!recurisiveCate.isEmpty())request.setAttribute(attributeName, recurisiveCate);
        return recurisiveCate;
    ...
    public static List<GenericValue> getParentCategoriesRet(ServletRequest request, String childId, boolean limitView, boolean recursive) {
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        return getParentCategoriesRet(delegator, childId, limitView, recursive);
    }
     ...
     * @return
     */
    public static List<GenericValue> getParentCategoriesRet(Delegator delegator, String childId, boolean limitView, boolean recursive) {
        List<GenericValue> categories = FastList.newInstance();

        if (Debug.verboseOn()) {
            Debug.logVerbose("[CategoryWorker.getParentCategoriesRet] ChildID: " + childId, module);
        }
        List<GenericValue> rollups = null;

        try {
            rollups = delegator.findByAndCache("ProductCategoryRollup",
                    UtilMisc.toMap("productCategoryId", childId),
                    UtilMisc.toList("sequenceNum"));
            if (limitView) {
                rollups = EntityUtil.filterByDate(rollups, true);
            }
        ...
        if (rollups != null) {
            ...
            for (GenericValue parent : rollups) {
                ...
                GenericValue cv = null;
                try {
                    cv = parent.getRelatedOneCache("ParentProductCategory");
                } catch (GenericEntityException e) {
                    Debug.logWarning(e.getMessage(), module);
                }
                if (cv != null) {
                    categories.add(cv);
                    if (recursive) {
                        categories.addAll(getParentCategoriesRet(delegator, cv.getString("productCategoryId"), limitView, recursive));
                    }
    ...
    /**
     * @param request
     * @param parentId     当前父category
     * @param limitView    是否过滤过期
     * @param excludeEmpty 是否去除空的目录
     * @param recursive    是否查找下级
     * @return
     */
    public static List<GenericValue> getTopRelatedCategoriesRet(ServletRequest request, String parentId, boolean limitView, boolean excludeEmpty, boolean recursive) {
        List<GenericValue> categories = FastList.newInstance();
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        if (Debug.verboseOn()) {
            Debug.logVerbose("[CategoryWorker.getRelatedCategories] ParentID: " + parentId, module);
        ...
        try {
            ...
            expList.add(EntityCondition.makeCondition("parentProductCategoryId", EntityOperator.EQUALS, parentId));
            List orExpList = FastList.newInstance();
            orExpList.add(EntityCondition.makeCondition("showInShop", EntityOperator.EQUALS, ""));
            orExpList.add(EntityCondition.makeCondition("showInShop", EntityOperator.EQUALS, null));
            EntityCondition condition = EntityCondition.makeCondition(orExpList, EntityOperator.OR);
            expList.add(condition);
            rollups = delegator.findList("ProductCategoryRollupAndChild", EntityCondition.makeCondition(expList, EntityOperator.AND), null, UtilMisc.toList("sequenceNum"), null,true);
            if (limitView) {
                rollups = EntityUtil.filterByDate(rollups, true);
            }
        } catch (GenericEntityException e) {
            ...
            for (GenericValue child : rollups) {
                ...
                if (excludeEmpty) {
                    if (!isCategoryEmpty(child)) {
                        ...
                        if (recursive) {
                            categories.addAll(getTopRelatedCategoriesRet(request, child.getString("productCategoryId"), limitView, excludeEmpty, recursive));
                        }
                    }
                } else {
                    categories.add(child);
