...
public class ProductStoreWorker {
    ...
    public static final String module = org.ofbiz.product.store.ProductStoreWorker.class.getName();
    ...
    public static GenericValue getProductStore(ServletRequest request) {
        ...
        String productStoreId = org.ofbiz.product.store.ProductStoreWorker.getProductStoreId(request);
        return org.ofbiz.product.store.ProductStoreWorker.getProductStore(productStoreId, delegator);
    }
    
    public static String getProductStoreIdBySiteId(String webSiteId, Delegator delegator) {
        ...
        return (String) webSite.get("productStoreId");
    }
    ...
    public static String getProductStoreId(ServletRequest request) {
        ...
        if (delegator != null) {
            try {
                List<GenericValue> productStores = delegator.findList("ProductStore", null, null, UtilMisc.toList("productStoreId"), null, true);
                if (UtilValidate.isNotEmpty(productStores)) {
                    GenericValue productStore = EntityUtil.getFirst(productStores);
                    return (String) productStore.get("productStoreId");
                }
            } catch (GenericEntityException e) {
                e.printStackTrace();
            }
    ...
    public static boolean autoSaveCart(GenericValue productStore) {
        return productStore != null && "Y".equalsIgnoreCase(productStore.getString("autoSaveCart"));
    }
    ...
    public static String getProductStorePayToPartyId(GenericValue productStore) {
        ...
        if (productStore != null && productStore.get("ownerPartyId") != null) {
            payToPartyId = productStore.getString("ownerPartyId");
        }
        return payToPartyId;
    }
    ...
    public static String getProductStorePaymentProperties(ServletRequest request, String paymentMethodTypeId, String paymentServiceTypeEnumId, boolean anyServiceType) {
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        String productStoreId = org.ofbiz.product.store.ProductStoreWorker.getProductStoreId(request);
        return org.ofbiz.product.store.ProductStoreWorker.getProductStorePaymentProperties(delegator, productStoreId, paymentMethodTypeId, paymentServiceTypeEnumId, anyServiceType);
    }
    ...
    public static String getProductStorePaymentProperties(Delegator delegator, String productStoreId, String paymentMethodTypeId, String paymentServiceTypeEnumId, boolean anyServiceType) {
        GenericValue setting = org.ofbiz.product.store.ProductStoreWorker.getProductStorePaymentSetting(delegator, productStoreId, paymentMethodTypeId, paymentServiceTypeEnumId, anyServiceType);
    ...
    public static boolean isStoreInventoryRequiredAndAvailable(ServletRequest request, GenericValue product, BigDecimal quantity, Boolean wantRequired, Boolean wantAvailable) {
        ...
        try {
            ...
            return (requiredOkay == null || requiredOkay.booleanValue()) && (availableOkay == null || availableOkay.booleanValue());
    ...
    public static BigDecimal getAverageProductStoreRating(GenericValue productStore, List<GenericValue> reviews, String productStoreId) {
        if (productStore == null) {
            Debug.logWarning("Invalid product store entity passed; unable to obtain valid product store rating", module);
        ...
        if (reviews == null) {
            try {
                reviews = productStore.getRelatedCache("ProductStoreReview", reviewByAnd, UtilMisc.toList("-postedDateTime"));
            } catch (GenericEntityException e) {
                Debug.logError(e, module);
            }
        }
    ...
    public static List<GenericValue> getProductStoreProducts(String productStoreId, Delegator delegator) {
            ...
            for (int i = 0; i < storeCatalogs.size(); i++) {
                ...
                if (UtilValidate.isNotEmpty(prodCategories)) {
                    for (int j = 0; j < prodCategories.size(); j++) {
                        ...
                        List<GenericValue> categories = CategoryWorker.getRelatedCategoriesRet(delegator, "", prodCategory.getString("categoryId"), true, false, true);
                        if (UtilValidate.isNotEmpty(categories)) {
                            for (int k = 0; k < categories.size(); k++) {
                                GenericValue category = categories.get(k);
                                //获取目录下的产品
                                categorySet.add(category.getString("categoryId"));
                            }
                        }
                    }
                }
        ...
        if (UtilValidate.isNotEmpty(categorySet)) {
            EntityCondition condition = EntityCondition.makeCondition("categoryId", EntityOperator.IN, categorySet);
            try {
                List<GenericValue> categoryMembers = EntityUtil.filterByDate(delegator.findList("ProductCategoryMember", condition, null, null, null, true));
                products = EntityUtil.getRelated("Product", categoryMembers);
                return products;
            } catch (GenericEntityException e) {
                e.printStackTrace();
            }
