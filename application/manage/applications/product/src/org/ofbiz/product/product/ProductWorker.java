...
public class ProductWorker {
    ...
    public static final String module = org.ofbiz.product.product.ProductWorker.class.getName();
    ...
    public static boolean shippingApplies(GenericValue product) {
        ...
        if (product != null) {
            ...
            if ("SERVICE".equals(productTypeId) || "SERVICE_PRODUCT".equals(productTypeId) || (org.ofbiz.product.product.ProductWorker.isDigital(product) && !org.ofbiz.product.product.ProductWorker.isPhysical(product))) {
    ...
    public static List<GenericValue> getVariantSelectionFeatures(GenericValue variantProduct) {
        ...
        GenericValue virtualProduct = org.ofbiz.product.product.ProductWorker.getParentProduct(variantProduct.getString("productId"), variantProduct.getDelegator());
        if (virtualProduct == null || !"Y".equals(virtualProduct.getString("productId"))) {
            return null;
        }
        // The selectable features from the virtual product
        List<GenericValue> selectableFeatures = org.ofbiz.product.product.ProductWorker.getProductFeaturesByApplTypeId(virtualProduct, "SELECTABLE_FEATURE");
        ...
        List<GenericValue> standardFeatures = org.ofbiz.product.product.ProductWorker.getProductFeaturesByApplTypeId(variantProduct, "STANDARD_FEATURE");
    ...
    public static BigDecimal getAverageProductRating(Delegator delegator, String productId, String productStoreId) {
        ...
        return org.ofbiz.product.product.ProductWorker.getAverageProductRating(product, productStoreId);
    }
...
//              Debug.logInfo("=====product found:" + productId + " and features: " + selectedFeatures);
     ...
     */
    public static List<Map<String,Object>> getProductReviewContent(Delegator delegator,String productReviewId){
        List<Map<String,Object>> dataResourceList = FastList.newInstance();
        try {
            List<GenericValue> productReviewContentList = delegator.findByAnd("ProductReviewContent",UtilMisc.toMap("productReviewId",productReviewId));
            for(GenericValue productReviewContent : productReviewContentList){
                GenericValue content = delegator.findByPrimaryKey("Content",UtilMisc.toMap("contentId",productReviewContent.getString("contentId")));
                if(null != content){
                    ...
                    if(null != dataResource) {
                        ...
                        map.put("title", dataResource.getString("dataResourceName"));
                        map.put("imgUrl", dataResource.getString("objectInfo"));
    ...
    // Add by zhajh at  20180423 获取商品特征属性 Begin
    /**
     * 获取商品的特征属性
     * @param delegator
     * @return
     */
    public static String getProductFeatureInfos(Delegator delegator,String productId) {
        Map<String,String> featureInfo= new HashMap<String,String>();// 特征信息
        List<Map<String,String>> feantureInfoList=FastList.newInstance();// 特征列表信息
        String strFeatureInfos="";
        try {
            // 取得商品信息
            GenericValue curProductInfo=delegator.findByPrimaryKey("Product",UtilMisc.toMap("productId",productId));
            GenericValue productFeatureInfo=null;
            GenericValue productFeatureTypeInfo=null;
            if(UtilValidate.isNotEmpty(curProductInfo)){
                String strProductFeatureInfos=curProductInfo.getString("featureProductId");
                if(UtilValidate.isNotEmpty(strProductFeatureInfos)){
                    String[] featureIds = strProductFeatureInfos.split("\\|");
                    if(UtilValidate.isNotEmpty(featureIds)){
                        for (String featureId : featureIds) {
                            featureInfo= new HashMap<String,String>();
                            productFeatureTypeInfo=null;
                            productFeatureInfo=null;
                            // 取得商品特征信息
                            productFeatureInfo=delegator.findByPrimaryKey("ProductFeature",UtilMisc.toMap("productFeatureId",featureId ));
                            if(UtilValidate.isNotEmpty(productFeatureInfo)){
                                featureInfo.put("featureId",featureId);
                                featureInfo.put("featureName",productFeatureInfo.getString("productFeatureName"));
                                // 取得商品特征类型信息
                                productFeatureTypeInfo=delegator.findByPrimaryKey("ProductFeatureType",UtilMisc.toMap("productFeatureTypeId",productFeatureInfo.getString("productFeatureTypeId")));
                                if(UtilValidate.isNotEmpty(productFeatureTypeInfo)){
                                    featureInfo.put("featureTypeId",productFeatureTypeInfo.getString("productFeatureTypeId"));
                                    featureInfo.put("featureTypeName",productFeatureTypeInfo.getString("productFeatureTypeName"));
                                }
                            }
                            if(UtilValidate.isNotEmpty(featureInfo)){
                                feantureInfoList.add(featureInfo);
                            }
                        }
                    }
                }
            ...
            if(UtilValidate.isNotEmpty(feantureInfoList)){
                for(int i=0;i<feantureInfoList.size();i++){
                    Map<String,String> curFeatureInfo=feantureInfoList.get(i);
                    String curFeatureTypeName=curFeatureInfo.get("featureTypeName");
                    String curFeatureName=curFeatureInfo.get("featureName");
                    String strFeatureInfo=curFeatureTypeName+"："+curFeatureName;
                    if(feantureInfoList.size()-1==i){
                        strFeatureInfos=strFeatureInfos+strFeatureInfo;
                    }else{
                        strFeatureInfos=strFeatureInfos+strFeatureInfo+",";
                    }
                }
        ...
        return strFeatureInfos;
    }
    // Add by zhajh at 20180423 获取商品特征属性 End
