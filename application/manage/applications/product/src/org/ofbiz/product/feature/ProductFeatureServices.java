...
public class ProductFeatureServices {
    ...
    public static final String module = org.ofbiz.product.feature.ProductFeatureServices.class.getName();
    ...
    /**
     * 获取sku的特征，根据分类ID，暂未实现
     * @param dctx
     * @param context
     * @return
     */
    public static Map<String, Object> findFeatureCategoriesByCategoryId(DispatchContext dctx, Map<String, ? extends Object> context) {
           Map<String,Object> ret =  ServiceUtil.returnSuccess();
           String categoryId = (String) context.get("productCategoryId");
           Delegator delegator = dctx.getDelegator();
        try {
            List<GenericValue> features =  EntityUtil.filterByDate(delegator.findByAnd("ProductFeatureCategoryAppl",UtilMisc.toMap("productCategoryId",categoryId)));
            List<GenericValue> proFeatureCategories = FastList.newInstance();
            if(UtilValidate.isNotEmpty(features)){
                for (int i = 0; i < features.size(); i++) {
                    GenericValue categoryAppl = features.get(i);
                    proFeatureCategories.add(categoryAppl.getRelatedOne("ProductFeatureCategory"));
                }
            }
            ret.put("features",proFeatureCategories);
    ...
    public static Map<String,Object> findFeaturesByFeatureCategoryId(DispatchContext dctx,Map<String,? extends Object> context){
        Map<String,Object> ret =  ServiceUtil.returnSuccess();
        String featureCategoryId = (String) context.get("featureCategoryId");
        Delegator delegator = dctx.getDelegator();
        try {
            List<GenericValue> features =  delegator.findByAnd("ProductFeatureAndDataSourceAppl",UtilMisc.toMap("productFeatureCategoryId",featureCategoryId),UtilMisc.toList("defaultSequenceNum"));
    ...
    /**
     * 上传特征图片到dataResource里面,同时增加feature_data_source
     * @param dctx
     * @param context
     * @return
     */
    public static Map<String,Object> uploadFeatureImage(DispatchContext dctx,Map<String,? extends Object> context){
        Map<String,Object> ret =  ServiceUtil.returnSuccess();
        String featureCategoryId = (String) context.get("featureCategoryId");
        LocalDispatcher dispatcher = dctx.getDispatcher();
        ...
        Delegator delegator = dctx.getDelegator();
        try {
             Map<String,Object> uploadRet = dispatcher.runSync("persistContentAndAssoc",context);
             if(ServiceUtil.isSuccess(uploadRet)){
                 ret.put("contentId", uploadRet.get("contentId"));
                 ret.put("dataResourceId", uploadRet.get("dataResourceId"));
                 delegator.removeByAnd("ProductFeatureDataResource",UtilMisc.toMap("productFeatureId",featureId));
                 GenericValue dataResource = delegator.makeValue("ProductFeatureDataResource");
                 dataResource.set("dataResourceId",uploadRet.get("dataResourceId"));
                 dataResource.set("productFeatureId",featureId);
     ...
     * @return
     */
    public static Map<String,Object> addFeatures(DispatchContext dctx,Map<String,? extends Object> context){
        Map<String,Object> ret =  ServiceUtil.returnSuccess();
        String featureCategoryId = (String) context.get("featureCategoryId");
        String featureNames = (String) context.get("featureNames");
        Delegator delegator = dctx.getDelegator();
        try {
            List<GenericValue> features =  delegator.findByAnd("ProductFeature",UtilMisc.toMap("productFeatureCategoryId",featureCategoryId));
            List<GenericValue> newFeatures = FastList.newInstance();
            if(UtilValidate.isNotEmpty(features)){
                String[] names = (featureNames.split(","));
                GenericValue oldFeature = features.get(features.size()-1);
                int seq = ((Long)oldFeature.get("defaultSequenceNum")).intValue()+1;
                for (int i = 0; i < names.length; i++) {
                    GenericValue feature =   delegator.makeValue("ProductFeature");
                    String name = names[i];
                    feature.setNextSeqId();
                    feature.set("description",name);
                    feature.set("idCode",feature.get("name"));
                    feature.set("productFeatureTypeId",oldFeature.get("productFeatureTypeId"));
                    feature.set("productFeatureCategoryId",oldFeature.get("productFeatureCategoryId"));
                    feature.set("defaultSequenceNum",new Long(seq));
                    feature.create();
                    seq++;
                    newFeatures.add(feature);
                }
            }
            ret.put("productFeatures",newFeatures);
        } catch (GenericEntityException e) {
            e.printStackTrace();
        }
        return ret;
