...
public class ProductContentWrapper implements ContentWrapper {
    ...
    public static final String module = org.ofbiz.product.product.ProductContentWrapper.class.getName();
    ...
    public static org.ofbiz.product.product.ProductContentWrapper makeProductContentWrapper(GenericValue product, HttpServletRequest request) {
        return new org.ofbiz.product.product.ProductContentWrapper(product, request);
    }
    ...
    /**
     * 获取商品分类  Add By Changsy
     * @param product
     * @param locale
     * @param dispatcher
     * @return
     */
    public static String getProductCategory(GenericValue product, Locale locale, LocalDispatcher dispatcher) {
        Delegator delegator=product.getDelegator();
        String categoryIds="";
        List<GenericValue> productCategoryMembers= null;
        try {
            productCategoryMembers = delegator.findByAnd("ProductCategoryMember", UtilMisc.toMap("productId", product.get("productId")));
        ...
        if(productCategoryMembers.size()>0){
            for(int i=0;i<productCategoryMembers.size();i++){
                if(i==0){
                    categoryIds=categoryIds + productCategoryMembers.get(i).get("productCategoryId");
                }else{
                    categoryIds=categoryIds +","+ productCategoryMembers.get(i).get("productCategoryId");
                }
            }
    ...
    public static String getProductContentAsText(GenericValue product, String productContentTypeId, Locale locale, String mimeTypeId, String partyId, String roleTypeId, Delegator delegator, LocalDispatcher dispatcher) {
        ...
        String imageUploadType = UtilProperties.getPropertyValue("content","content.image.upload.type");
        ...
        try {
           /* if (productContentCache.get(cacheKey) != null) {
                outString =  productContentCache.get(cacheKey);
            }*/
            ...
            if (outString.length() > 0) {
                ...
                outString = candidateOut == null? "" : candidateOut;
            }
        ...
        if(!"".equals(outString)) {
            if ("qiniu".equals(imageUploadType)) {
                if (productContentTypeId.startsWith("XTRA_IMG_") || productContentTypeId.startsWith("ADDITIONAL_IMAGE_") ||
                        "IMAGE".equals(productContentTypeId)) {
                    String domain = UtilProperties.getPropertyValue("content", "img.qiniu.domain");
                    outString = domain + outString;
                }
            }else if ("FTP".equals(imageUploadType)) {
                if (productContentTypeId.startsWith("XTRA_IMG_") || productContentTypeId.startsWith("ADDITIONAL_IMAGE_") ||
                        "IMAGE".equals(productContentTypeId)) {
                    String domain = UtilProperties.getPropertyValue("content", "content.image.upload.ftp.domain");
                    outString = domain + outString;
                }
            }
        }
    ...
    public static void getProductContentAsText(String productId, GenericValue product, String productContentTypeId, Locale locale, String mimeTypeId, String partyId, String roleTypeId, Delegator delegator, LocalDispatcher dispatcher, Writer outWriter) throws GeneralException, IOException {
        ...
        List<GenericValue> productContentList = delegator.findByAnd("ProductContent", UtilMisc.toMap("productId", productId, "productContentTypeId", productContentTypeId), UtilMisc.toList("-fromDate"));
        ...
        if (UtilValidate.isEmpty(productContentList) && ("Y".equals(product.getString("isVariant")))) {
            ...
            if (UtilValidate.isNotEmpty(parent)) {
                productContentList = delegator.findByAnd("ProductContent", UtilMisc.toMap("productId", parent.get("productId"), "productContentTypeId", productContentTypeId), UtilMisc.toList("-fromDate"));
                productContentList = EntityUtil.filterByDate(productContentList);
            }
        }
