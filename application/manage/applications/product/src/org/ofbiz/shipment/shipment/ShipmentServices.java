...
public class ShipmentServices {
    ...
    public static final String module = org.ofbiz.shipment.shipment.ShipmentServices.class.getName();
    ...
    public static Map<String, Object> editShipmentEstimate(DispatchContext dctx, Map<String, ? extends Object> context) {
        Map<String, Object> result = FastMap.newInstance();
        Delegator delegator = dctx.getDelegator();
        Locale locale = (Locale) context.get("locale");
        List<GenericValue> storeAll = FastList.newInstance();
        ...
        try {
            // Create the basic entity.
            GenericValue estimate = delegator.findByPrimaryKey("ShipmentCostEstimate", UtilMisc.toMap("shipmentCostEstimateId", shipmentCostEstimateId));
            
            estimate.set("shipmentCostEstimateId", shipmentCostEstimateId);
            estimate.set("productStoreShipMethId", productStoreShipMethId);
            estimate.set("shipmentMethodTypeId", productStoreShipMeth.getString("shipmentMethodTypeId"));
            estimate.set("carrierPartyId", productStoreShipMeth.getString("partyId"));
            estimate.set("carrierRoleTypeId", "CARRIER");
        ...
        try {
            ...
            for (GenericValue packageContent : packageContents) {
                ...
                if (ServiceUtil.isError(getOrderItemValueResult)) {
                    return getOrderItemValueResult;
                }
