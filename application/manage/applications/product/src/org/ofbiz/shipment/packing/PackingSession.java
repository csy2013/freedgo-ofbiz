...
public class PackingSession implements java.io.Serializable {
    ...
    public static final String module = org.ofbiz.shipment.packing.PackingSession.class.getName();
    ...
    public String complete(boolean force) throws GeneralException {
        ...
        //设置订单项目，订单状态.在所有的订单项都已经picked的情况下
        List oisgirs = this.getDelegator().findByAnd("OrderItemShipGrpInvRes",UtilMisc.toMap("orderId",getPrimaryOrderId()));
        if(UtilValidate.isEmpty(oisgirs)) {
            getDispatcher().runSync("changeOrderStatus", UtilMisc.toMap("orderId", getPrimaryOrderId(), "statusId", "ORDER_PICKED", "setItemStatus", "Y", "userLogin", getUserLogin()));
            getDispatcher().runSync("changeOrderStatus", UtilMisc.toMap("orderId", getPrimaryOrderId(), "statusId", "ORDER_PACKED", "setItemStatus", "Y", "userLogin", getUserLogin()));
        }
