    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="EditForm" method="post" class="form-horizontal" role="form" action="<@ofbizUrl>AddFacility</@ofbizUrl>">
          ...
          <div class="form-group" data-type="format" data-reg="/^[1-9]\d*$/" data-mark="${uiLabelMap.InventoryAvailable}" >
            <label class="control-label col-sm-2"><i class="required-mark">*</i>库存总量:</label>
            <div class="col-sm-10">
...
<script>
	var data_tbl;
	var ajaxUrl = "InventoryListData?facilityId=${facilityId}";
	$(function(){
			...
			columns:[
				{"title":"${uiLabelMap.InventoryProductName}","code":"productName","sort":true,
				 "handle":function(td,record){
				 	var img="";
				 	if(record.imgUrl){
                        img = "<img class='img-responsive' style='max-height: 100px;max-width: 100px;' src='"+record.imgUrl+"'>";
				 	}
				...
				{"title":"${uiLabelMap.InventoryTotalNum}","code":"totalNum"},
				{"title":"${uiLabelMap.InventoryAlreadyLock}","code":"alreadyLock"},
				{"title":"可用库存","code":"available"}
				<!-- 是否有编辑权限-->
				<#if security.hasEntityPermission("INVENTORY_LIST", "_UPDATE", session)>
					,
