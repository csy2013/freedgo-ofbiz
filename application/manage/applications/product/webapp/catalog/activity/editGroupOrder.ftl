<form class="form-horizontal dp-validateForm" role="form" method="post" name="addGroupOrder" action="<@ofbizUrl>addGroupOrder</@ofbizUrl>">
...
<@htmlScreenTemplate.renderScreenletBegin id="" title="团购活动基本信息" collapsible="" showMore=true/>
    ...
    <#if security.hasEntityPermission("PRODPROMO_GROUPORDER", "_UPDATE", session)>
    <#--<button class="btn btn-danger btn-flat" onclick="deleteGroupOrder(${request.getParameter("activityId")})">删除</button>-->
        <button class="btn btn-success btn-flat" onclick="updateGroupOrder()">修改</button></#if>
    </div>
            ...
            <div class="col-sm-9">
                ...
                <#list activityTypeEnums as activityTypeEnum>
                    <option value="${(activityTypeEnum.enumId)?if_exists}"
                            <#if activityTypeEnum.enumId == 'GROUP_ORDER'>selected</#if>>${(activityTypeEnum.get("description",locale))?if_exists}</option>
                </#list>
                </select>
    ...
    <div class="row">
        ...
        <div class="form-group col-sm-6">
            <label for="subTitle" class="col-sm-3 control-label"><i class="required-mark">*</i>参团支付条件</label>

            <div class="col-sm-6">
                <select class="form-control" id="activityPayType" name="activityPayType">
                <#list activityPayTypes as activityPayType>
                    <option value="${(activityPayType.enumId)?if_exists}">${(activityPayType.get("description",locale))?if_exists}</option>
                </#list>
                </select>
            </div>
            <div class="col-sm-3">
...
<script type="text/javascript">
    function addGroupOrderRule() {
        event.preventDefault();
        $('#orderGouprRuleId').val('');
        $('#orderGouprRulePersonNum').val('');
        $('#orderGouprRulePersonAmount').val('');
        $('#modal_add').modal('show');
    }

    function saveGroupOrderRule() {
        var len = $('#groupOrderRuleTable').find("tr").length;
        var options = "<tr><td>" + (len) + "</td><td>" + $('#orderGouprRuleId').val() + "</td><td>" + $('#orderGouprRulePersonNum').val() + "</td><td>" + $('#orderGouprRulePersonAmount').val() + "</td>";
        options += "<td><button class='btn-default' onclick='orderGroupRuleDel(this)'>删除</button></td></tr>";
        $('#groupOrderRuleTable').find("tr").parent().append(options);
        $('#modal_add').modal('toggle');
    }

    function orderGroupRuleDel(obj) {
        event.preventDefault();
    ...
    $(function () {
        ...
        var payType = $('#activityPayType').val();
        console.log('paryType' + payType);
        if (payType == 'PART_PAY') {
            $('#productPrice').show();
        } else {
            $('#productPrice').hide();
        ...
        $('#activityPayType').change(function () {
            if ($(this).val() == 'PART_PAY') {
                $('#productPrice').show();
            } else {
                $('#productPrice').hide();
            }
        ...
        $.ajax(
                {
                    ...
                    success: function (data) {
                        ...
                        if (data) {
                            ...
                            if (data.activityPayType == 'PART_PAY') {
                                $('#productPrice').show();
                            } else {
                                $('#productPrice').hide();
                            }
                            ...
                            if (data.d_productGroupOrderRules) {
                                for (var i = 0; i < data.d_productGroupOrderRules.length; i++) {
                                    ...
                                    trObj += "<td><button class='btn-default' onclick='orderGroupRuleDel(this)'>删除</button></td>";
                                    trObj += "</tr>";
                                    $('#groupOrderRuleTable').find('tr').parent().append(trObj);
                                }
                            }
    ...
    function updateGroupOrder() {
        event.preventDefault();
        var rules = "";
        $('#groupOrderRuleTable').find('tr').each(function (i, elem) {

            if (i > 0) {
                var seq = $(elem).children()[1].innerText;
                var quantity = $(elem).children()[2].innerText;
                var price = $(elem).children()[3].innerText;
                rules += seq + ":" + quantity + ":" + price;
                rules += ",";
            }
