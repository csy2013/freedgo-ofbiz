...
<script language="JavaScript" type="text/javascript">
    ...
    $(function(){
       ...
       var optypeTmp="${parameters.operateType?if_exists}";
	    ...
	    if(optypeTmp=="update"){
	       getProductFeatureNamesForUpdate(productId)
	    }else{
           getProductFeatureNamesForCreate(ids);
        }
    ...
    // 取得选择的特征值
   function getProductFeatureNamesForCreate(ids){
   ...
   // 取得选择的特征值
   function getProductFeatureNamesForUpdate(productId){
		...
		jQuery.ajax({
	        ...
	        success: function(data) {
	           ...
	           for(var i=0;i<listSize;i++){
	               ...
	               var salePrice=priceList[1].price;
	               var costPrice=priceList[0].price;
	               var marketPrice=priceList[2].price;
	               ...
	               '<div class="'+conId+'">'+
			           ...
			           '<div class="form-group">'+
			                ...
			                '<label  class="col-sm-2 control-label">重量(kg):</label>'+
			                '<div class="col-sm-3">'+
			                    '<input type="text" class="form-control" id="weight" name="weight" value="'+productGoodInfo.productInfo.weight+'" placeholder="" />'+
			                '</div>'+
			            '</div>'+
			            ...
			            '<div class="form-group">'+
			                '<label  class="col-sm-2 control-label">体积(m³):</label>'+
			                '<div class="col-sm-3">'+
			                    '<input type="text" class="form-control" id="volume" name="volume" value="'+productGoodInfo.productInfo.weight+'" placeholder="" />'+
			                '</div>'+
			            '</div>'+
	    ...
	    jQuery.ajax({
	        ...
	        success: function(data) {
	           ...
	           for (var i=0;i<facilityInfoList.length;i++){
	              ...
	              var tr1=' <tr><td><input type="hidden" name="facilitynameId" id="" value="'+facilityId+'">'+facilityName+'</td> <td>'+totalNum+'</td><td>'+alreadyLockQuantitySum+'</td><td>'+accountingQuantityTotal+'</td><td>'+warningQuantity+'</td> <td class="fc_td"><button type="button" class="js-button btn btn-danger btn-sm">${uiLabelMap.BrandDel}</button></td></tr>';
