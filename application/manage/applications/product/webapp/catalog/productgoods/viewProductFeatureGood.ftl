	...
	<div class="box-body">
	    ...
	    <form class="form-horizontal" method="post"  role="form" action="" name="updateProductFeatureGoods"  id="updateProductFeatureGoods"  class="" enctype="multipart/form-data">
	       ...
	       <div class="modal-footer">
		        ...
		        <#assign  productMainInfo = delegator.findByPrimaryKey("Product",{"productId":parameters.productId?if_exists})>
		        <#if !(productMainInfo.isVerify?if_exists=='Y' || productMainInfo.isOnline?if_exists=='Y') >
       ...
       getProductFeatureNames(productId);
        ...
        $('.js-btn-group').on('click','.btn',function(){
	     ...
	     $("#btnReturn").click(function(){
	         document.location.href="<@ofbizUrl>viewProductGoodForHasFeature?productId="+${parameters.productId?if_exists}+"&operateType=view</@ofbizUrl>";
	    ...
	    jQuery.ajax({
	        ...
	        success: function(data) {
	           ...
	           for (var i=0;i<facilityInfoList.length;i++){
	              ...
	              var tr1=' <tr><td><input type="hidden" name="" id="" value="'+facilityId+'">'+facilityName+'</td> <td>'+totalNum+'</td><td>'+alreadyLockQuantitySum+'</td><td>'+accountingQuantityTotal+'</td><td>'+warningQuantity+'</td><td>'+warningMail+'</td> <td class="fc_td"><button type="button" class="js-button btn btn-danger btn-sm">${uiLabelMap.BrandDel}</button></td></tr>';
