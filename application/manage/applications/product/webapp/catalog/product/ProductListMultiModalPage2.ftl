...
<script>
	...
	var sDate='${parameters.startDate?if_exists}';
	var eDate='${parameters.endDate?if_exists}';

    console.log(timeStamp2String(parseInt(sDate) * 1000));
    console.log(timeStamp2String(parseInt(eDate) * 1000));


	var start_date=timeStamp2String(parseInt(sDate) * 1000);
    var end_date=timeStamp2String(parseInt(eDate) * 1000);

    var ProductFM_ajaxUrl = "/catalog/control/UnUsedProductListForJson?VIEW_SIZE=5&startDate="+start_date+"&endDate="+end_date;

    console.log(ProductFM_ajaxUrl);
	$(function(){
