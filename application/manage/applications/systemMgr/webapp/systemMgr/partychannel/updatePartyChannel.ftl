...
<form class="form-horizontal" role="form" method="post" name="addPartyChannel" id="addPartyChannel"
    ...
    <div class="box box-info">
        ...
        <input type="hidden"  class="form-control dp-vd" id="channelId" name="channelId" >
        <div class="box-body">
            <div class="row">
...
<script>
    $(function () {
        ...
        //获取该channel的信息并进行回显
        $.ajax({
            url: "getChannelById${externalKeyParam}",
            type: "POST",
            data:{
                channelId:channelId
            },
            ...
            success: function (data) {
                if(data.hasOwnProperty("_ERROR_MESSAGE_")){
                    ...
                    var channel = data.channelInfo;
                    $("#channelId").val(channel.channelId);
                    $("#channelCode").val(channel.channelCode);
                    $("#channelName").val(channel.channelName);
                    $("#remark").val(channel.remark);
                    $("input[name='status'][value='"+channel.status+"']").attr("checked",true);
                }
            },
            error: function (data) {
                $.tipLayer("操作失败！");
