...
<div class="box box-info">
    ...
    <div class="box-body">
        <form id="express-form" class="form-inline clearfix" role="form" action="<@ofbizUrl>editDocuments</@ofbizUrl>" method="post">
            <input type="hidden" name="logisticsDocumentsId" value="${logisticsDocuments.logisticsDocumentsId?if_exists}">
            <div class="form-group m-b-10">
                <label class="control-label">单据名称：</label>
                <select name="billName" class="form-control">
                    <#list logisticsCompanys as logisticsCompany>
                        <#if logisticsDocuments.logisticsCompanyId == logisticsCompany.id>
                            <option value="${logisticsCompany.id}"
                                    <#if logisticsDocuments.logisticsCompanyId == logisticsCompany.id>selected="selected"</#if>>${logisticsCompany.companyName?if_exists}</option>
                        </#if>
                        <#assign logisticsDocument = delegator.findByAnd("LogisticsDocuments",{"logisticsCompanyId":logisticsCompany.id})>
                        <#if !logisticsDocument?has_content>
                            <option value="${logisticsCompany.id}"
                                    <#if logisticsDocuments.logisticsCompanyId == logisticsCompany.id>selected="selected"</#if>>${logisticsCompany.companyName?if_exists}</option>
                        </#if>
                    </#list>
            ...
            <div class="form-group m-b-10">
                ...
                <input id="billWidth" name="billWidth" type="text" class="form-control" style="width:75px;" value="${logisticsDocuments.width?if_exists}">
                X
                <input id="billHeight" name="billHeight" type="text" class="form-control" style="width:75px;" value="${logisticsDocuments.height?if_exists}">
            </div>
            <div class="form-group m-b-10">
            ...
            <input id="express-bg" name="express-bg" type="hidden" value="${logisticsDocuments.contentId?if_exists}">
            <input id="express-content" name="express-content" type="hidden" value="${logisticsDocuments.content?if_exists}">
            <div class="express-box">
                <img class="express-bg" src="/content/control/getImage?contentId=${logisticsDocuments.contentId?if_exists}"
                     style="width: ${logisticsDocuments.width?if_exists}px;height: ${logisticsDocuments.height?if_exists}px;">
                <div id="express-field-box" class="express-field-box"><#if logisticsDocuments.content?has_content>${StringUtil.wrapString(logisticsDocuments.content)}</#if></div>
            </div>
        </form>
    </div>
</div>
<script>
    $(function () {
        ...
        $('#printItem').on('select2:select', function () {
            var key = this.value, text = $(':selected', this).text();
