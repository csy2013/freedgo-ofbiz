...
public class PartyServices {
    ...
    public static Map<String, Object> deleteParty(DispatchContext ctx, Map<String, ? extends Object> context) {
        ...
        try {
            ...
            delegator.removeByAnd("PartyContactMechPurpose", UtilMisc.toMap("partyId", partyId));
            
            List<GenericValue> logins = delegator.findByAnd("UserLogin", UtilMisc.toMap("partyId", partyId));
            if (UtilValidate.isNotEmpty(logins)) {
                for (int i = 0; i < logins.size(); i++) {
                    GenericValue userlogin = logins.get(i);
                    delegator.removeByAnd("UserLoginSecurityGroup", UtilMisc.toMap("userLoginId", userlogin.get("userLoginId")));
                    delegator.removeByAnd("UserLoginHistory", UtilMisc.toMap("partyId", partyId));
            ...
            delegator.removeByAnd("PartyStatus", UtilMisc.toMap("partyId", partyId));
            delegator.removeByAnd("PartyAttribute", UtilMisc.toMap("partyId", partyId));
            delegator.removeByAnd("PartyScore", UtilMisc.toMap("partyId", partyId));
            delegator.removeByAnd("PartyContent", UtilMisc.toMap("partyId", partyId));
            ...
            delegator.removeByAnd("ProductStoreRole", UtilMisc.toMap("partyId", partyId));
            delegator.removeByAnd("PartyRole", UtilMisc.toMap("partyId", partyId));
            
            delegator.removeByAnd("PartyDataSource", UtilMisc.toMap("partyId", partyId));
            
            delegator.removeByAnd("ShoppingList", UtilMisc.toMap("partyId", partyId));
            delegator.removeByAnd("Party", UtilMisc.toMap("partyId", partyId));
            
        } catch (GenericEntityException e) {
            e.printStackTrace();
            return ServiceUtil.returnError("会员已经关联其他信息无法删除");
        }
...
//        }
        ...
        if (party != null) {
            ...
            /*add by gss 普通会员类型  企业会员或者个人会员 2016-4-7  */
            String partyCategory = (String) context.get("partyCategory");
            if (UtilValidate.isNotEmpty(partyCategory)) {
                newPartyMap.put("partyCategory", partyCategory);
            }
            /*add by gss 普通会员类型  企业会员或者个人会员   end 2016-4-7 */
        ...
        /*person.set("name",person.get("firstName")==null?"":(String)person.get("firstName")+
                person.get("middleName")==null?"":(String)person.get("middleName")+
                person.get("lastName")==null?"":(String)person.get("lastName"));*/
    ...
    public static Map<String, Object> findParty(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        if ("Y".equals(lookupFlag)) {
            ...
            if (!"Y".equals(showAll)) {
                ...
                //去除system,和_NA_
                andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("partyId"), EntityOperator.NOT_EQUAL, "system"));
                andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("partyId"), EntityOperator.NOT_EQUAL, "_NA_"));
                if (andExprs.size() > 0) {
     ...
     * @return
     */
    public static Map<String, Object> checkRegisterLoginId(DispatchContext dtx, Map<String, ? extends Object> context) {
        Map<String, Object> result = ServiceUtil.returnSuccess();
        String userLoginId = (String) context.get("USER_LOGIN_ID");
        ...
        try {
            ...
            if (UtilValidate.isNotEmpty(delegator.findList("UserLogin", condition, null, null, null, false))) {
                result.put("_PASSED_", false);
            } else {
                result.put("_PASSED_", true);
            }
        } catch (GenericEntityException e) {
            result.put("_PASSED_", false);
        }
    ...
    public static Map<String, Object> updateHotel(DispatchContext ctx, Map<String, ? extends Object> context) {
        Map<String, Object> result = FastMap.newInstance();
        Delegator delegator = ctx.getDelegator();
        LocalDispatcher dispatcher = ctx.getDispatcher();
        Locale locale = (Locale) context.get("locale");
        ...
        //营业时间
        String businessHours_c_hour = (String) context.get("businessHours_c_hour");
        String businessHours_c_minutes = (String) context.get("businessHours_c_minutes");
        String businessHours = businessHours_c_hour + ":" + businessHours_c_minutes;
        
        String businessHoursend_c_hour = (String) context.get("businessHoursend_c_hour");
        String businessHoursend_c_minutes = (String) context.get("businessHoursend_c_minutes");
        String businessHoursend = businessHoursend_c_hour + ":" + businessHoursend_c_minutes;
        //接受预约时间
        String matime_c_hour = (String) context.get("matime_c_hour");
        String matime_c_minutes = (String) context.get("matime_c_minutes");
        String matime = matime_c_hour + ":" + matime_c_minutes;
        
        String matimeend_c_hour = (String) context.get("matimeend_c_hour");
        String matimeend_c_minutes = (String) context.get("matimeend_c_minutes");
        String matimeend = matimeend_c_hour + ":" + matimeend_c_minutes;
        
        String cuisine = (String) context.get("cuisine");
        String banquetTag = (String) context.get("banquetTag");
        String serviceTag = (String) context.get("serviceTag");
        ...
        GenericValue partyGroup = null;
        GenericValue party = null;
        GenericValue partyAdditions = null;
        
        try {
            partyGroup = delegator.findByPrimaryKey("PartyGroup", UtilMisc.toMap("partyId", partyId));
            party = delegator.findByPrimaryKey("Party", UtilMisc.toMap("partyId", partyId));
            partyAdditions = delegator.findByPrimaryKey("PartyAdditions", UtilMisc.toMap("partyId", partyId));
        } catch (GenericEntityException e) {
            Debug.logWarning(e, module);
            return ServiceUtil.returnError(UtilProperties.getMessage(resourceError,
                    "partyservices.could_not_update_party_information_read",
        ...
        if (partyGroup == null || party == null || partyAdditions == null) {
            return ServiceUtil.returnError(UtilProperties.getMessage(resourceError,
                    "partyservices.could_not_update_party_information_not_found", locale));
        }
        ...
        String oldStatusId = party.getString("statusId");
        partyGroup.setNonPKFields(context);
        partyGroup.set("groupName", context.get("hotelname"));
        party.setNonPKFields(context);
        party.set("statusId", oldStatusId);
        partyAdditions.setNonPKFields(context);
        partyAdditions.set("businessHours", businessHours);
        partyAdditions.set("businessHoursend", businessHoursend);
        partyAdditions.set("matime", matime);
        partyAdditions.set("matimeend", matimeend);
        //   partyAdditions.set("cuisine",cuisine);
        partyAdditions.set("banquetTag", banquetTag);
        partyAdditions.set("serviceTag", serviceTag);
        
        
        try {
            partyGroup.store();
            party.store();
            partyAdditions.store();
        } catch (GenericEntityException e) {
            Debug.logWarning(e.getMessage(), module);
            return ServiceUtil.returnError(UtilProperties.getMessage(resourceError,
                    "partyservices.could_not_update_party_information_write",
    ...
    public static Map<String, Object> createHotel(DispatchContext ctx, Map<String, ? extends Object> context) {
        Map<String, Object> result = FastMap.newInstance();
        Delegator delegator = ctx.getDelegator();
        GenericValue userLogin = (GenericValue) context.get("userLogin");
        Timestamp now = UtilDateTime.nowTimestamp();
        ...
        String businessHoursend_c_minutes = (String) context.get("businessHoursend_c_minutes");
        String businessHoursend = businessHoursend_c_hour + ":" + businessHoursend_c_minutes;
        SimpleDateFormat hoursFormat = new SimpleDateFormat("yyyy-mm-dd hh:mm:ss[.ffffffff]");
        ...
        try {
            ...
            GenericValue hotel = delegator.findByPrimaryKey("PartyAdditions", UtilMisc.toMap("partyId", partyId));
            if (hotel != null) {
                return ServiceUtil.returnError(UtilProperties.getMessage(resourceError,
                        "partyservices.cannot_create_hotel_already_exists", locale));
            }

//            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd hh:mm");
//            java.sql.Timestamp authDate = null;
//            try {
//                authDate = new java.sql.Timestamp(sdf.parse(businessHours).getTime());
//            } catch (ParseException e) {
//                Debug.logError(e, "Cannot parse date string: " + businessHours, module);
//                authDate = UtilDateTime.nowTimestamp();
//            } catch (NullPointerException e) {
//                Debug.logError(e, "Cannot parse date string: " + businessHours, module);
//                authDate = UtilDateTime.nowTimestamp();
//            }
            hotel = delegator.makeValue("PartyAdditions", UtilMisc.toMap("partyId", partyId));
            hotel.setNonPKFields(context);
            hotel.set("businessHours", businessHours);
            hotel.set("businessHoursend", businessHoursend);
            hotel.set("matime", matime);
            hotel.set("matimeend", matimeend);
            //   hotel.set("cuisine",cuisine);
            hotel.set("banquetTag", banquetTag);
            hotel.set("serviceTag", serviceTag);
            hotel.create();
        } catch (GenericEntityException e) {
            Debug.logWarning(e, module);
            return ServiceUtil.returnError(UtilProperties.getMessage(resourceError,
    ...
    /**
     * 查询会员列表，弹出框使用   add by qianjin 2016/01/26
     *
     * @param dctx
     * @param context
     * @return
     */
    public static Map<String, Object> getPersonListForModal(DispatchContext dctx, Map<String, ? extends Object> context) {
        Map<String, Object> result = ServiceUtil.returnSuccess();
        Delegator delegator = dctx.getDelegator();
        //LocalDispatcher对象
        LocalDispatcher dispatcher = dctx.getDispatcher();
        ...
        dynamicView.addAlias("P", "partyId", "partyId", null, true, true, null);
        dynamicView.addAlias("P", "statusId");
        dynamicView.addAlias("P", "partyCategory");
        
        dynamicView.addMemberEntity("Per", "Person");
        dynamicView.addAlias("Per", "name");
        dynamicView.addAlias("Per", "mobile");
        dynamicView.addViewLink("P", "Per", false, ModelKeyMap.makeKeyMapList("partyId", "partyId"));
        ...
        if (UtilValidate.isNotEmpty(context.get("partyId"))) {
            andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("partyId"), EntityOperator.LIKE, EntityFunction.UPPER("%" + context.get("partyId") + "%")));
        }
        //按会员名称模糊查询
        if (UtilValidate.isNotEmpty(context.get("name"))) {
            andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("name"), EntityOperator.LIKE, EntityFunction.UPPER("%" + context.get("name") + "%")));
        }
        //按联系方式模糊查询
        if (UtilValidate.isNotEmpty(context.get("mobile"))) {
            andExprs.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("mobile"), EntityOperator.LIKE, EntityFunction.UPPER("%" + context.get("mobile") + "%")));
        }
        //按联系方式模糊查询
        if (UtilValidate.isNotEmpty(context.get("roleTypeId"))) {
            ...
            dynamicView.addAlias("PR", "roleTypeId");
//            fieldsToSelect.add("roleTypeId");
            dynamicView.addViewLink("P", "PR", false, ModelKeyMap.makeKeyMapList("partyId", "partyId"));
            andExprs.add(EntityCondition.makeCondition("roleTypeId", context.get("roleTypeId")));
        }
        //默认条件
        andExprs.add(EntityCondition.makeCondition("statusId", EntityOperator.EQUALS, "PARTY_ENABLED"));
        andExprs.add(EntityCondition.makeCondition("partyCategory", EntityOperator.IN, UtilMisc.toList("MEMBER", "COMPANY")));
    ...
    public static Map<String, Object> savePostalAddressOtherIsDefault(DispatchContext dctx, Map<String, ? extends Object> context) throws GenericEntityException {
        Map<String, Object> result = ServiceUtil.returnSuccess();
        Delegator delegator = dctx.getDelegator();
        //LocalDispatcher对象
        LocalDispatcher dispatcher = dctx.getDispatcher();
        String partyId = (String) context.get("partyId");
        String contactMechIdd = (String) context.get("contactMechId");
        GenericValue party = delegator.findByPrimaryKey("Party", UtilMisc.toMap("partyId", partyId));
        ...
        for (GenericValue scm : shippingContactMechList) {
            ...
            postalAddress.set("isDefault", "0");
            postalAddress.store();
        }
        if (UtilValidate.isNotEmpty(contactMechIdd)) {
            GenericValue postalAddress = delegator.findByPrimaryKey("PostalAddress", UtilMisc.toMap("contactMechId", contactMechIdd));
            postalAddress.set("isDefault", "1");
            postalAddress.store();
        }
        GenericValue defalutAddress = UtilValidate.isNotEmpty(delegator.findByAnd("PartyProfileDefault", UtilMisc.toMap("partyId", partyId))) ?
                delegator.findByAnd("PartyProfileDefault", UtilMisc.toMap("partyId", partyId)).get(0) : null;
        if (defalutAddress != null) {
            defalutAddress.set("defaultShipAddr", contactMechIdd);
            defalutAddress.store();
        } else {
            delegator.create("PartyProfileDefault", UtilMisc.toMap("partyId", partyId, "productStoreId", "10000", "defaultShipAddr", contactMechIdd));
        }
        return result;
    }
    
    public static Map<String, Object> saveDeleteDefault(DispatchContext dctx, Map<String, ? extends Object> context) throws GenericEntityException {
        Map<String, Object> result = ServiceUtil.returnSuccess();
        Delegator delegator = dctx.getDelegator();
        //LocalDispatcher对象
        LocalDispatcher dispatcher = dctx.getDispatcher();
        ...
        if (UtilValidate.isNotEmpty(shippingContactMechList)) {
            String contactMechIdd = (String) shippingContactMechList.get(0).get("contactMechId");
            for (GenericValue scm : shippingContactMechList) {
                String contactMechId = scm.getString("contactMechId");
                GenericValue postalAddress = delegator.findByPrimaryKey("PostalAddress", UtilMisc.toMap("contactMechId", contactMechId));
                postalAddress.set("isDefault", "0");
