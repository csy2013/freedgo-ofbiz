...
public class TaxAuthorityServices {
     ...
     * @param product
     * @param productStore
     * @param payToPartyId :付款给的partyId 卖家
     * @param billToPartyId：收票人的partyId 买家
     * @param taxAuthoritySet
     * @param itemPrice
     * @param itemQuantity
     * @param itemAmount
     * @param shippingAmount
     * @param orderPromotionsAmount
     * @return
     */
        ...
        try {
            ...
            for(GenericValue taxAuthorityRateProduct : filteredList) {
                ...
                if (productPrice != null && itemQuantity != null && 
                    ...
                    // not be shorted)
                    //增值税税收,如果计算项目的总价格加税不同于预期的计算基于最初的输入价格和税收(如果输入的价格是这样),
//                    然后创建一个调整,纠正差异,这种调整将有效减去价格而不是税收(税收是为了计算基于税务机关规则,所以不应做空)
