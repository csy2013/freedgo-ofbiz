...
public class CommonEvents {
    ...
    static final String[] ignoreAttrs = new String[]{ // Removed for security reason; _ERROR_MESSAGE_ is kept
            ...
            "_SERVER_ROOT_URL_",
            "_CONTROL_PATH_",
            "thisRequestUri", "jsonpCallbackFunction"
    };
    ...
    /**
     * add by changsy 2016.6.2 手机端使用
     *resultDataSkipRequestGetMethod 表示可以使用Get method
     * @param request
     * @param response
     * @return
     */
    public static String appJsonResponseFromRequestAttributes(HttpServletRequest request, HttpServletResponse response) {
        // pull out the service response from the request attribute
        Map<String, Object> attrMap = UtilHttp.getAppJSONAttributeMap(request);
...
//        System.out.println("attrMap =========== " + attrMap);
        ...
        boolean ignoreGetMethod = false;
        if(attrMap.get("resultDataSkipRequestGetMethod")!=null && attrMap.get("resultDataSkipRequestGetMethod").equals("true")){
            ignoreGetMethod = true;
        }
        writeJSONStrtoResponse(gson.toJson(attrMap, Map.class), request.getMethod(), response,ignoreGetMethod);
        
        return "success";
    }
    
    /**
     * add by changsy 2016.8.29 手机端使用
     *resultDataSkipRequestGetMethod 表示可以使用Get method
     * @param request
     * @param response
     * @return
     */
    public static String djResponseFromRequestAttributes(HttpServletRequest request, HttpServletResponse response) {
        // pull out the service response from the request attribute
        Object attrMap = UtilHttp.getDjJSONAttributeMap(request);
       ...
       /* JsonConfig jsonConfig = new JsonConfig();
        ...
        Gson gson = new GsonBuilder().registerTypeAdapter(java.util.Date.class, new UtilDateSerializer())
                .setDateFormat(DateFormat.LONG).create();
        boolean ignoreGetMethod = false;
        writeJSONStrtoResponse(gson.toJson(attrMap), request.getMethod(), response,callbackFuntion);
//        writeJSONStrtoResponse(gson.toJson(attrMap),request.getMethod(),response);
        
        return "success";
    }
    
    /**
     * add by changsy 2015.8.24
     *
     * @param request
     * @param response
     * @return
     */
    public static String jsonpResponseFromRequestAttributes(HttpServletRequest request, HttpServletResponse response) {
        // pull out the service response from the request attribute
        Map<String, Object> attrMap = UtilHttp.getJSONAttributeMap(request);
//        String callbackFuntion = (String) attrMap.get("jsonpCallbackFunction");
        String callbackFuntion = request.getParameter("callback");
        for (String ignoreAttr : ignoreAttrs) {
            if (attrMap.containsKey(ignoreAttr)) {
                attrMap.remove(ignoreAttr);
        ...
        // create a JSON Object for return 注意此处date 要转换，
//        JsonConfig jsonConfig = new JsonConfig();
//        jsonConfig.registerJsonBeanProcessor(java.sql.Date.class, new JsDateJsonBeanProcessor());
//        JSONObject json = JSONObject.fromObject(attrMap, jsonConfig);
        ...
        Gson gson = new GsonBuilder().registerTypeAdapter(java.util.Date.class, new UtilDateSerializer())
                .setDateFormat(DateFormat.LONG).create();
        writeJSONStrtoResponse(gson.toJson(attrMap, Map.class), request.getMethod(), response, callbackFuntion);
        return "success";
    }
    
    public static String jsonpResponseNotAuthorized(HttpServletRequest request, HttpServletResponse response) {
        //notAuthenticated
        response.setStatus(403);
        return "success";
    }
    
    public static String jsonpResponseNotAuthenticated(HttpServletRequest request, HttpServletResponse response) {
        //notAuthenticated
...
//        response.setContentType("application/x-json");
    ...
    private static void writeJSONStrtoResponse(String jsonStr, String httpMethod, HttpServletResponse response,boolean ignoreGetMethod) {
    ...
    private static void     writeJSONStrtoResponse(String jsonStr, String httpMethod, HttpServletResponse response) {
    ...
    private static void writeJSONStrtoResponse(String jsonStr, String httpMethod, HttpServletResponse response, String callBackFuntion) {
        if (jsonStr == null) {
            Debug.logError("JSON Object was empty; fatal error!", module);
            return;
        }
...
//            jsonStr = "//" + jsonStr;
        ...
        // set the X-JSON content type
        jsonStr = callBackFuntion + "(" + jsonStr + ")";
    ...
    /**
     * add josnp by changsy 2015.8.24
     *
     * @param json
     * @param httpMethod
     * @param response
     */
    private static void writeJSONPtoResponse(JSON json, String httpMethod, HttpServletResponse response, String callBackFuntion) {
        String jsonStr = json.toString();
        if (jsonStr == null) {
            Debug.logError("JSON Object was empty; fatal error!", module);
            return;
...
//            jsonStr = "//" + jsonStr;
    ...
    public static String getCaptcha(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            final String captchaCodeId = StringUtils.defaultIfEmpty(request.getParameter("captchaCodeId"), "REG"); // this is used to uniquely identify in the user session the attribute where the captcha code for the last captcha for the form is stored
            final String length = StringUtils.defaultIfEmpty(request.getParameter("l"), "4");
            ...
            String captchaCode = RandomStringUtils.random(Integer.parseInt(length), availableChars);
    ...
    public static class UtilDateSerializer implements JsonSerializer<Date> {
        ...
        public JsonElement serialize(Date src, Type typeOfSrc,
            ...
            if(src instanceof java.sql.Date) {
                src = new java.util.Date(src.getTime());
            }
            Calendar c = Calendar.getInstance();
            c.setTime(src);
            JsonObject jsonObject = new JsonObject();
            jsonObject.addProperty("year",c.get(1));
            jsonObject.addProperty("month",c.get(2));
            jsonObject.addProperty("day",c.get(5));
            jsonObject.addProperty("hours",c.get(11));
            jsonObject.addProperty("minutes",c.get(12));
            jsonObject.addProperty("seconds",c.get(13));
            jsonObject.addProperty("milliseconds",c.get(14));
            return jsonObject;
        }
