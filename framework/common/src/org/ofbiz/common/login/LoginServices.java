...
public class LoginServices {
     ...
     * isSSO 如果是单点登录方式为true，密码为加密后的密文，只需要判断是否相等。
     */
    public static Map<String, Object> userLogin(DispatchContext ctx, Map<String, ?> context) {
        ...
        Boolean isSSO = (Boolean) context.get("isSSO");
        if(isSSO == null) isSSO = false;
            ...
            while (repeat) {
                ...
                if (userLogin != null) {
                    ...
                    if (UtilValidate.isEmpty(userLogin.getString("enabled")) || "Y".equals(userLogin.getString("enabled")) ||
                        ...
                        if ((!authFatalError && externalAuth) || checkPassword(userLogin.getString("currentPassword"), useEncryption, password,isSSO)) {
                            ...
                            //记录用户最后登录时间
                            userLogin.set("lastLoginTime",UtilDateTime.nowTimestamp());
    ...
    /** Updates UserLogin Password info 当有currentPassword 需要检查
     *@param ctx The DispatchContext that this service is operating in
     *@param context Map containing the input parameters
     *@return Map with the result of the service, the output parameters
     */
    ...
    public static void checkNewPassword(GenericValue userLogin, String currentPassword, String newPassword, String newPasswordVerify, String passwordHint, List<String> errorMessageList, boolean ignoreCurrentPassword, Locale locale) {
        ...
        String errMsg = null;
//        当前currentPassword有传值, 或者忽列检查
        if (!ignoreCurrentPassword && currentPassword != null) {
            // if the password.accept.encrypted.and.plain property in security is set to true allow plain or encrypted passwords
            // if this is a system account don't bother checking the passwords
            boolean passwordMatches = checkPassword(userLogin.getString("currentPassword"), useEncryption, currentPassword,false);
            ...
            if (checkPassword(userLogin.getString("currentPassword"), useEncryption, newPassword,false)) {
    ...
    private static boolean checkPassword(String oldPassword, boolean useEncryption, String currentPassword,Boolean isSSO) {
        boolean passwordMatches = false;
        if (oldPassword != null) {
            if (useEncryption && (!isSSO)) {
