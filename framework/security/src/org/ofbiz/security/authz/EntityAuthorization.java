...
import org.ofbiz.base.util.UtilRedis;
...
public class EntityAuthorization extends AbstractAuthorization {
    ...
    /**
     * redis有效时间，默认2小时，单位毫秒
     */
    private static final long REDIS_EXPIRE = 1000 * 60 * 60 * 2;
    ...
    public List<String> getAutoGrantPermissions(String userId, String permission, Map<String, ? extends Object> context) {
        ...
        while (checking) {
            if (Debug.verboseOn()) {
                Debug.logVerbose("Looking for auto-grant permissions for : " + checkString, module);
            }
    ...
    public boolean hasDynamicPermission(String userId, String permission, Map<String, ? extends Object> context) {
        ...
        while (checking) {
            if (Debug.verboseOn()) {
                Debug.logVerbose("Looking for dynamic access for permission -- " + permissionId, module);
            }
            ...
            if (UtilValidate.isEmpty(dynamicAccess)) {
                ...
                if (Debug.verboseOn()) {
                    Debug.logVerbose("Dynamic access implementation found : " + dynamicAccess, module);
                }
                checking = false;
            }
        }
        ...
        if (UtilValidate.isNotEmpty(dynamicAccess)) {
            ...
            if (Debug.verboseOn()) {
                Debug.logVerbose("Loading DynamicAccessHandler for -- " + dynamicAccess, module);
            }
            DynamicAccessHandler dah = DynamicAccessFactory.getDynamicAccessHandler(delegator, dynamicAccess);
            if (dah != null) {
                if (Debug.verboseOn()) {
                    Debug.logVerbose("Calling DynamicAccessHandler : " + dah.getClass().getName(), module);
                }
    ...
    public boolean hasStaticPermission(String userId, String permission, Map<String, ? extends Object> context) {
        ...
        Iterator<Map<String,Object>> iterator = getUserLoginSecurityGroupByUserLoginId(userId);
        Map<String,Object> userLoginSecurityGroup = null;

        while (iterator.hasNext()) {
            userLoginSecurityGroup = iterator.next();
            if (securityGroupHasPermission(userLoginSecurityGroup.get("groupId").toString(), permission)) {
                return true;
            }
        }
        return false;
    ...
    private Iterator<Map<String,Object>> getUserLoginSecurityGroupByUserLoginId(String userId) {
        List collection;
        String redisKey = "UserLoginSecurityGroup" + "_" + userId;
        try {
//            if(UtilRedis.exists(redisKey)){
//                collection = UtilRedis.getListEntity(redisKey,Map.class);
//            }else {
                collection = delegator.findByAnd("UserLoginSecurityGroup", UtilMisc.toMap("userLoginId", userId), null);
//                UtilRedis.setList(redisKey,collection);
//            }
    ...
    private boolean securityGroupHasPermission(String groupId, String permission) {
        String redisKey = "SecurityGroupPermission" + "_" + groupId + "_" + permission;
        /*if (UtilRedis.exists(redisKey)){
            return (boolean)UtilRedis.get(redisKey);
        }else {*/
            ...
            try {
                GenericValue genericValue = delegator.findOne(securityGroupPermissionValue.getEntityName(), securityGroupPermissionValue, false);
                boolean result = genericValue != null;
//                UtilRedis.set(redisKey,result,REDIS_EXPIRE);
                return result;
            } catch (GenericEntityException e) {
                Debug.logWarning(e, module);
                return false;
...
//        JedisCache cache = delegator.getCache();
        ...
        String redisKey = "SecurityPermissionAutoGrant" + "_" + permission;
        List<String> autoGrants = FastList.newInstance();
        if(UtilRedis.exists(redisKey)){
           autoGrants = UtilRedis.getListString(redisKey);
        }else {
            ...
            UtilRedis.set(redisKey,autoGrants,REDIS_EXPIRE);
        }
        return autoGrants;
    }
    
    private String getPermissionDynamicAccess(String perm) {
        String redisKey = "SecurityPermission" + "_" + perm;
        if (UtilRedis.exists(redisKey)){
            return UtilRedis.get(redisKey).toString();
        }else{
            ...
            if (permission != null) {
                UtilRedis.set(redisKey,permission.getString("dynamicAccess"),REDIS_EXPIRE);
