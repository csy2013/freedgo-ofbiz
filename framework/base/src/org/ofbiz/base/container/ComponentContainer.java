...
public class ComponentContainer implements Container {
    ...
    public synchronized void loadComponents(String loaderConfig, boolean updateClasspath) throws ComponentException {
        loadComponents(loaderConfig, updateClasspath, null, null);
    }

    public synchronized void loadComponents(String loaderConfig, boolean updateClasspath, String instrumenterClassName, String instrumenterFile) throws ComponentException {
...
//            System.out.println("classPath = " + classPath);
    ...
    private void loadComponentFromConfig(String parentPath, ComponentLoaderConfig.ComponentDef def) {
        ...
        if (def.type == ComponentLoaderConfig.SINGLE_COMPONENT) {
            ...
            try {
                if(def.useWebapps.equals("")){
                    config = ComponentConfig.getComponentConfig(def.name, location,"true");
                }else {
                    config = ComponentConfig.getComponentConfig(def.name, location, def.useWebapps);
                }
            ...
            this.loadComponentDirectory(location,def.useWebapps);
        }
    }


    private void loadComponentFromConfig(String parentPath, ComponentLoaderConfig.ComponentDef def,String useWebapps) {
        String location;
        if (def.location.startsWith("/")) {
            location = def.location;
        } else {
        ...
        if (def.type == ComponentLoaderConfig.SINGLE_COMPONENT) {
            ...
            try {
                if(def.useWebapps.equals("")){
                    if(useWebapps.equals("")){
                        config = ComponentConfig.getComponentConfig(def.name, location,"true");
                    }else {
                        config = ComponentConfig.getComponentConfig(def.name, location, useWebapps);
                    }
                }else {
                    config = ComponentConfig.getComponentConfig(def.name, location, def.useWebapps);
                }
    ...
    private void loadComponentDirectory(String directoryName,String useWebapps) {
                ...
                try {
                    ...
                    if (componentsToLoad != null) {
                        for (ComponentLoaderConfig.ComponentDef def: componentsToLoad) {
                            this.loadComponentFromConfig(parentPath.toString(), def,useWebapps);
                        }
                    }
                } catch (MalformedURLException e) {
                        ...
                        if (componentPath.isDirectory() && !sub.equals("CVS") && !sub.equals(".svn")) {
                            ...
                            if (configFile.exists()) {
                                ...
                                try {
                                    ...
                                    if(useWebapps.equals("")) useWebapps ="true";
                                    config = ComponentConfig.getComponentConfig(null, componentLocation,useWebapps);
                                } catch (ComponentException e) {
     ...
     */
    public static synchronized void loadComponents(boolean updateClasspath) throws ComponentException {
