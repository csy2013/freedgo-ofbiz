...
public class UtilHttp {
    ...
    /**
     * add by changsy 2014.12.24 处理ofbiz在session保存parameterMap中含有中文时乱码的问题
     * @param request
     * @return
     */
    ...
    public static Map<String, Object> getAppJSONAttributeMap(HttpServletRequest request) {
        Map<String, Object> returnMap = FastMap.newInstance();
        Map<String, Object> attrMap = getAttributeMap(request);
        for (Map.Entry<String, Object> entry : attrMap.entrySet()) {
            String key = entry.getKey();
            Object val = entry.getValue();
            if (key.equals("resultData")) {
                returnMap.put(key, val);
            }
            if(key.equals("resultDataSkipRequestGetMethod") && val.equals("true")){
                returnMap.put("resultDataSkipRequestGetMethod", "true");
            }
        }

        return returnMap;
    }
    public static Object getDjJSONAttributeMap(HttpServletRequest request) {
        Map<String, Object> attrMap = getAttributeMap(request);
        for (Map.Entry<String, Object> entry : attrMap.entrySet()) {
            String key = entry.getKey();
            Object val = entry.getValue();
...
//        if(!contextPath.equals(""))requestUrl.append("/"+contextPath);
//        if(pathInfo!=null && (!pathInfo.equals("")))requestUrl.append("/"+pathInfo);
...
//        }
    ...
    public static void streamContentToBrowser2(HttpServletResponse response, InputStream in, int length, String contentType, String fileName) throws IOException {
        // tell the browser not the cache
        setResponseBrowserProxyNoCache(response);
...
//            response.setHeader("Content-Disposition", "attachment;filename=" + fileName);
    ...
    public static void streamContentToBrowser1(HttpServletResponse response, InputStream in, int length, String contentType, String fileName) throws IOException {
        // tell the browser not the cache
        setResponseBrowserProxyNoCache(response);
    ...
    public static Object makeParamValueFromComposite(HttpServletRequest request, String prefix, Locale locale) {
        ...
        if ("Timestamp".equals(compositeType)) {
            ...
            try {
                ...
                Time time = new Time(h,Integer.parseInt(minutes),0);
                return time;
            } catch (IllegalArgumentException e) {
                Debug.logWarning("User input for composite timestamp was invalid: " + e.getMessage(), module);
                return null;
    ...
    public static String getParameterByName(HttpServletRequest request,String name ,String defaultValue){
        String value = request.getParameter(name);
    ...
    /**
     * 获取tenant Id 形式为:www_alifind_cn,www_changsy_cn。
     *
     * @param request
     * @return
     */
    public static String getTenantByURL(HttpServletRequest request) {
        String rootUrl = UtilHttp.getServerRootUrl(request).toString();
        String domain = "";
        if (request.getServerPort() != 80 && request.getServerPort() != 443 ) {
            domain = rootUrl.substring(rootUrl.indexOf("://") + 3, rootUrl.lastIndexOf(":"));
        } else {
            domain = rootUrl.substring(rootUrl.indexOf("://") + 3);
        }
        String regex = "([1-9]|[1-9]\\d|1\\d{2}|2[0-4]\\d|25[0-5])(\\.(\\d|[1-9]\\d|1\\d{2}|2[0-4]\\d|25[0-5])){3}";
        if (!domain.matches(regex) && (!domain.equalsIgnoreCase("localhost")) && (!domain.endsWith("yuaoq.com"))) {
            domain = StringUtil.replaceString(domain, ".", "_");
    ...
    /**
     * //add by changsy delegator 默认是request.attribute("delegator') ,如果不是 选择 1、session, 3、从web.xml的entityDelegatorName、4、从url的http 请求 5、取默认
     *
     * @param request
     * @return
     */
    public static String getDelegatorNameByRequest(HttpServletRequest request) {
        
        String delegatorName = (String) request.getSession().getAttribute("delegatorName");
        if (UtilValidate.isNotEmpty(delegatorName)) {
            return delegatorName;
        } else {
            delegatorName = request.getServletContext().getInitParameter("entityDelegatorName");
            if (UtilValidate.isNotEmpty(delegatorName)) {
                return delegatorName;
            } else {
                delegatorName = getTenantByURL(request);
                if (!delegatorName.equals("127.0.0.1") && (!delegatorName.equalsIgnoreCase("localhost"))) {
                    return delegatorName;
                } else {
                    return "default";
                }
