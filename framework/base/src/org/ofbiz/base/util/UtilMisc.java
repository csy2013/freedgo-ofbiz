...
public class UtilMisc {
    ...
    public static <K, V> Map<String, V> toMap(Object... data) {
        ...
        if (data.length == 1 && data[0] instanceof Map) {
            ...
            return UtilGenerics.checkMap(data[0]);
        }
        if (data.length % 2 == 1) {
    ...
    public static <K, IK, V> Map<IK, V> getMapFromMap(Map<K, Object> outerMap, K key) {
        Map<IK, V> innerMap = UtilGenerics.checkMap(outerMap.get(key));
        if (innerMap == null) {
    ...
    public static <K, V> List<V> getListFromMap(Map<K, Object> outerMap, K key) {
        List<V> innerList = UtilGenerics.checkList(outerMap.get(key));
        if (innerList == null) {
    ...
    protected static class SimpleMap<V> implements Map<String, V>, Serializable {
        ...
        public boolean equals(Object obj) {
            if (realMapIfNeeded != null) {
                ...
                Map<String, V> mapObj = UtilGenerics.checkMap(obj);
    ...
    /**
     * 判断如果是整数，则不保留小数，如果是浮点数，则保留2位小数 add by qianjin 2016.4.27
     * @param d
     * @return
     */
