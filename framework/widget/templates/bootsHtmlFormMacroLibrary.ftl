...
<select name="${name?default("")}<#rt/>" class="form-control input-sm" <#if id?has_content> id="${id}"</#if><#if multiple?has_content> multiple="multiple"</#if><#if otherFieldSize gt 0>
        onchange="process_choice(this,document.${formName}.${otherFieldName})"</#if><#if event?has_content> ${event}="${action}"</#if><#if size?has_content> size="${size}"</#if> <#if required =='true'> data-parsley-required="true"</#if>>
    ...
    <#if listSize gt viewSize>
        <div class=" clearfix">&nbsp;
            ...
            ${paginateViewSizeLabel}:
                <select name="pageSize" size="1" class="nav-pagesize  input-sm"
...
<#macro makeHyperlinkString linkStyle hiddenFormName event action imgSrc title alternate linkUrl targetWindow description confirmation imgStyle>
    ...
    <#if imgSrc?length == 0 && title?has_content> title="${title}"</#if>><#if imgSrc?has_content><img src="${imgSrc}" alt="${alternate}" title="${title}" <#if imgStyle?has_content>class="${imgStyle}"</#if>/></#if>${description}</a>
</#macro>
