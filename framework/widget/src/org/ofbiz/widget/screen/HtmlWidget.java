    ...
    public static void renderHtmlTemplate(Appendable writer, FlexibleStringExpander locationExdr, Map<String, Object> context) {
        ...
        if (location.endsWith(".ftl")) {
            try {
                ...
                //增加扩展theme，extend支持  如：xxx_amaze.ftl
                ...
                /*if(UtilValidate.isNotEmpty(themeId) && UtilStrings.contains(themeIds, themeId)){
                    try {
                        String themeType = themeId.toLowerCase();
                        if (location.endsWith(".fo.ftl")) { // FOP can't render correctly escaped characters
                            String location1 = location.substring(0, location.indexOf(".fo.ftl")) + "_" + themeType + ".fo.ftl";
                            template = FreeMarkerWorker.getTemplate(location1);
                        } else {
                            String location1 = location.substring(0, location.indexOf(".ftl")) + "_" + themeType + ".ftl";
                            template = FreeMarkerWorker.getTemplate(location1, specialTemplateCache, specialConfig);
                        }
                    }catch (Exception e){
                        if (location.endsWith(".fo.ftl")) { // FOP can't render correctly escaped characters
                ...
                sw.start("渲染ftl:"+template.getName());*/
                FreeMarkerWorker.renderTemplate(template, context, writer);
                /*sw.stop();
                Debug.log(sw.prettyPrint(),module);*/
                if (insertWidgetBoundaryComments) {
...
//            stopWatch.start("Html 渲染执行时间:"+this.modelScreen.getName());
            renderHtmlTemplate(writer, this.locationExdr, context);
//            stopWatch.stop();
//            Debug.logInfo(stopWatch.prettyPrint(),module);
        }
