...
//        Debug.log("接受请求时间:"+UtilDateTime.nowDateString()+"|"+UtilDateTime.nowAsString());
       /* StopWatch sw = new StopWatch();
        sw.start("服务端接受开始请求开始");*/
        ...
        if (chain != null) {
            ...
            // Invoke the pre-processor (but NOT in a chain) 处理预登录，包括externalLoginKey方式
        ...
        //single productstore， so productstoreId is one 这块处理对应的B-B-C业务关系
        try {
            userLogin = (GenericValue) request.getSession().getAttribute("userLogin");
            if(UtilValidate.isNotEmpty(userLogin)) {
                String partyId = (String) userLogin.get("partyId");
                List<GenericValue> relations = delegator.findByAndCache("PartyRelationship", UtilMisc.toMap("partyIdTo", partyId, "roleTypeIdTo", "EMPLOYEE"));
                if (UtilValidate.isNotEmpty(relations)) {
                    String primaryPId = (String) relations.get(0).get("partyIdFrom");
                    GenericValue partyGroup = delegator.findByPrimaryKeyCache("PartyGroup", "partyId", primaryPId);
                    if(UtilValidate.isNotEmpty(partyGroup)) {
                        request.setAttribute("productStoreId", partyGroup.get("productStoreId"));
                        request.setAttribute("ownerPartyId", primaryPId);
                        request.setAttribute("isInner", partyGroup.get("isInner"));
                    }else{
                        throw new RequestHandlerException("对应的商户不存在");
                    }
                }
            }
        } catch (GenericEntityException e) {
        ...
        if ("request".equals(nextRequestResponse.type)) {
            ...
            if ("url".equals(nextRequestResponse.type)) {
                ...
                if (session.getAttribute("_SAVED_VIEW_NAME_") != null) {
                    ...
                    urlParams = UtilGenerics.checkMap(session.getAttribute("_SAVED_VIEW_PARAMS_"));
                } else if (session.getAttribute("_HOME_VIEW_NAME_") != null) {
                    viewName = (String) session.getAttribute("_HOME_VIEW_NAME_");
                    urlParams = UtilGenerics.checkMap(session.getAttribute("_HOME_VIEW_PARAMS_"));
...
//            此处设置extend为空
            ...
            String[] themeIds = UtilProperties.getPropertyValues(url, "widget.themeId.extends");
    ...
    public String makeLink(HttpServletRequest request, HttpServletResponse response, String url, boolean fullPath, boolean secure, boolean encode) {
        ...
        String encodedUrl;
        if (encode) {
            boolean forceManualJsessionid = "false".equals(getServletContext().getInitParameter("cookies"));
