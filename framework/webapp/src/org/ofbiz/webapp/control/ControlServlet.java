...
public class ControlServlet extends HttpServlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        super.init(config);
        if (Debug.infoOn())
            Debug.logInfo("LOADING WEBAPP [" + config.getServletContext().getContextPath().substring(1) + "] " + config.getServletContext().getServletContextName() + ", located at " + config.getServletContext().getRealPath("/"), module);
...
//        UtilHttp.setLocale(request, "zh_CN");
//        request.setAttribute("locale","zh_CN");
        ...
        //tenentId delegator
        if (delegator == null) {
            delegatorName = UtilHttp.getTenantByURL(request);
            delegator = DelegatorFactory.getDelegator(delegatorName);
        }
        if (delegator == null) {
        ...
        //判断是application/x-www-form-urlencoded 还是 request payload
        String conType = request.getHeader("Content-Type");
        if (conType != null && !conType.startsWith("application/x-www-form-urlencoded") && (!conType.startsWith("multipart/form-data"))) {
            ...
            try {
                ...
                Debug.logError("get Request Payload error: " + ex.getMessage(), module);
            } finally {
            ...
            Object  payLoadMap = null;
            if (UtilValidate.isNotEmpty(stringBuilder)) {
                try {
                    payLoadMap = new Gson().fromJson(stringBuilder.toString(), Object.class);
                } catch (Exception e) {
                    e.printStackTrace();
                }
                /*if (payLoadMap != null) {
                    Set keySet = payLoadMap.keySet();
                    Iterator iterator = keySet.iterator();
                    while (iterator.hasNext()) {
                        String key = (String) iterator.next();
                        Object val = payLoadMap.get(key);
                        request.setAttribute(key, val);
                    }
                }*/
            request.setAttribute("payLoadObj", payLoadMap);
            }
        }
