...
public class ServiceEventHandler implements EventHandler {
    ...
    public String invoke(Event event, RequestMap requestMap, HttpServletRequest request, HttpServletResponse response) throws EventHandlerException {
        ...
        //add by userlogin 可以从request.getAttribute()中取
        if(userLogin==null){
            userLogin =  (GenericValue)request.getAttribute("userLogin");
        }
        //add by changsy 20150831 in json service must know webSiteId, productStoreId ,so add them when params not have.
        //WebSiteWorker.getWebSiteId(),ProductStoreWorker.

        String webSiteId = null;
        String productStoreId = (String) request.getAttribute("productStoreId");
        String ownerPartyId = (String) request.getAttribute("ownerPartyId");
        String isInner = (String) request.getAttribute("isInner");
        /*ServletContext application = ((ServletContext) request.getAttribute("servletContext"));
        if (application != null) {
            webSiteId = application.getInitParameter("webSiteId");
        }
        ...
        if (session != null && session.getAttribute("productStoreId") != null) {
            ...
            try {
                webSite = dispatcher.getDelegator().findOne("WebSite", true, UtilMisc.toMap("webSiteId", webSiteId));
        ...
        if (isMultiPart) {
            ...
            try {
                uploadedItems = UtilGenerics.checkList(upload.parseRequest(request));
        ...
        //request payload 此处解决通过request payLoad 方式 获取json 数据
     /*   StringBuilder stringBuilder = new StringBuilder();
        ...
        try {
            ...
            throw new EventHandlerException("get Request Payload error: "+ ex.getMessage());
        } finally {
        ...
        Map<String,Object> payLoadMap = FastMap.newInstance();
        if(UtilValidate.isNotEmpty(stringBuilder)){
            payLoadMap = new Gson().fromJson(stringBuilder.toString(),Map.class);
//            request.setAttribute("payLoadMap", payLoadMap);
        ...
        for (ModelParam modelParam: model.getInModelParamList()) {
            ...
            if (UtilValidate.isNotEmpty(modelParam.stringMapPrefix)) {
              ...
              /*  // check the request payload
                ...
                if("localeString".equals(name) && value instanceof Locale){
                    value = "zh_CN";
                }
            }
           
            if(ownerPartyId!=null && name.equals("ownerPartyId")){
                serviceContext.put("ownerPartyId", ownerPartyId);
            }
            else if(isInner!=null && name.equals("isInner")){
                serviceContext.put("isInner", isInner);
            }else{
        ...
        //add by changsy add webSiteId, productStroeId to service,ownerPartyId,isIner
        if(webSiteId!=null && serviceContext.get("webSiteId")==null){
            serviceContext.put("webSiteId", webSiteId);
        }
        if(productStoreId!=null && serviceContext.get("productStoreId")==null){
            serviceContext.put("productStoreId", productStoreId);
        }
        
        //end by changsy add webSiteId, productStroeId to service
