...
/**
 * Created by changsy on 2015/4/13.
 */
...

{
    ...
    public static final String J2EE_SERVER = "Yabiz Container 3.1";
    public static final String J2EE_APP = "Yabiz";
    ...
    protected ContainerConfig.Container.Property sessionManager = null;
    ...
    public void init(String[] args, String configFile) throws ContainerException {
        ...
        //增加ARP Listener product on
//        AprLifecycleListener listener = new AprLifecycleListener();
//        listener.setSSLEngine("off"); //y
//        server.addLifecycleListener(listener);
        
        // 增加对tomcat的性能优化部分
        List<ContainerConfig.Container.Property> executorProps = cc.getPropertiesWithValue("executor");
        if (!UtilValidate.isEmpty(executorProps)) {
            for (ContainerConfig.Container.Property executorProp : executorProps) {
                createExecutor(executorProp);
            }
        }
        ...
        ContainerConfig.Container.Property sessionManagerProp = cc.getProperty("sessionManager");
        if (UtilValidate.isNotEmpty(sessionManagerProp)) {
            sessionManager = sessionManagerProp;
        }
    ...
    protected Connector createConnector(ContainerConfig.Container.Property connectorProp) throws ContainerException {
        ...
        if (UtilValidate.isNotEmpty(connectorProp.properties)) {
            ...
            try {
                ...
                } else if (protocol.equals("http11")) {
            ...
            try {
                for (ContainerConfig.Container.Property prop : connectorProp.properties.values()) {
                    ...
                    //增加executor
                    if (prop.name.equals("executor")) {
                        Method m = IntrospectionUtils.findMethod(connector.getProtocolHandler().getClass(), "setExecutor", new Class[]{java.util.concurrent.Executor.class});
                        if (m != null) {
                            Executor ex = tomcat.getService().getExecutor(prop.value);
                            m.invoke(connector.getProtocolHandler(), ex);
                        } else {
                            Debug.logWarning("Connector [" + connector + "] does not support external executors. Method setExecutor(java.util.concurrent.Executor) not found.", module);
                        }
                    }
                }
    ...
    private StandardContext configureContext(Engine engine, Host host, ComponentConfig.WebappInfo appInfo) throws ContainerException {
        ...
        ContainerConfig.Container.Property clusterProp = clusterConfig.get(engine.getName());
        ...
        if (clusterProp != null && contextIsDistributable) {
            ...
            if (UtilValidate.isNotEmpty(sessionManager)) {
                String cacheType = sessionManager.value;
                if (cacheType.equals("redis")) {
                    RedisSessionHandlerValve value = new RedisSessionHandlerValve();
                    context.getPipeline().addValve(value);
                    RedisSessionManager redisManager = new RedisSessionManager();
                    redisManager.setHost(sessionManager.getProperty("host").value);
                    redisManager.setDatabase(Integer.parseInt(sessionManager.getProperty("database").value));
                    redisManager.setPort(Integer.parseInt(sessionManager.getProperty("port").value));
                    value.setRedisSessionManager(redisManager);
                    sessionMgr = redisManager;
                } else if (cacheType.equals("memcached")) {
        ...
        if (appInfo.name.equals("images") || appInfo.name.equalsIgnoreCase("front")) {
    ...
    protected void configureMimeTypes(Context context) throws ContainerException {
        Map<String, String> mimeTypes = CatalinaContainer1.getMimeTypes();
    ...
    protected Executor createExecutor(ContainerConfig.Container.Property executorProp) throws ContainerException {
        if (tomcat == null) {
            throw new ContainerException("Cannot load web applications without Tomcat instance!");
        }
        ...
        String executorType = ContainerConfig.getPropertyValue(executorProp, "executorType", "http");
        if (executorType.equals("http")) {
            executor.setName(ContainerConfig.getPropertyValue(executorProp, "name", "http-threadPool"));
        } else {
            executor.setName(ContainerConfig.getPropertyValue(executorProp, "name", "https-threadPool"));
        }
        executor.setNamePrefix(ContainerConfig.getPropertyValue(executorProp, "namePrefix", "catalina-exec-"));
        executor.setMaxThreads(new Integer(ContainerConfig.getPropertyValue(executorProp, "maxThreads", "1000")));
        executor.setMaxIdleTime(new Integer(ContainerConfig.getPropertyValue(executorProp, "maxIdleTime", "60000")));
        executor.setMinSpareThreads(new Integer(ContainerConfig.getPropertyValue(executorProp, "minSpareThreads", "20")));
